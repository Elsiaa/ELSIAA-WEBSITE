        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          {cars.map((c, i) => (
            <div key={i} className="rounded-lg border border-black/[0.07] p-2">
              ...
              <p className="mt-1.5 text-[13px] font-medium" style={inter}>
                {c[0]}
              </p>
              <p className="text-[8.5px] text-[#111111]/50" style={mono}>
                {c[1]}
              </p>
              <div className="mt-1">
                <Pill tone={c[3] as "ok" | "warn" | "crit"}>{c[4]}</Pill>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="hidden w-[36%] flex-none border-l border-black/[0.06] bg-[#FBFBFA] p-3 lg:block">
        ...
        <div className="mt-2 rounded-lg bg-white p-2 shadow-sm">
          ...
          <p className="mt-1 text-[13px] text-[#111111]/55" style={inter}>
            Recommend <b className="text-[#1e6b3c]">−$1,450</b> to clear in 8 days. Regional demand
            softening.
          </p>
          <button
            className="mt-2 w-full rounded-md bg-[#1e6b3c] py-1 text-[13px] font-bold text-white "
            style={mono}
          >
            Approve
          </button>
...
const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    pitch:
      "Every bed, intake, and clinician on one live board — with an AI agent that answers the call, triages, and requests the bed before a human picks up.",
    ...
    mock: (
      <Dash
        name="Patient Flow OS"
        nav={["Census", "Intake", "Beds", "Staff", "Reports"]}
        active={0}
        kpis={[
          ["Occupancy", "87%", "up"],
          ["Avg intake", "14m", "down"],
  ...
  },
  {
    ...
    pitch:
      "Forty-two rooftops on one inventory, one lead pipeline, and an AI pricing engine that clears aging stock before it costs you — decisions, not dashboards.",
    ...
    mock: (
      <Dash
        name="Dealership & Fleet OS"
        nav={["Inventory", "Leads", "Service", "Pricing", "Fleet"]}
        active={0}
        kpis={[
          ["Units", "8,412", "flat"],
          ["Avg age", "38d", "down"],
  ...
  },
  {
    ...
    pitch:
      "Twelve thousand vehicles, live: availability, dynamic pricing, and an AI that inspects every return from photos and drafts the damage claim itself.",
    metrics: [
      "+$1.8M ancillary revenue",
      "12,480 vehicles tracked",
      "99.2% utilization visibility",
    ],
    cta: { label: "Request a walkthrough", href: "/contact" },
    ...
    mock: (
      <Dash
        name="Fleet & Reservations OS"
        nav={["Fleet", "Bookings", "Pricing", "Returns", "Sites"]}
        active={0}
        kpis={[
          ["Utilization", "91%", "up"],
          ["Available", "3,102", "flat"],
          ["Rev/vehicle", "$54", "up"],
          ["Idle days", "1.8", "down"],
        ]}
      >
  ...
  },
  {
    ...
    name: "Dispatch OS",
    pitch:
      "A plumbing business run end to end — the map, the queue, the technician's day — with a voice agent that books the emergency and routes the nearest tech, no dispatcher.",
    ...
    mock: (
      <Dash
        name="Dispatch OS"
        nav={["Map", "Queue", "Techs", "Invoices", "Voice"]}
        active={0}
        kpis={[
          ["Techs live", "18", "flat"],
          ["Jobs today", "63", "up"],
  ...
  },
  {
    ...
    pitch:
      "The month-end close, compressed: every account reconciled automatically, every anomaly flagged with a correcting entry drafted, every step audit-ready.",
    ...
    mock: (
      <Dash
        name="Close & Reconciliation OS"
        nav={["Close", "Recs", "Anomalies", "Journal", "Audit"]}
        active={0}
        kpis={[
          ["Reconciled", "92%", "up"],
          ["Exceptions", "3", "down"],
          ["Days to close", "1.4", "down"],
          ["Caught", "$920k", "up"],
        ]}
      >
        <FinanceClose />
        ...
        <Reveal>
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div className="max-w-3xl">
              ...
              <p
                className="mt-3 text-[13px] leading-relaxed tracking-[0.04em] text-[#111111]/40"
                style={mono}
              >
                Client names, data, and branding shown here have been changed or removed to protect
                privacy.
              </p>
            </div>
            <div className="flex items-center gap-2">
        ...
        <Reveal delay={0.08}>
          <div className="mt-7 grid grid-cols-1 gap-6 md:grid-cols-[minmax(0,360px)_1fr] md:gap-8">
            ...
            <div className="order-2 md:order-1">
              <div className="flex items-center gap-2">
                <span
                  className={`rounded-full px-2 py-0.5 text-[13px] font-bold  ${s.kind === "before" ? "bg-[#111111] text-white" : "bg-[#1e6b3c]/12 text-[#1e6b3c]"}`}
                  style={mono}
                >
              ...
              <h3
                className="mt-2.5 text-xl font-semibold tracking-[-0.02em] text-[#111111]"
                style={inter}
              >
                {s.name}
              </h3>
              ...
              {s.cost && (
                <p
                  className="mt-4 border-l-2 border-[#111111]/20 pl-3 text-[13px] leading-relaxed text-[#111111]/55"
                  style={inter}
                >
                  {s.cost}
                </p>
                ...
                <div className="mt-5 space-y-2">
                  {s.metrics.map((m) => (
                    <div key={m} className="flex items-center gap-2.5">
                      ...
                      <span className="text-[13px] font-medium text-[#111111]" style={inter}>
                        {m}
                      </span>
                    </div>
                  ))}
            ...
            <div className="order-1 md:order-2">
              <Chrome url={s.url}>
                {SLIDES.map((d, i) => (
                  <div
                    key={d.name}
                    className={`absolute inset-0 transition-opacity duration-500 ${i === idx ? "z-10 opacity-100" : "pointer-events-none z-0 opacity-0"}`}
                  >
                    {d.mock}
                  </div>
                ))}
