...
export default function ClientPortal({
  ...
  const updateURL = useCallback(
    (params: { chat?: string; chatState?: string; userId?: string; project?: string }) => {
      ...
      writePortalPlace({
        ...
        chatState:
          (newParams.get("chatState") as "closed" | "sidebar" | "expanded" | null) || "closed",
      });
  ...
  const [chatState, setChatState] = useState<"closed" | "sidebar" | "expanded">(() => {
    ...
    if (
      chatStateFromUrl === "sidebar" ||
      chatStateFromUrl === "expanded" ||
      chatStateFromUrl === "closed"
    ) {
        ...
        <div className="flex items-center justify-center min-h-screen bg-[#F5F5F3] text-[#111]">
          <div className="text-center p-8 max-w-md">
            <div className="mb-8 rounded-xl border border-black/[0.08] bg-white px-6 py-5 text-left shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-[#111]/55 mb-2">
                Signed in as
              </p>
              <p className="text-lg font-semibold text-[#111]">{userName}</p>
              {displayEmail && <p className="text-sm text-[#111]/55 mt-1">{displayEmail}</p>}
        ...
        >
          {chatState === "sidebar" ? (
            <>
              ...
              <div className="p-4 border-b border-black/[0.08] bg-white/80">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="font-bold text-lg text-[#111]">{getProjectForChat()?.title}</h3>
                ...
                <p className="text-sm text-[#111]/55">{getProjectForChat()?.description}</p>
              </div>
              {/* Chat Content */}
              <div
                          ...
                          {showSeparator && (
                            <div className="relative my-4">
                              ...
                              <div className="relative flex justify-center">
                                <span className="bg-[#F5F5F3] px-2 text-xs text-[#111]/55">
                                  New messages
                                </span>
                              </div>
                            </div>
                ...
                {selectedFiles.length > 0 && (
                  <div className="mb-2 space-y-1">
                    {selectedFiles.map((file, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-2 bg-black/[0.06]/70 rounded px-2 py-1 text-xs"
                      >
            ...
            <div className="w-full h-screen bg-white flex flex-col fixed inset-0 z-50">
              ...
              <div
                ref={chatContainerRef}
                className="flex-1 min-h-0 p-4 overflow-y-auto flex flex-col bg-[#F5F5F3]/50"
              >
                {isLoadingMessages ? (
                  <div className="flex-1 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-3">
            ...
            <>
              ...
              <div className="p-4 border-b border-black/[0.08]/50 flex justify-between items-center bg-white/70 relative z-10">
                <div>
                  ...
                  <p className="text-sm text-[#111]/55">Welcome, {userName}!</p>
                </div>
                <DropdownMenu>
                ...
                <div className="p-4 border-b border-black/[0.08]/50 bg-white/20 space-y-3 relative z-10">
                  <div>
                    ...
                    >
                      ...
                      {Array.from(
                        new Set(
                          users.filter((u) => u.company_id).map((u) => u.company_id as string),
                        ),
                      ).map((companyId) => {
                  ...
                  {selectedCompanyId && !loadingProjects && (
                    <div className="text-xs text-[#111]/55">
                      ...
                      for {users.find((u) => u.company_id === selectedCompanyId)?.company?.name}
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="flex-1 bg-[#F5F5F3] relative">
            {chatState === "expanded" ? (
              <div className="w-full h-full bg-white flex flex-col">
                ...
                <div
                  ref={chatContainerRef}
                  className="flex-1 p-4 overflow-y-auto flex flex-col bg-[#F5F5F3]/50"
                >
                  <div className="space-y-4">
                    {messages.map((msg, index) => {
                      const firstUnreadIndex = getFirstUnreadIndex();
                ...
                <div className="p-4 border-t border-black/[0.08]/50 bg-white/70">
                  ...
                  {audioBlob && (
                    <div className="mb-2 bg-black/[0.06] rounded px-3 py-2 flex items-center gap-2">
                      ...
                      <button onClick={cancelRecording} className="text-[#1e6b3c] hover:text-coral">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  )}
                  ...
                  <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`${
                        isRecording
                          ? "bg-[#1e6b3c] hover:bg-[#2e9e58]"
                          : "bg-black/[0.06] hover:bg-black/[0.04]"
                      } rounded-lg p-2`}
                      disabled={uploadingFiles || audioBlob !== null}
                    >
                      <Mic className="w-5 h-5" />
              ...
              <div className="flex items-center justify-center h-full bg-[#F5F5F3] text-[#111]">
                <div className="text-center max-w-md p-8">
                  <div className="mb-4">
                    <svg
                      className="w-20 h-20 mx-auto text-[#111]/55"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
