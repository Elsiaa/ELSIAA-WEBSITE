...
export const mono = { fontFamily: "'SF Mono', ui-monospace, SFMono-Regular, 'IBM Plex Mono', monospace" } as const;
export const inter = { fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'SF Pro Text', 'Inter', system-ui, sans-serif" } as const;
const GREEN = "#1e6b3c";
...
export function Pill({ tone, children }: { tone: "ok" | "warn" | "crit"; children: React.ReactNode }) {
  ...
  return <span className={`rounded-full px-2 py-0.5 text-[8.5px] font-semibold tracking-[0.12em] uppercase ${map[tone]}`} style={mono}>{children}</span>;
}
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-black/[0.09] bg-white shadow-[0_40px_100px_-55px_rgba(17,17,17,0.5)]">
      <div className="flex items-center gap-2 border-b border-black/[0.06] bg-[#FBFBFA] px-4 py-2.5">
        ...
        <span className="ml-3 truncate rounded-md bg-black/[0.04] px-3 py-1 text-[10px] tracking-[0.12em] text-[#111111]/45" style={mono}>{url}</span>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between border-b border-black/[0.06] px-4 py-2.5">
          <div className="flex items-baseline gap-2.5">
            <span className="text-[12px] font-semibold tracking-[-0.01em]" style={inter}>{name}</span>
            <span className="hidden text-[9px] tracking-[0.2em] text-[#111111]/35 uppercase md:inline" style={mono}>{nav[active]}</span>
          </div>
          <span className="flex items-center gap-1.5 rounded-full bg-[#1e6b3c]/10 px-2 py-0.5">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#1e6b3c]" /><span className="text-[8px] tracking-[0.18em] text-[#1e6b3c] uppercase" style={mono}>Live</span>
          </span>
        </div>
        <div className="grid grid-cols-4 gap-px border-b border-black/[0.06] bg-black/[0.05]">
          {kpis.map(([label, val, dir]) => (
            <div key={label} className="bg-white px-3 py-2.5">
              <p className="truncate text-[8px] tracking-[0.18em] text-[#111111]/40 uppercase" style={mono}>{label}</p>
  ...
  return (
    <div className="flex h-full">
      <div className="min-w-0 flex-1 overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2"><p className="text-[9px] tracking-[0.2em] text-[#111111]/45 uppercase" style={mono}>Live census · 214 beds</p><div className="flex gap-1"><Pill tone="ok">Stable 186</Pill><Pill tone="warn">Watch 22</Pill><Pill tone="crit">Critical 6</Pill></div></div>
        <div className="px-3">
          <div className="grid grid-cols-[1fr_1.4fr_auto_1fr_auto] gap-x-2 border-b border-black/[0.06] pb-1 text-[8px] tracking-[0.14em] text-[#111111]/35 uppercase" style={mono}><span>Unit</span><span>Stage</span><span>Status</span><span>Clinician</span><span>Elapsed</span></div>
          {rows.map((r, i) => (<div key={i} className="grid grid-cols-[1fr_1.4fr_auto_1fr_auto] items-center gap-x-2 border-b border-black/[0.04] py-[7px] text-[10px]" style={inter}><span className="font-medium">{r[0]}</span><span className="text-[#111111]/60">{r[1]}</span><Pill tone={r[2] as "ok" | "warn" | "crit"}>{r[2] === "ok" ? "OK" : r[2] === "warn" ? "Wait" : "Now"}</Pill><span className="text-[#111111]/60">{r[3]}</span><span className="tabular-nums text-[#111111]/50" style={mono}>{r[4]}</span></div>))}
        </div>
      </div>
      <div className="hidden w-[38%] flex-none border-l border-black/[0.06] bg-[#FBFBFA] p-3 lg:block">
        ...
        <div className="mt-2 space-y-2"><div className="rounded-lg bg-white p-2 shadow-sm"><p className="text-[9px] text-[#111111]/45" style={mono}>PATIENT</p><p className="mt-0.5 text-[10px]" style={inter}>"...chest tightness since this morning, and I'm short of breath."</p></div><div className="rounded-lg border border-[#1e6b3c]/25 bg-[#1e6b3c]/[0.06] p-2"><p className="text-[9px] tracking-[0.14em] text-[#1e6b3c] uppercase" style={mono}>Agent · triage</p><p className="mt-0.5 text-[10px]" style={inter}>Flagged <b>urgent</b> · cardiac pathway · bed request sent to 4-West.</p></div></div>
        <div className="mt-3 flex items-center gap-1.5">{[3, 6, 4, 8, 5, 7, 3, 6, 9, 4, 6, 3].map((h, i) => (<span key={i} className="w-1 rounded-full bg-[#1e6b3c]/50" style={{ height: h * 2 }} />))}<span className="ml-1 text-[8px] text-[#111111]/40" style={mono}>listening</span></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-[42%] flex-none border-l border-black/[0.06] p-3">
        ...
        {[["Delgado · burst pipe", "crit", "assigned · 6m"], ["Okafor · water heater", "warn", "en route"], ["Kim · inspection", "ok", "scheduled"]].map((j, i) => (<div key={i} className="mt-2 rounded-lg border border-black/[0.07] p-2"><div className="flex items-center justify-between"><span className="text-[10px] font-medium" style={inter}>{j[0]}</span><Pill tone={j[1] as "ok" | "warn" | "crit"}>{j[1] === "crit" ? "Now" : j[1] === "warn" ? "Soon" : "Set"}</Pill></div><p className="mt-0.5 text-[8.5px] text-[#111111]/50" style={mono}>{j[2]}</p></div>))}
        <div className="mt-2 rounded-lg border border-[#1e6b3c]/25 bg-[#1e6b3c]/[0.06] p-2"><p className="text-[9px] tracking-[0.14em] text-[#1e6b3c] uppercase" style={mono}>Voice agent</p><p className="mt-0.5 text-[9.5px]" style={inter}>Booked the Delgado emergency, matched the nearest tech, and texted the ETA — no dispatcher.</p></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full">
      <div className="min-w-0 flex-1 p-3">
        <div className="flex items-center justify-between"><p className="text-[9px] tracking-[0.2em] text-[#111111]/45 uppercase" style={mono}>Close · Day 2 of 2</p><Pill tone="ok">92% reconciled</Pill></div>
        <div className="mt-2">{rows.map((r, i) => (<div key={i} className="grid grid-cols-[1.4fr_1fr_auto] items-center gap-2 border-b border-black/[0.04] py-[7px] text-[10px]" style={inter}><span className="truncate font-medium">{r[0]}</span><span className="tabular-nums text-[#111111]/60" style={mono}>{r[1]}</span><Pill tone={r[3] as "ok" | "warn" | "crit"}>{r[2]}</Pill></div>))}</div>
      </div>
      <div className="hidden w-[38%] flex-none border-l border-black/[0.06] bg-[#FBFBFA] p-3 lg:block">
        <p className="text-[9px] tracking-[0.2em] text-[#1e6b3c] uppercase" style={mono}>Anomaly · AI</p>
        <div className="mt-2 rounded-lg bg-white p-2 shadow-sm"><p className="text-[10px]" style={inter}>Deferred revenue · Contract #8841</p><p className="mt-1 text-[9px] text-[#111111]/55" style={inter}>Recognition schedule off by <b className="text-[#b42318]">$41,200</b> vs. contract terms. Draft correcting entry ready.</p><button className="mt-2 w-full rounded-md border border-[#111111]/20 py-1 text-[9px] font-bold tracking-[0.15em] uppercase" style={mono}>Review entry</button></div>
...
export function RentalFleet() {
  return (
    <div className="flex h-full flex-col p-3">
      <div className="flex items-center justify-between"><p className="text-[9px] tracking-[0.2em] text-[#111111]/45 uppercase" style={mono}>Fleet board · 12,480 vehicles</p><div className="flex gap-1"><Pill tone="ok">Available 3,102</Pill><Pill tone="warn">Cleaning 288</Pill><Pill tone="crit">Damage AI 14</Pill></div></div>
      ...
      <div className="rounded-lg border border-[#1e6b3c]/25 bg-[#1e6b3c]/[0.06] p-2"><p className="text-[9px] tracking-[0.14em] text-[#1e6b3c] uppercase" style={mono}>AI return inspection</p><p className="mt-0.5 text-[10px]" style={inter}>Unit EV-2231 · <b>2 new dents</b> detected from 6 photos · claim drafted, $840 estimated.</p></div>
    </div>
  );
}
  ...
  return (
    <div className="relative h-full w-full bg-[#e9eaec] p-3">
      <div className="absolute top-3 left-3 w-[62%] rotate-[-1.5deg] overflow-hidden rounded-md border border-black/15 bg-white shadow-lg">
        <div className="flex items-center gap-1 bg-[#1f7a45] px-2 py-1"><span className="h-2 w-2 rounded-full bg-white/70" /><span className="text-[8px] font-semibold text-white" style={inter}>jobs_final_v7_USE THIS ONE.xlsx</span></div>
      ...
      <div className="absolute top-1/2 left-1/2 w-24 -translate-x-1/2 -translate-y-1/2 rotate-[6deg] bg-[#fde68a] p-2 shadow-md"><p className="text-[8px] leading-tight text-black/70" style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>call back the Delgado job — DON'T double book!!</p></div>
    </div>
  );
}
  ...
  return (
    <div className="relative h-full w-full bg-[#eef0ef] p-3">
      ...
      <div className="absolute inset-3 rounded-md border-2 border-[#c9cdcb] bg-white shadow-inner">
        <div className="flex items-center justify-between border-b-2 border-[#e2e5e3] px-3 py-1.5"><span className="text-[10px] font-bold tracking-wide text-[#1f3a5f]" style={scrawl}>BED BOARD — 4 WEST</span><span className="text-[9px] text-[#b42318]" style={scrawl}>?? = who knows</span></div>
        <div className="grid grid-cols-3 gap-px bg-[#e2e5e3]">
          {[["401", "Amir?", "#1f3a5f"], ["402", "DISCH?", "#b42318"], ["403", "—", "#8a5a12"], ["404", "ICU xfer", "#1f3a5f"], ["405", "clean??", "#b42318"], ["406", "Lee - hold", "#111"]].map(([bed, note, c], i) => (
            <div key={i} className="bg-white px-2 py-2"><p className="text-[9px] font-bold text-[#111111]/70" style={mono}>{bed}</p><p className="mt-0.5 text-[12px] leading-none" style={{ ...scrawl, color: c as string }}>{note}</p></div>
          ))}
        </div>
        <div className="flex items-center gap-2 px-3 py-2"><span className="text-[9px] text-black/50" style={scrawl}>erase + rewrite every shift · phone for the rest</span></div>
      </div>
      {/* phone-tree sticky + fax */}
      <div className="absolute top-6 right-6 w-24 rotate-[5deg] bg-[#fde68a] p-2 shadow-md"><p className="text-[8px] leading-tight text-black/70" style={scrawl}>intake line ringing — page Dr. on call?? ext 4021</p></div>
  ...
  return (
    <div className="relative h-full w-full bg-[#e9eaec] p-3">
      ...
      <div className="absolute inset-x-3 top-3 h-[64%] overflow-hidden rounded-md border border-black/15 bg-white shadow-lg">
        <div className="flex items-center gap-1 bg-[#1f7a45] px-2 py-1"><span className="h-2 w-2 rounded-full bg-white/70" /><span className="text-[8px] font-semibold text-white" style={inter}>CLOSE_Q3_final_FINAL_v12 (Rana's copy).xlsx</span></div>
      ...
      {/* sticky + printout */}
      <div className="absolute right-6 bottom-6 w-24 rotate-[4deg] bg-[#fde68a] p-2 shadow-md"><p className="text-[8px] leading-tight text-black/70" style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>doesn't tie out by $41k — DO NOT SEND yet</p></div>
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-6 md:grid-cols-[minmax(0,360px)_1fr] md:gap-10">
      ...
      <div className="md:pt-2">
        <p className="text-[10px] tracking-[0.28em] text-[#111111]/40 uppercase" style={mono}>{v.sector}</p>
        ...
        <div className="mt-5 inline-flex rounded-full border border-black/10 bg-white p-1">
          <button onClick={() => setSide("before")} className={`rounded-full px-4 py-1.5 text-[10px] font-bold tracking-[0.16em] uppercase transition-all ${!isAfter ? "bg-[#111111] text-white" : "text-[#111111]/55"}`} style={mono}>The old way</button>
        ...
        <div className="mt-5 space-y-2.5">
          {active.ledger.map((row) => (
            <div key={row.label} className="flex items-baseline justify-between gap-3 border-b border-black/[0.06] pb-2">
              <span className="text-[11px] tracking-[0.06em] text-[#111111]/45 uppercase" style={mono}>{row.label}</span>
        ...
        {v.live && (
          <a href={v.live.href} target="_blank" rel="noreferrer" className="mt-5 inline-block text-[11px] tracking-[0.24em] text-[#1e6b3c] uppercase hover:underline" style={mono}>
            {v.live.label} ↗
          </a>
        )}
      </div>
      ...
      <div>
        ...
        <p className="mt-3 text-[11px] tracking-[0.04em] text-[#111111]/40" style={mono}>
