...
export function Pill({
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[8.5px] font-semibold tracking-[0.12em]  ${map[tone]}`}
      style={mono}
    >
      {children}
    </span>
      ...
      <div className="hidden w-12 flex-none flex-col items-center gap-4 border-r border-black/[0.06] bg-[#FBFBFA] py-4 sm:flex">
        ...
        {nav.map((label, i) => (
          <button
            ...
            aria-label={label}
            onClick={() => setActive(i)}
            className={`h-5 w-5 rounded-md transition-all ${i === active ? "bg-[#1e6b3c]/15 ring-1 ring-[#1e6b3c]/40" : "bg-black/[0.05] hover:bg-black/10"}`}
          />
        ))}
      </div>
        ...
        <div className="grid grid-cols-4 gap-px border-b border-black/[0.06] bg-black/[0.05]">
          {kpis.map(([label, val, dir]) => (
            <div key={label} className="bg-white px-3 py-2.5">
              ...
              <p
                className="mt-1 flex items-baseline gap-1 text-[15px] font-semibold tabular-nums tracking-[-0.02em]"
                style={inter}
              >
                {val}
                <span
                  className={dir === "down" ? "text-[#b42318]" : "text-[#1e6b3c]"}
                  style={{ fontSize: 9 }}
                >
                  {dir === "up" ? "▲" : dir === "down" ? "▼" : "—"}
                </span>
      ...
      <div className="min-w-0 flex-1 overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2">
          ...
          <div className="flex gap-1">
            <Pill tone="ok">Stable 186</Pill>
            <Pill tone="warn">Watch 22</Pill>
            <Pill tone="crit">Critical 6</Pill>
          </div>
        </div>
        <div className="px-3">
          <div
            className="grid grid-cols-[1fr_1.4fr_auto_1fr_auto] gap-x-2 border-b border-black/[0.06] pb-1 text-[13px] text-[#111111]/35 "
            style={mono}
          >
            <span>Unit</span>
            <span>Stage</span>
          ...
          {rows.map((r, i) => (
            <div
              key={i}
              className="grid grid-cols-[1fr_1.4fr_auto_1fr_auto] items-center gap-x-2 border-b border-black/[0.04] py-[7px] text-[13px]"
              style={inter}
            >
              <span className="font-medium">{r[0]}</span>
              <span className="text-[#111111]/60">{r[1]}</span>
              <Pill tone={r[2] as "ok" | "warn" | "crit"}>
                {r[2] === "ok" ? "OK" : r[2] === "warn" ? "Wait" : "Now"}
              </Pill>
      ...
      <div className="hidden w-[38%] flex-none border-l border-black/[0.06] bg-[#FBFBFA] p-3 lg:block">
        ...
        <div className="mt-2 space-y-2">
          <div className="rounded-lg bg-white p-2 shadow-sm">
            ...
            <p className="mt-0.5 text-[13px]" style={inter}>
              "...chest tightness since this morning, and I'm short of breath."
            </p>
          </div>
          <div className="rounded-lg border border-[#1e6b3c]/25 bg-[#1e6b3c]/[0.06] p-2">
            ...
            <p className="mt-0.5 text-[13px]" style={inter}>
              Flagged <b>urgent</b> · cardiac pathway · bed request sent to 4-West.
            </p>
          </div>
        </div>
        <div className="mt-3 flex items-center gap-1.5">
          ...
          <span className="ml-1 text-[13px] text-[#111111]/40" style={mono}>
            listening
          </span>
        </div>
      </div>
  ...
  return (
    <div className="flex h-full">
      <div className="relative min-w-0 flex-1 overflow-hidden bg-[#eef1f0]">
        ...
        {[
          ...
          <span
            ...
            style={{ left: `${(x as number) / 3}%`, top: `${(y as number) / 2}%` }}
          >
            <span
      ...
      <div className="w-[42%] flex-none border-l border-black/[0.06] p-3">
        ...
        {[
          ["Delgado · burst pipe", "crit", "assigned · 6m"],
          ...
          <div key={i} className="mt-2 rounded-lg border border-black/[0.07] p-2">
            <div className="flex items-center justify-between">
              ...
              <Pill tone={j[1] as "ok" | "warn" | "crit"}>
                {j[1] === "crit" ? "Now" : j[1] === "warn" ? "Soon" : "Set"}
              </Pill>
            </div>
        ...
        <div className="mt-2 rounded-lg border border-[#1e6b3c]/25 bg-[#1e6b3c]/[0.06] p-2">
          ...
          <p className="mt-0.5 text-[9.5px]" style={inter}>
            Booked the Delgado emergency, matched the nearest tech, and texted the ETA — no
            dispatcher.
          </p>
        </div>
      </div>
    ...
    <div className="flex h-full">
      <div className="min-w-0 flex-1 p-3">
        ...
        <div className="mt-2">
          {rows.map((r, i) => (
            <div
              key={i}
              className="grid grid-cols-[1.4fr_1fr_auto] items-center gap-2 border-b border-black/[0.04] py-[7px] text-[13px]"
              style={inter}
            >
      ...
      <div className="hidden w-[38%] flex-none border-l border-black/[0.06] bg-[#FBFBFA] p-3 lg:block">
        ...
        <div className="mt-2 rounded-lg bg-white p-2 shadow-sm">
          <p className="text-[13px]" style={inter}>
            Deferred revenue · Contract #8841
          </p>
          <p className="mt-1 text-[13px] text-[#111111]/55" style={inter}>
            Recognition schedule off by <b className="text-[#b42318]">$41,200</b> vs. contract
            terms. Draft correcting entry ready.
          </p>
          <button
            className="mt-2 w-full rounded-md border border-[#111111]/20 py-1 text-[13px] font-bold "
            style={mono}
          >
            Review entry
          </button>
        </div>
        <p className="mt-3 text-[13px] text-[#111111]/40 " style={mono}>
          Audit trail
        </p>
        <div className="mt-1.5 space-y-1">
          {["Every entry sourced", "Approvals timestamped", "SOX-ready export"].map((t, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-[#1e6b3c]" />
              <span className="text-[9.5px] text-[#111111]/65" style={inter}>
                {t}
              </span>
            </div>
          ))}
  ...
  return (
    <div className="flex h-full flex-col p-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <Pill tone="ok">Available 3,102</Pill>
          <Pill tone="warn">Cleaning 288</Pill>
          <Pill tone="crit">Damage AI 14</Pill>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-[#1e6b3c]/25 bg-[#1e6b3c]/[0.06] p-2">
        ...
        <p className="mt-0.5 text-[13px]" style={inter}>
          Unit EV-2231 · <b>2 new dents</b> detected from 6 photos · claim drafted, $840 estimated.
        </p>
      </div>
    </div>
  );
    ...
    <div className="relative h-full w-full bg-[#e9eaec] p-3">
      <div className="absolute top-3 left-3 w-[62%] rotate-[-1.5deg] overflow-hidden rounded-md border border-black/15 bg-white shadow-lg">
        <div className="flex items-center gap-1 bg-[#1f7a45] px-2 py-1">
          ...
          <span className="text-[13px] font-semibold text-white" style={inter}>
            jobs_final_v7_USE THIS ONE.xlsx
          </span>
        </div>
        <div className="grid grid-cols-6">
          {Array.from({ length: 42 }).map((_, i) => (
            <div
              key={i}
              className={`h-3 border-r border-b border-black/[0.08] ${i % 6 === 0 ? "bg-[#f3f4f6]" : "bg-white"} ${[7, 13, 19, 26].includes(i) ? "bg-[#fde68a]/60" : ""} ${[9, 22].includes(i) ? "bg-[#fecaca]/60" : ""}`}
            />
          ))}
        </div>
      </div>
      <div className="absolute top-6 right-4 w-[30%] rotate-[3deg] rounded-sm border border-black/15 bg-[#fffdf5] p-2 shadow-lg">
        ...
        <p className="mt-2 text-center text-[7px] tracking-widest text-black/40" style={mono}>
          WORK ORDER · 3-PART
        </p>
      </div>
      <div className="absolute bottom-3 left-6 w-[46%] -rotate-1 overflow-hidden rounded-md border border-black/15 bg-white shadow-lg">
        <div
          className="bg-[#f3f4f6] px-2 py-1 text-[13px] font-semibold text-black/60"
          style={inter}
        >
          Re: Re: Re: FWD: schedule?? (17)
        </div>
      ...
      <div className="absolute right-6 bottom-5 w-[34%] rotate-2 overflow-hidden rounded-sm border border-black/25 bg-[#0b2b2b] p-2 shadow-lg">
        <p className="text-[7px] leading-relaxed text-[#37d67a]" style={mono}>
          &gt; DISPATCH.EXE v3.1
          <br />
          &gt; F2=SCHEDULE F4=INVOICE
          <br />
          &gt; REC 1182 LOCKED BY: KAREN
          <br />
          &gt; _
        </p>
      </div>
      <div className="absolute top-1/2 left-1/2 w-24 -translate-x-1/2 -translate-y-1/2 rotate-[6deg] bg-[#fde68a] p-2 shadow-md">
        <p
          className="text-[13px] leading-tight text-black/70"
    ...
    <div className="relative h-full w-full bg-[#eef0ef] p-3">
      ...
      <div className="absolute top-6 right-6 w-24 rotate-[5deg] bg-[#fde68a] p-2 shadow-md">
        <p className="text-[13px] leading-tight text-black/70" style={scrawl}>
          intake line ringing — page Dr. on call?? ext 4021
        </p>
      </div>
      <div className="absolute bottom-4 left-8 w-28 -rotate-2 rounded-sm border border-black/15 bg-[#fffdf5] p-1.5 shadow">
        <p className="text-center text-[7px] tracking-widest text-black/40" style={mono}>
          FAX — INTAKE FORM
          <br />
          pg 1 of 3
        </p>
...
export function BeforeSpreadsheet() {
  return (
    <div className="relative h-full w-full bg-[#e9eaec] p-3">
      ...
      {/* calculator */}
      <div className="absolute bottom-4 left-6 w-20 rotate-[-3deg] rounded-md border border-black/20 bg-[#1a1a1a] p-1.5 shadow-lg">
        <div
          className="mb-1 h-3 rounded-sm bg-[#9fd8a6] px-1 text-right text-[7px] leading-3 text-black/70"
          style={mono}
        >
          41,200
        </div>
        <div className="grid grid-cols-4 gap-0.5">
          {Array.from({ length: 12 }).map((_, i) => (
            <span key={i} className="h-2 rounded-[1px] bg-white/25" />
          ))}
        </div>
      </div>
      {/* sticky + printout */}
      <div className="absolute right-6 bottom-6 w-24 rotate-[4deg] bg-[#fde68a] p-2 shadow-md">
        <p
          ...
          style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}
        >
          doesn't tie out by $41k — DO NOT SEND yet
        </p>
      </div>
      <div className="absolute right-28 bottom-4 w-20 -rotate-2 rounded-sm border border-black/15 bg-white p-1.5 shadow">
        <div className="space-y-1">
          {["w-full", "w-4/5", "w-full", "w-3/5"].map((w, i) => bar(w, "bg-black/12", "h-1"))}
        </div>
        <p className="mt-1 text-center text-[6.5px] tracking-widest text-black/40" style={mono}>
          RECON — PRINTED
        </p>
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-6 md:grid-cols-[minmax(0,360px)_1fr] md:gap-10">
      ...
      <div className="md:pt-2">
        ...
        <h3 className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-[#111111]" style={inter}>
          {v.name}
        </h3>
        <p className="mt-2.5 text-[14px] leading-relaxed text-[#111111]/60" style={inter}>
          {v.line}
        </p>
        ...
        <div className="mt-5 inline-flex rounded-full border border-black/10 bg-white p-1">
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-[13px] font-bold  transition-all ${!isAfter ? "bg-[#111111] text-white" : "text-[#111111]/55"}`}
            style={mono}
          >
            The old way
          </button>
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-[10px] font-bold tracking-[0.16em] uppercase transition-all ${isAfter ? "bg-[#1e6b3c] text-white" : "text-[#111111]/55"}`}
            style={mono}
          >
            With ELSIAA
          </button>
        ...
        <div className="mt-5 space-y-2.5">
          {active.ledger.map((row) => (
            ...
            >
              <span className="text-[13px] tracking-[0.06em] text-[#111111]/45 " style={mono}>
                {row.label}
              </span>
              <span
                className={`text-[14px] font-semibold tabular-nums ${isAfter ? "text-[#1e6b3c]" : "text-[#b42318]"}`}
                style={inter}
              >
                {row.value}
              </span>
      ...
      <div>
        <Chrome url={isAfter ? v.after.url : "the-old-way.xlsx"}>
          <div
            className={`absolute inset-0 transition-opacity duration-500 ${isAfter ? "z-10 opacity-100" : "pointer-events-none opacity-0"}`}
          >
            {v.after.mock}
          </div>
          <div
        ...
        <p className="mt-3 text-[13px] tracking-[0.04em] text-[#111111]/40" style={mono}>
          {isAfter
            ? "The system, running live. Click the tabs — it's real."
            : "What it replaced: spreadsheets, paper, and the phone."}
        </p>
      </div>
    </div>
  );
