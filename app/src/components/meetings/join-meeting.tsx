...
export default function JitsiMeetClient({
  ...
  const [showRecordingStartedNotification, setShowRecordingStartedNotification] = useState(false);
      ...
      try {
        ...
        jitsiApiRef.current.addEventListener("recordingStatusChanged", (event: any) => {
          ...
          if (!wasRecording && event.on) {
            ...
            setTimeout(() => {
              setShowRecordingStartedNotification(false);
            }, 5000);
          }
        });
  ...
  return (
    <div className="relative h-screen w-full bg-background flex flex-col">
      ...
      {showRecordingStartedNotification && (
        <div className="bg-primary/90 border-b border-primary px-4 py-3 flex items-center justify-between z-30 text-primary-foreground">
          <div className="flex items-center gap-3">
            <Circle className="h-5 w-5 text-primary-foreground animate-pulse fill-primary-foreground" />
            <div>
              <p className="text-primary-foreground font-medium">Recording Started</p>
              <p className="text-primary-foreground/90 text-sm">
                The meeting is now being recorded. Everything will be saved.
              </p>
            </div>
          </div>
          <Button
            onClick={() => setShowRecordingStartedNotification(false)}
            variant="ghost"
            size="sm"
      ...
      {showRecordingNotification && recordingStopped && (
        <div className="bg-steel/95 border-b border-navy px-4 py-3 flex items-center justify-between z-30">
          <div className="flex items-center gap-3">
