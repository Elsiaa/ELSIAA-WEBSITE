...
function SuccessPageContent() {
  ...
  const getStoredEmail = () => {
    if (typeof window !== "undefined") {
      return (
        sessionStorage.getItem("payment_customer_email") ||
        sessionStorage.getItem("payment_payer_email") ||
        ""
      );
    }
    return "";
  ...
  useEffect(() => {
    ...
    const storedToken = urlToken || sessionStorage.getItem("payment_public_token");
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const amt =
          typeof (data as { displayAmount?: number }).displayAmount === "number"
        ...
        if (!cancelled) {
          setError(
            "Could not confirm payment with the server. If you were charged, contact support.",
          );
        }
      }
    };
    ...
    if (clientSecret && stripe) {
      if (isSetupIntent) {
        ...
        stripe
          ...
          .then(({ setupIntent }) => {
            logToServer("info", "Setup Intent retrieved", {
              id: setupIntent?.id,
            ...
            if (setupIntent) {
              ...
              if (setupIntent.status === "succeeded") {
                ...
                if (publicToken && setupIntent.payment_method) {
                  console.log("Saving payment method with:", {
                    publicToken,
            ...
            if (paymentIntent) {
              ...
              try {
                const res = await fetch(
                  `/api/payments/payment-intent-method?paymentIntentId=${paymentIntent.id}`,
                );
                if (res.ok) {
                  const data = await res.json();
                ...
                logToServer("warn", "Could not fetch actual payment method, using metadata", {
                  error: String(e),
                });
              }
              ...
              if (isValidStatus) {
                ...
                const isStandalonePayment =
                  !pi.metadata.public_token || pi.metadata.public_token === "";
                ...
                const storedName =
                  typeof window !== "undefined"
                    ...
                    : "";
                const customerName =
                  pi.metadata.customer_name || customerNameParam || storedName || name;
                ...
                if (isStandalonePayment && isCardOrACH && customerName && !notificationSent) {
                  ...
                  fetch("/api/payments/notify-payment", {
                    ...
                    .catch((err) => {
                      logToServer("error", "Failed to send payment notification email", {
                        error: String(err),
                      });
                    });
                } else {
                  logToServer("info", "Notification email not sent", {
                    reason: !isStandalonePayment
                      ? "not standalone"
                      : !isCardOrACH
                        ? "not card/ACH"
                        : !customerName
                ...
                const invoiceEmail =
                  customerEmailParam ||
                  storedEmail ||
                  (achMethod ? pi.metadata.payer_email || "" : "") ||
                  email;
                ...
                if (customerName && invoiceEmail && !invoiceAutoSent && !pi.metadata.public_token) {
                  ...
                  logToServer("info", "Auto-sending payment receipt email", {
                    ...
                    email: invoiceEmail,
                    amount: total,
                    emailSource: customerEmailParam
                      ? "url_param"
                      : pi.metadata.payer_email
                        ? "metadata"
                        : "state",
                  });
                  ...
                  fetch("/api/payments/send-receipt", {
                    ...
                    .then(async (response) => {
                      if (response.ok) {
                        ...
                        logToServer("error", "Failed to auto-send receipt", {
                          status: response.status,
                        });
                  ...
                  setShowForm(true);
                  logToServer("info", "Prefilling name, showing form for email", {
                    name: customerName,
                  });
                } else if (!customerName && invoiceEmail) {
                  ...
                  setEmail(invoiceEmail);
                  setShowForm(true);
                  logToServer("info", "Prefilling email, showing form for name", {
                    email: invoiceEmail,
                  });
                } else {
                ...
                if (pi.metadata.public_token) {
                  logToServer("info", "Processing payment with public_token", {
                    public_token: pi.metadata.public_token,
                  });
                  ...
                  if (paymentMethodId) {
                    logToServer(
                      "info",
                      "Calling save-payment-method API (non-blocking after record-success)",
                    );
                    fetch("/api/payments/save-payment-method", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      ...
                      .then(async (response) => {
                        ...
                        try {
                          ...
                          logToServer("warn", "Error saving payment method to account", {
                            error: String(err),
                          });
                        }
                        if (!response.ok) {
                      ...
                      .catch((err) => {
                        logToServer(
                          "warn",
                          "save-payment-method failed (status already recorded)",
                          {
                            error: String(err),
                          },
                        );
      ...
      return;
    } else if (!isSetupIntent && paymentIntentIdParam?.startsWith("pi_") && !clientSecret) {
          ...
          {isSetupOnly ? (
            <div className="text-center">
              <p className="text-primary mb-4">
                Your payment method has been securely saved. No charge was made.
              </p>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  {isACH && payerEmail && (
                    <p className="text-xs text-muted-foreground">Email prefilled from payment.</p>
                  )}
                </div>
