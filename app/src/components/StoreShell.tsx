    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4">
        ...
        <nav className="flex items-center gap-4 md:gap-6">
          <a
            href="/store"
            className="text-[13.5px] font-medium text-[#111111] underline-offset-4 hover:underline"
          >
            The Store
          </a>
          <a
            href="mailto:info@elsiaa.com"
            className="hidden text-[13.5px] text-[#111111]/60 hover:text-[#111111] md:inline"
          >
            info@elsiaa.com
          </a>
          <a
            href="/quote"
            className="hidden text-[13.5px] font-medium text-[#1e6b3c] hover:underline md:inline"
          >
            Get a quote →
          </a>
          <button
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {lines.length === 0 ? (
            <p className="pt-10 text-center text-[14px] text-[#111111]/50">
            ...
            lines.map((l) => {
              ...
              return (
                <div
                  key={`${l.slug}-${l.size}`}
                  className="flex gap-4 border-b border-black/[0.05] py-4"
                >
                  ...
                  <div className="flex flex-1 flex-col">
                    <div className="flex items-baseline justify-between gap-3">
                      <a
                        href={`/product/${p.slug}`}
                        className="text-[13.5px] font-medium text-[#111111] hover:underline"
                      >
                        {p.name}
                      </a>
                    ...
                    <div className="mt-auto flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="text-[12.5px] text-[#111111]/45 hover:text-[#111111]"
                      >
                        Remove
                      </button>
                    </div>
        ...
        {lines.length > 0 && (
          <div className="border-t border-black/[0.06] px-6 py-5">
            ...
            <a
              href="/checkout"
              className="mt-4 flex min-h-[50px] w-full items-center justify-center rounded-full bg-[#111111] text-[14.5px] font-semibold text-white transition-opacity hover:opacity-85"
            >
              Checkout →
            </a>
            <a
              href="/cart"
              onClick={() => setDrawerOpen(false)}
              className="mt-2.5 block text-center text-[13px] text-[#111111]/55 hover:text-[#111111]"
            >
              View full cart
            </a>
          </div>
  ...
  return (
    ...
    >
      {toasts.map((t) => (
        <div
          key={t.id}
          className="rounded-full bg-[#111111] px-5 py-2.5 text-[13px] font-medium text-white shadow-lg"
          style={{ animation: "stToast .25s ease" }}
        >
          {t.text}
        </div>
      ))}
  ...
  return (
    <footer className="border-t border-black/[0.06] bg-white" style={F}>
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-2 px-6 py-10 text-center">
        <p className="text-[13px] text-[#111111]/50">
        ...
        <p className="mt-1 text-[12.5px] text-[#111111]/35">
          © {new Date().getFullYear()} ELSIAA Merch — AI Done Better
        </p>
      </div>
    </footer>
  );
