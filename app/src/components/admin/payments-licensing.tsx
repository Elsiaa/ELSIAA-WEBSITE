...
export default function PaymentsLicensing({
  ...
  const [subscriptionBillingInterval, setSubscriptionBillingInterval] = useState<
  ...
  const [subscriptionTransactions, setSubscriptionTransactions] = useState<
    Record<string, ProjectSubscriptionTransaction[]>
  >({});
  ...
  const [loadingEditMessage, setLoadingEditMessage] = useState<
    "Setting up edit…" | "Loading payment details…"
  ...
  const [editingPaymentsAuthDeviceId, setEditingPaymentsAuthDeviceId] = useState<string | null>(
    null,
  );
  ...
  const [accessOverrideByProject, setAccessOverrideByProject] = useState<
    Record<string, "allowed" | "blocked" | null>
  >({});
  const [loadingAccessOverrideProjectId, setLoadingAccessOverrideProjectId] = useState<
    string | null
  >(null);

  const [deviceLimitInputByProject, setDeviceLimitInputByProject] = useState<
  ...
  const [deviceLimitOverrideByProject, setDeviceLimitOverrideByProject] = useState<
  ...
  const [extensionSourceByProject, setExtensionSourceByProject] = useState<
    Record<
      string,
      { owner: string; repo: string; ref: string; deploymentVisibleFrom?: string | null } | null
    >
  >({});
  const [extensionUrlInputByProject, setExtensionUrlInputByProject] = useState<
    Record<string, string>
  >({});
  const [savingExtensionSourceProjectId, setSavingExtensionSourceProjectId] = useState<
    string | null
  >(null);
  const [deploymentVisibleFromInputByProject, setDeploymentVisibleFromInputByProject] = useState<
    Record<string, string>
  >({});
  const [savingDeploymentDateProjectId, setSavingDeploymentDateProjectId] = useState<string | null>(
    null,
  );
        ...
        if (batchRes.ok) {
          ...
          for (const company of visibleCompanies) {
            ...
            if (row) {
              ...
              Object.entries(row.subscriptionsByProject || {}).forEach(
                ([projectId, projectSubs]) => {
            ...
            visibleCompanies.map(async (company) => {
              try {
                ...
                if (feesSubsRes.ok) {
                  ...
                  Object.entries(subscriptionsByProject || {}).forEach(
                    ([projectId, projectSubs]) => {
                      subscriptionsData[projectId] = projectSubs as ProjectSubscription[];
                    },
                  );
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          `/api/admin/companies/${authorizationsCompanyId}/authorizations-data`,
          {
            cache: "no-store",
          },
        );
        ...
        for (const r of rows) {
          if (r.extensionSource) {
            urlDrafts[r.id] =
              `https://github.com/${r.extensionSource.owner}/${r.extensionSource.repo}/tree/${r.extensionSource.ref}`;
      ...
      if (res.ok) {
        ...
        if (project) {
          const statusRes = await fetch(
            `/api/companies/${project.companyId}/payment-status?skipPreemptive=1`,
          );
          if (statusRes.ok) {
            const { status } = await statusRes.json();
    ...
    try {
      const res = await fetch(`/api/projects/${selectedProjectId}/subscriptions`, {
        ...
        body: JSON.stringify({
          ...
          billingDayOfMonth:
            subscriptionBillingDayOfMonth !== "" ? subscriptionBillingDayOfMonth : undefined,
          billingDayOfWeek:
            subscriptionBillingDayOfWeek !== "" ? subscriptionBillingDayOfWeek : undefined,
        }),
      });
  ...
  const getPaymentRequestId = (item: {
    paymentRequestId?: string | null;
    payment_request_id?: string | null;
  }): string | null => {
    return (
      item.paymentRequestId ??
      (item as { payment_request_id?: string | null }).payment_request_id ??
      null
    );
  };

  // Open edit modal with form prefilled from data we already have. No page refetch.
  const openEditModalWithForm = (
  ...
  const handleDryRunBilling = async () => {
    ...
    try {
      ...
      toast.success(
        "Dry run complete. Review what would be billed below, then click Run billing to charge.",
      );
    } catch {
  ...
  const handleRunBilling = async () => {
    ...
    try {
      ...
      setDryRunResult(null);
      const statusRes = await fetch(
        `/api/companies/${runBillingCompanyId}/payment-status?skipPreemptive=1`,
      );
      if (statusRes.ok) {
        const { status } = await statusRes.json();
      ...
      try {
        const feesSubsRes = await fetch(
          `/api/companies/${runBillingCompanyId}/fees-and-subscriptions`,
        );
        if (feesSubsRes.ok) {
          const { feesByProject, subscriptionsByProject } = await feesSubsRes.json();
          Object.entries(feesByProject || {}).forEach(([projectId, projectFees]) => {
  ...
  const handleOpenEditForFee = async (fee: ProjectFee) => {
          ...
          if (data?.request) {
            ...
            if (editPaymentFormInitialRef.current) {
              editPaymentFormInitialRef.current = {
                ...
                recipientEmail:
                  r.recipient_email ?? editPaymentFormInitialRef.current.recipientEmail,
              };
            }
          }
        })
    ...
    try {
      const res = await fetch(
        `/api/admin/payments/resolve-request?feeId=${encodeURIComponent(fee.id)}`,
      );
      if (!res.ok) {
        setLoadingEditPayment(false);
      ...
      if (newPrId) {
        ...
        if (request) {
          openEditModalWithForm(newPrId, {
            ...
            nextBillingDate: request.next_billing_date
              ? request.next_billing_date.slice(0, 10)
              : "",
  ...
  const handleOpenEditForSubscription = async (sub: ProjectSubscription) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/payments/resolve-request?subscriptionId=${encodeURIComponent(sub.id)}`,
      );
      if (!res.ok) {
        setLoadingEditPayment(false);
        setShowEditPaymentDialog(false);
  ...
  const handleSaveEditPayment = async () => {
    ...
    try {
      ...
      if (editPaymentForm.recipientName !== undefined)
        body.recipientName = editPaymentForm.recipientName;
      if (editPaymentForm.recipientEmail !== undefined)
        body.recipientEmail = editPaymentForm.recipientEmail;
  ...
  const handleViewReceipt = async (
    transaction: ProjectFeeTransaction | ProjectSubscriptionTransaction,
  ) => {
  ...
  const handlePaySubscription = async (subscriptionId: string, projectId: string) => {
    try {
      ...
      const res = await fetch(
        `/api/projects/${projectId}/subscriptions/${subscriptionId}/create-payment`,
        {
          method: "POST",
        },
      );
  ...
  const handleMarkSubscriptionPaid = async (sub: ProjectSubscription, projectId: string) => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/subscriptions/${sub.id}/mark-completed`, {
        method: "POST",
      });
      const data = await res.json();
  ...
  const projectsByCompany = visibleCompanies.reduce(
    (acc, company) => {
  ...
  const companiesToShow = useMemo(() => {
    ...
    isSuperAdmin,
    authorizationsElevated,
    variant,
    visibleCompanies,
    runBillingCompanyId,
    authorizationsCompanyId,
  ]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[320px] gap-6 p-8">
        ...
        <div className="text-center space-y-1">
          <p className="font-medium text-[#111]">Loading payment data…</p>
  ...
  return (
    <div className="space-y-6">
      ...
      {isSuperAdmin && variant === "billing" && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-[#111]/55">
              Pick a company to view its fees and subscriptions below. Use dry run / run billing
              when you are ready to charge that company.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {dryRunResult && (
              <div className="rounded-lg border bg-black/[0.04] p-4 space-y-4 text-sm">
                ...
                {dryRunResult.subscriptionDebug && dryRunResult.subscriptionDebug.length > 0 && (
                  <div>
                    <p className="font-medium text-[#111]/55 mb-2">Subscriptions</p>
                ...
                {dryRunResult.paymentRequestDebug &&
                  dryRunResult.paymentRequestDebug.length > 0 && (
                    <div>
                      <p className="font-medium text-[#111]/55 mb-2">
                        Payment requests (would charge)
                      </p>
                      <ul className="list-disc pl-5 space-y-1">
                        {dryRunResult.paymentRequestDebug.map((pr) => (
                          <li key={pr.id}>
                            ID {pr.id.slice(0, 8)}… {pr.payment_type} ${(pr.amount || 0).toFixed(2)}{" "}
                            (due: {pr.next_billing_date ?? "—"}) – {pr.reason}
                          </li>
                        ))}
                      </ul>
                    </div>
                  )}
                {dryRunResult.dryRunDebug?.allPaymentRequestsBreakdown &&
                  dryRunResult.dryRunDebug.allPaymentRequestsBreakdown.length > 0 && (
                    <div>
                      <p className="font-medium text-[#111]/55 mb-2">
                        All payment requests (breakdown)
                      </p>
                      <div className="overflow-x-auto max-h-60 overflow-y-auto rounded border">
                        <table className="w-full text-xs">
                          <thead className="bg-black/[0.04] sticky top-0">
                            <tr>
                ...
                {dryRunResult.dryRunDebug?.skippedNoMethod &&
                  dryRunResult.dryRunDebug.skippedNoMethod.length > 0 && (
                    <div>
                      <p className="font-medium text-amber-600 mb-2">
                        Skipped (due but no payment method)
                      </p>
                      <ul className="list-disc pl-5 space-y-1">
                        {dryRunResult.dryRunDebug.skippedNoMethod.map((s) => (
                          <li key={s.id}>
                            ID {s.id.slice(0, 8)}… {s.payment_type} – {s.reason}
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                {(!dryRunResult.subscriptionDebug || dryRunResult.subscriptionDebug.length === 0) &&
                  (!dryRunResult.paymentRequestDebug ||
                    dryRunResult.paymentRequestDebug.length === 0) &&
                  (!dryRunResult.dryRunDebug?.allPaymentRequestsBreakdown ||
                    dryRunResult.dryRunDebug.allPaymentRequestsBreakdown.filter(
                      (r) => r.reason === "would_charge",
                    ).length === 0) && (
                    <p className="text-[#111]/55">
                      No items would be charged (all skipped or not due).
                    </p>
                  )}
              </div>
            )}
      ...
      {isSuperAdmin && variant === "billing" && !runBillingCompanyId && (
        <Card>
          <CardContent className="py-10 text-center text-sm text-[#111]/55">
            Select a company above to load fees, subscriptions, and payment status for that company
            only.
          </CardContent>
        </Card>
      )}
      ...
      {authorizationsFullUi && variant === "authorizations" && !authorizationsCompanyId && (
        <Card>
          <CardContent className="py-10 text-center text-sm text-[#111]/55">
            Select a company above to manage devices, access overrides, GitHub extension, and
            integration details.
          </CardContent>
        </Card>
      )}
      ...
      authorizationsLoading ? (
        <Card>
          <CardContent className="py-14 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-[#111]/55">Loading devices and extension data…</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={company.id}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  {variant === "billing" && status && (
                    <Badge
                      variant={status.allUpToDate ? "default" : "destructive"}
                      className="ml-auto"
                    >
                      {status.allUpToDate ? (
                        <>
              ...
              <CardContent>
                {variant === "billing" && status && !status.allUpToDate && (
                  <div className="mb-4 p-3 bg-black/[0.04] border border-black/[0.08] rounded-lg">
                    <p className="text-sm">
                      {status.pendingFees > 0 &&
                        `${status.pendingFees} pending fee${status.pendingFees !== 1 ? "s" : ""}`}
                      {status.pendingFees > 0 &&
                ...
                {companyProjects.length === 0 ? (
                  <p className="text-[#111]/55">No projects found</p>
                ) : (
                  <div className="space-y-6">
                    {companyProjects.map((project) => {
                      ...
                      const currentOverride =
                        accessOverrideByProject[project.id] !== undefined
                          ? accessOverrideByProject[project.id]
                          : (displayProject.accessOverride ?? null);
                      const resolvedDeviceLimit =
                        deviceLimitOverrideByProject[project.id] ??
                        displayProject.deviceLimit ??
                        null;
                      const authDevicesUsed = (authDevicesByProject[project.id] || []).filter(
                        (d) => (d.status === "active" || d.status === "paused") && !d.isAdminDevice,
                      ).length;
                      return (
                        <div key={project.id} className="border rounded-lg p-4 space-y-4">
                          ...
                          {authorizationsFullUi && variant === "authorizations" && (
                            <div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4 space-y-4">
                              <div>
                                ...
                                <p className="text-sm text-[#111]/55">
                                  Override entitlement for this project. Otherwise, typical payment
                                  rules apply.
                                </p>
                              </div>
                              ...
                              <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-primary/20">
                                <div className="flex items-center gap-2">
                                  <Smartphone className="w-4 h-4 text-[#111]/55 shrink-0" />
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setDeviceLimitInputByProject((prev) => ({
                                      ...prev,
                                    ...
                                    try {
                                      ...
                                      if (!empty) {
                                        ...
                                        if (!Number.isFinite(n) || n < 0 || !Number.isInteger(n)) {
                                          toast.error(
                                            "Enter a non-negative integer or leave blank for no limit",
                                          );
                                          return;
                                        }
                                        parsed = n;
                                      ...
                                      const res = await fetch(
                                        `/api/projects/${project.id}/device-limit`,
                                        {
                                          method: "PATCH",
                                          headers: { "Content-Type": "application/json" },
                                      ...
                                      if (res.ok) {
                                        setDeviceLimitOverrideByProject((prev) => ({
                                          ...prev,
                                          [project.id]: parsed,
                                        }));
                                        setAuthzRowByProject((prev) => {
                                          ...
                                          return {
                                            ...prev,
                                            [project.id]: { ...row, deviceLimit: parsed },
                                          };
                                        });
                                ...
                                <span className="text-xs text-[#111]/55">
                                  {authDevicesUsed} device{authDevicesUsed !== 1 ? "s" : ""} used
                                  (active + paused)
                                  {resolvedDeviceLimit != null &&
                                    ` / ${resolvedDeviceLimit} allowed`}
                                </span>
                              </div>
                              ...
                              <div className="flex flex-wrap gap-2 pt-1">
                                <Button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      const res = await fetch(
                                        `/api/projects/${project.id}/access-override`,
                                        {
                                          method: "PATCH",
                                          headers: { "Content-Type": "application/json" },
                                      ...
                                      if (res.ok) {
                                        setAccessOverrideByProject((prev) => ({
                                          ...prev,
                                          [project.id]: "allowed",
                                        }));
                                ...
                                >
                                  {loadingAccessOverrideProjectId === project.id &&
                                  currentOverride !== "allowed" ? (
                                ...
                                <Button
                                  size="sm"
                                  variant={
                                    currentOverride === "blocked" ? "destructive" : "outline"
                                  }
                                  disabled={loadingAccessOverrideProjectId === project.id}
                                  onClick={async () => {
                                    setLoadingAccessOverrideProjectId(project.id);
                              ...
                              {currentOverride !== null && (
                                <p className="text-xs text-[#111]/55">
                                  Current:{" "}
                                  {currentOverride === "allowed"
                                    ? "Always allow (overrides payments)"
                                    : "Blocked (overrides payments)"}
                                </p>
                              )}
                              ...
                              <div className="pt-3 border-t border-primary/20 space-y-3">
                                <div>
                                  ...
                                  <p className="text-sm text-[#111]/55">
                                    Baseline flags for this project. Devices can override these.
                                    Returned on auth/config as{" "}
                          ...
                          {variant === "billing" && (
                            <div>
                              ...
                              {!fees[project.id] || fees[project.id].length === 0 ? (
                                <p className="text-sm text-[#111]/55">No fees</p>
                              ) : (
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="flex-1">
                                          ...
                                          <div className="text-sm text-[#111]/55">
                                            ${fee.amount.toFixed(2)} - Created{" "}
                                            {new Date(fee.createdAt).toLocaleDateString()}
                                          </div>
                                          <div className="text-xs text-[#111]/55 mt-1">
                                            Payment Method:{" "}
                                            ...
                                            >
                                              <div className="p-2">
                                                <div className="font-semibold mb-2">
                                                  Setup Fee Transactions
                                                </div>
                                                {loadingTransactionsId === `fee-${fee.id}` ? (
                                                  <div className="flex items-center gap-2 py-4 text-sm text-[#111]/55">
                                                    <Loader2 className="h-4 w-4 animate-spin shrink-0" />
                                                    Loading transactions…
                                                  </div>
                                                ) : (feeTransactions[fee.id] || []).length === 0 ? (
                                                  <div className="text-sm text-[#111]/55 py-2">
                                                    No transactions yet
                                                  </div>
                                                ) : (
                                                  <div className="space-y-2">
                          ...
                          {variant === "billing" && (
                            <div>
                              ...
                              subscriptions[project.id].length === 0 ? (
                                <p className="text-sm text-[#111]/55">No subscriptions</p>
                              ) : (
                                <div className="space-y-2">
                                  {subscriptions[project.id].map((sub) => {
                                    ...
                                    const canProcessBilling =
                                      needsManualBilling(sub) && isDue && sub.status === "active";
                                      ...
                                      >
                                        <div className="flex-1">
                                          ...
                                          {sub.lastBilledDate && (
                                            <div className="text-xs text-[#111]/55">
                                              Last billed:{" "}
                                              {new Date(sub.lastBilledDate).toLocaleDateString()}
                                            </div>
                                          )}
                                        </div>
                                        <div className="flex gap-2">
                                          ...
                                          {canProcessBilling && (
                                            <Button
                                              ...
                                              variant="default"
                                              onClick={() =>
                                                handleProcessBilling(sub.id, project.id, sub)
                                              }
                                          ...
                                          {isSuperAdmin && sub.status === "active" && (
                                            <Button
                                              ...
                                              variant="outline"
                                              onClick={() =>
                                                handleMarkSubscriptionPaid(sub, project.id)
                                              }
                                          ...
                                          >
                                            ...
                                            >
                                              <div className="p-2">
                                                ...
                                                {loadingTransactionsId ===
                                                `subscription-${sub.id}` ? (
                                                  <div className="flex items-center gap-2 py-4 text-sm text-[#111]/55">
                                                    <Loader2 className="h-4 w-4 animate-spin shrink-0" />
                                                    Loading transactions…
                                                  </div>
                                                ) : (subscriptionTransactions[sub.id] || [])
                                                    .length === 0 ? (
                                                  <div className="text-sm text-[#111]/55 py-2">
                                                    No transactions yet
                                                  </div>
                                                  ...
                                                  <div className="space-y-2">
                                                    {(subscriptionTransactions[sub.id] || []).map(
                                                      (tx) => (
                                                        <div
                                                          key={tx.id}
                                                          className="flex items-center justify-between p-2 border rounded text-sm"
                                            ...
                                            <>
                                              {sub.status === "active" && (
                                                <Button
                                                  ...
                                                  variant="outline"
                                                  onClick={() =>
                                                    handleStopSubscription(sub.id, project.id)
                                                  }
                                              ...
                                              <Button
                                                ...
                                                variant="destructive"
                                                onClick={() =>
                                                  handleDeleteSubscription(sub.id, project.id)
                                                }
                          ...
                          {authorizationsFullUi && variant === "authorizations" && (
                            <div className="mt-4 pt-4 border-t border-black/[0.08]">
                              <div className="p-3 border rounded-md bg-black/[0.04] space-y-3">
                                <h4 className="font-medium text-sm flex flex-wrap items-center gap-x-2 gap-y-1">
                                  <GitBranch className="w-4 h-4 shrink-0" />
                                  <Key className="w-4 h-4 shrink-0 text-[#111]/55" />
                                  <span>Extension repo &amp; API integration</span>
                                </h4>
                                <p className="text-xs text-[#111]/55">
                                  Repo used by{" "}
                                  <code className="text-xs">/api/extension/project/config</code> and{" "}
                                  <code className="text-xs">
                                    /api/extension/project/scripts/:name
                                  </code>{" "}
                                  after entitlement + device checks.
                                </p>
                                {extensionSourceByProject[project.id] && (
                                  <p className="text-xs font-mono">
                                    Linked: {extensionSourceByProject[project.id]!.owner}/
                                    {extensionSourceByProject[project.id]!.repo} @{" "}
                                    {extensionSourceByProject[project.id]!.ref}
                                  </p>
                                )}
                                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                                  ...
                                  <div className="flex gap-2 shrink-0">
                                    <Button
                                      ...
                                      onClick={async () => {
                                        const url = (
                                          extensionUrlInputByProject[project.id] ?? ""
                                        ).trim();
                                        if (!url) {
                                        ...
                                        try {
                                          ...
                                          const statusRes = await fetch(
                                            `/api/projects/${project.id}/github-status`,
                                            {
                                              cache: "no-store",
                                            },
                                          );
                                    ...
                                    <Button
                                      ...
                                      onClick={async () => {
                                        if (!confirm("Remove linked GitHub repo for this project?"))
                                          return;
                                        setSavingExtensionSourceProjectId(project.id);
                                        try {
                                          const res = await fetch(
                                ...
                                {extensionSourceByProject[project.id] && (
                                  <div className="mt-3 pt-2 border-t border-black/[0.08]/40 space-y-2">
                                    <Label
                                      htmlFor={`deploy-from-${project.id}`}
                                      className="text-xs font-medium"
                                    >
                                      Deployment visible from (UTC)
                                    </Label>
                                    <p className="text-[11px] text-[#111]/55">
                                      Company admins only see commits on or after this calendar day
                                      (UTC). Leave empty to show every commit in the list.
                                    </p>
                                        ...
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            if (!res.ok) {
                                              const err = await res.json().catch(() => ({}));
                                              toast.error(
                                                err.error || "Failed to save deployment date",
                                              );
                                              return;
                                            }
                                            const data = await res.json();
                                            const saved = data.deploymentVisibleFrom as
                                              string | null;
                                            setExtensionSourceByProject((prev) => {
                                ...
                                {githubStatusByProject[project.id]?.hasGithubUrl && (
                                  <div className="mt-2 flex flex-wrap items-center gap-2">
                                    {githubStatusByProject[project.id]?.deploymentVisibleFrom ? (
                                      <p className="text-[11px] text-[#111]/55 w-full">
                                        <span className="text-amber-700 font-medium">Amber</span> in
                                        the list = commits before{" "}
                                        {githubStatusByProject[project.id]!.deploymentVisibleFrom}{" "}
                                        (UTC); you can still pin them.
                                      </p>
                                    ) : null}
                                    <span className="text-xs text-[#111]/55 whitespace-nowrap">
                                      Software version:
                                    </span>
                                    {githubStatusByProject[project.id]?.commits?.length ? (
                                      <>
                                        {(() => {
                                          ...
                                          const isDirty =
                                            Boolean(selectValue) && selectValue !== savedRef;
                                          ...
                                          return (
                                            <>
                                              <SoftwareVersionSelect
                                                ...
                                                commitRawOffset={
                                                  gh.commitRawOffset ?? commits.length
                                                }
                                ...
                                <div className="mt-4 pt-3 border-t border-black/[0.08]/50 space-y-3">
                                  <div>
                                    <p className="text-xs font-medium text-[#111]/55 mb-2">
                                      Project API key
                                    </p>
                                    <div className="flex flex-wrap gap-2 items-center">
                                      {projectApiKeyReveal[project.id] !== undefined ? (
                                        <>
                                          <code className="text-xs bg-[#F5F5F3] px-2 py-1 rounded border break-all max-w-md">
                                        ...
                                        <Button
                                          ...
                                          onClick={async () => {
                                            ...
                                            try {
                                              const res = await fetch(
                                                `/api/projects/${project.id}/api-key`,
                                              );
                                              if (!res.ok) {
                                              ...
                                              if (data.apiKey)
                                                toast.success(
                                                  "Key revealed. Use the copy button to copy.",
                                                );
                                            } finally {
                                      ...
                                      <Button
                                        ...
                                        onClick={async () => {
                                          if (
                                            !confirm(
                                              "Regenerate API key? The previous key will stop working.",
                                            )
                                          )
                                            return;
                                          setLoadingApiKeyProjectId(project.id);
...
// data.features — project + device overrides, e.g. { myFeature: true }`;
                          ...
                          {variant === "authorizations" && (
                            <div className="mt-6 rounded-lg border-2 border-black/[0.08] bg-black/[0.04] p-4 space-y-3">
                              <div>
                                ...
                                <p className="text-xs text-[#111]/55 mt-1">
                                  Approve pending requests, pause or resume, edit identifiers, or
                                  add a device from here.
                                </p>
                              </div>
                              <div className="space-y-2">
                                {(authDevicesByProject[project.id] || []).map((d) => (
                                  <div
                                    key={d.id}
                                    className={`p-2 border rounded text-sm ${d.status === "pending" ? "border-black/[0.08] bg-black/[0.04]" : ""}`}
                                  >
                                    {editingPaymentsAuthDeviceId === d.id ? (
                                      <div className="space-y-2">
                                        <div className="grid gap-2 sm:grid-cols-2">
                                          <div className="flex flex-col gap-1">
                                            <span className="text-xs text-[#111]/55">Name</span>
                                            <Input
                                              value={editPaymentsAuthName}
                                              onChange={(e) =>
                                                setEditPaymentsAuthName(e.target.value)
                                              }
                                              className="text-sm"
                                            />
                                          </div>
                                          <div className="flex flex-col gap-1">
                                            ...
                                            <Input
                                              ...
                                              onChange={(e) =>
                                                setEditPaymentsAuthExternalId(e.target.value)
                                              }
                                        ...
                                        {isSuperAdmin ? (
                                          <label className="flex items-center gap-2 text-sm cursor-pointer">
                                            <Checkbox
                                              ...
                                              onCheckedChange={(v) =>
                                                setEditPaymentsAuthIsAdmin(v === true)
                                              }
                                            />
                                            Admin device (hidden from company admins, not counted
                                            toward quota, bypasses block/payment rules; use API key
                                            suffix =dev for latest GitHub)
                                          </label>
                                        ) : null}
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                          <Button
                                            ...
                                            onClick={async () => {
                                              ...
                                              try {
                                                const res = await fetch(
                                                  `/api/projects/${project.id}/auth-devices/${d.id}`,
                                                  {
                                                    method: "PATCH",
                                                    headers: { "Content-Type": "application/json" },
                                                    body: JSON.stringify({
                                                      ...
                                                      features:
                                                        Object.keys(editPaymentsAuthFeatures)
                                                          .length > 0
                                                          ? editPaymentsAuthFeatures
                                                          : null,
                                                      ...(isSuperAdmin
                                                        ? { isAdminDevice: editPaymentsAuthIsAdmin }
                                                        : {}),
                                                    }),
                                                  },
                                                );
                                                ...
                                                if (updated) {
                                                  setAuthDevicesByProject((prev) => ({
                                                    ...prev,
                                                    [project.id]: (prev[project.id] || []).map(
                                                      (dev) =>
                                                        dev.id === d.id
                                                          ? { ...dev, ...updated }
                                                          : dev,
                                      ...
                                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                        <div className="flex-1 min-w-0 space-y-1">
                                          ...
                                          {d.deviceId ? (
                                            <p className="text-xs text-[#111]/55 font-mono break-all">
                                              {d.deviceId}
                                            </p>
                                          ) : null}
                                ...
                                <div className="flex flex-col gap-3">
                                  <div className="flex gap-2 items-center flex-wrap">
                                    <Input
                                      ...
                                      value={
                                        addingDeviceProjectId === project.id ? newDeviceName : ""
                                      }
                                      onChange={(e) => {
                                      ...
                                      onKeyDown={(e) => {
                                        if (e.key === "Enter") {
                                          e.preventDefault();
                                          document
                                            .getElementById(`add-device-${project.id}`)
                                            ?.click();
                                        }
                                      }}
                                      className="max-w-xs"
                                    ...
                                    <Button
                                      ...
                                      onClick={async () => {
                                        const name =
                                          (addingDeviceProjectId === project.id
                                            ? newDeviceName
                                            : ""
                                          ).trim() || "New device";
                                        const featuresForCreate =
                                          addingDeviceProjectId === project.id &&
                                          Object.keys(newDeviceFeatures).length > 0
                                            ? newDeviceFeatures
                                            : undefined;
                                        setLoadingAuthDeviceId(`add-${project.id}`);
                                        try {
                                          const res = await fetch(
                                            `/api/projects/${project.id}/auth-devices`,
                                            {
                                              method: "POST",
                                              headers: { "Content-Type": "application/json" },
                                              body: JSON.stringify({
                                          ...
                                          setAuthDevicesByProject((prev) => ({
                                            ...prev,
                                            [project.id]: [device, ...(prev[project.id] || [])],
                                          }));
                                          setNewDeviceName("");
                                  ...
                                  <div className="rounded-md border border-black/[0.08] bg-[#F5F5F3] p-3 space-y-2">
                                    <p className="text-xs font-medium text-[#111]/55">
                                  ...
                                  {isSuperAdmin ? (
                                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                                      <Checkbox
                                        checked={
                                          addingDeviceProjectId === project.id && newDeviceIsAdmin
                                        }
                                        onCheckedChange={(v) => {
                                          setAddingDeviceProjectId(project.id);
                                      ...
                                      />
                                      Admin device (hidden from company admins, not counted toward
                                      quota, bypasses block/payment rules; use API key suffix =dev
                                      for latest GitHub)
                                    </label>
                                  ) : null}
                                </div>
      ...
      <Dialog open={showFeeDialog} onOpenChange={setShowFeeDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create a one-time fee for the selected project</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: "daily" | "weekly" | "monthly") => {
                  setSubscriptionBillingInterval(value);
                  setSubscriptionBillingDayOfMonth("");
                  setSubscriptionBillingDayOfWeek("");
                }}
              >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setSubscriptionBillingDayOfMonth(
                      v === "" ? "" : Math.min(31, Math.max(1, parseInt(v, 10) || 1)),
                    );
                  }}
            ...
            {subscriptionBillingInterval === "weekly" && (
              <div className="space-y-2">
                ...
                <Select
                  value={
                    subscriptionBillingDayOfWeek === ""
                      ? "any"
                      : String(subscriptionBillingDayOfWeek)
                  }
                  onValueChange={(v) =>
                    setSubscriptionBillingDayOfWeek(v === "any" ? "" : Number(v))
                  }
                >
                  <SelectTrigger>
      ...
      <Dialog open={showPaymentModal} onOpenChange={handlePaymentModalClose}>
        <DialogContent className="max-w-4xl max-h-[90vh] p-0">
          <DialogHeader className="px-6 pt-6">
            ...
            <DialogDescription>Complete your payment in the form below</DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {loadingTransactionsDialog ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <p className="font-medium text-[#111]">Loading billing history…</p>
              <p className="text-sm text-[#111]/55">Please wait…</p>
            </div>
          ) : (
            transactionId && (
              <div className="space-y-2">
                {(transactionType === "fee"
                  ? feeTransactions[transactionId]
                  : subscriptionTransactions[transactionId]
                )?.length === 0 ? (
                  <p className="text-[#111]/55">No transactions yet</p>
                ) : (
                  <div className="space-y-2">
                    {(transactionType === "fee"
                      ? feeTransactions[transactionId]
      ...
      <Dialog open={showReceiptDialog} onOpenChange={setShowReceiptDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Receipt details for this payment</DialogDescription>
          </DialogHeader>
          {loadingReceipt ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              <Loader2 className="h-12 w-12 animate-spin text-primary" />
              <p className="font-medium text-[#111]">Loading receipt…</p>
              <p className="text-sm text-[#111]/55">Please wait…</p>
            </div>
          ) : receiptData ? (
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-[#111]/55">Invoice Number</Label>
                  <p className="font-semibold">{receiptData.invoiceNumber || "N/A"}</p>
                </div>
                <div>
                  <Label className="text-[#111]/55">Date</Label>
                  <p>
              ...
              <div>
                <Label className="text-[#111]/55">Bill To</Label>
                <p className="font-medium">{receiptData.recipientName}</p>
                <p className="text-sm text-[#111]/55">{receiptData.recipientEmail}</p>
              </div>

              <div className="border-t pt-4">
                <Label className="text-[#111]/55">Payment Details</Label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-[#111]/55">Payment Method</Label>
                  <p className="font-medium">
                    {paymentRailDisplayLabel(receiptData.paymentMethod)}
                  </p>
                </div>
                <div>
                  <Label className="text-[#111]/55">Payment Type</Label>
            ...
            <div className="py-8 text-center text-[#111]/55">No receipt data available</div>
          )}
          <DialogFooter>
            <Button
              variant="outline"
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Change amount, due date, or recipient. Super admin only.
            </DialogDescription>
          </DialogHeader>
          {loadingEditPayment ? (
            <div className="py-12 flex flex-col items-center justify-center gap-4">
              <Loader2 className="h-12 w-12 animate-spin text-primary" />
              <p className="text-sm font-medium text-[#111]">{loadingEditMessage}</p>
              <p className="text-xs text-[#111]/55">Please wait…</p>
            </div>
          ) : (
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditPaymentForm((f) => ({ ...f, recipientName: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditPaymentForm((f) => ({ ...f, recipientEmail: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditPaymentForm((f) => ({ ...f, nextBillingDate: e.target.value }))
                  }
                />
                <p className="text-xs text-[#111]/55">Leave empty for immediate / next billing.</p>
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button
              ...
              disabled={savingEditPayment || loadingEditPayment}
            >
              {savingEditPayment ? (
                <>
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
