...
export default function PaymentsLicensing({
  ...
  const getStatusBadge = (status: string, item: ProjectFee | ProjectSubscription) => {
    ...
    if ('nextBillingDate' in item) {
      ...
      return <Badge variant="default" className="bg-primary text-white"><CheckCircle className="w-3 h-3 mr-1" />Active</Badge>;
    }
    
    // For fees, use the status directly
    switch (status) {
      case 'completed':
        return <Badge variant="default" className="bg-primary text-white"><CheckCircle className="w-3 h-3 mr-1" />Paid</Badge>;
      case 'pending':
            ...
            {dryRunResult && (
              <div className="rounded-lg border bg-black/[0.04] p-4 space-y-4 text-sm">
                ...
                {dryRunResult.paymentRequestDebug && dryRunResult.paymentRequestDebug.length > 0 && (
                  <div>
                    <p className="font-medium text-[#111]/55 mb-2">Payment requests (would charge)</p>
                    <ul className="list-disc pl-5 space-y-1">
                ...
                {dryRunResult.dryRunDebug?.allPaymentRequestsBreakdown && dryRunResult.dryRunDebug.allPaymentRequestsBreakdown.length > 0 && (
                  <div>
                    <p className="font-medium text-[#111]/55 mb-2">All payment requests (breakdown)</p>
                ...
                {dryRunResult.dryRunDebug?.skippedNoMethod && dryRunResult.dryRunDebug.skippedNoMethod.length > 0 && (
                  <div>
                    <p className="font-medium text-amber-600 mb-2">Skipped (due but no payment method)</p>
                    <ul className="list-disc pl-5 space-y-1">
                ...
                {(!dryRunResult.subscriptionDebug || dryRunResult.subscriptionDebug.length === 0) &&
                 ...
                 (!dryRunResult.dryRunDebug?.allPaymentRequestsBreakdown || dryRunResult.dryRunDebug.allPaymentRequestsBreakdown.filter((r) => r.reason === 'would_charge').length === 0) && (
                  <p className="text-[#111]/55">No items would be charged (all skipped or not due).</p>
                )}
              </div>
            )}
          </CardContent>
                        ...
                        <div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4 space-y-4">
                          ...
                          <div className="flex flex-wrap gap-2 pt-1">
                            <Button
                              ...
                              className={currentOverride === 'allowed' ? 'bg-primary hover:bg-primary/90 text-white' : ''}
                                      ...
                                      <DropdownMenuContent align="end" className="w-80 max-h-96 overflow-y-auto">
                                        <div className="p-2">
                                          ...
                                          {loadingTransactionsId === `fee-${fee.id}` ? (
                                            ...
                                            <div className="text-sm text-[#111]/55 py-2">No transactions yet</div>
                                          ) : (
                                            <div className="space-y-2">
                        ...
                        <div className="mt-4 pt-4 border-t border-black/[0.08]">
                          <div className="p-3 border rounded-md bg-black/[0.04] space-y-3">
                              ...
                              <div className="mt-2 flex flex-wrap items-center gap-2">
                                {githubStatusByProject[project.id]?.deploymentVisibleFrom ? (
                                  <p className="text-[11px] text-[#111]/55 w-full">
                                    <span className="text-amber-700 font-medium">Amber</span> in the
                                ...
                                {githubStatusByProject[project.id]?.commits?.length ? (
                                  ...
                                  <span className="text-xs text-[#111]/55">Loading versions…</span>
                                )}
                              </div>
                            )}
                            ...
                            <div className="mt-4 pt-3 border-t border-black/[0.08]/50 space-y-3">
                              <div>
                                <p className="text-xs font-medium text-[#111]/55 mb-2">Project API key</p>
...
// data.features — project + device overrides, e.g. { myFeature: true }`;
                                ...
                                return (
                                  <div>
                                    <p className="text-xs font-medium text-[#111]/55 mb-2">Copy integration snippets</p>
                                    <div className="flex flex-wrap gap-2">
                                      <Button
                                        size="sm"
                                        variant="outline"
                      ...
                      <div className="mt-6 rounded-lg border-2 border-black/[0.08] bg-black/[0.04] p-4 space-y-3">
                        ...
                        <div className="space-y-2">
                              ...
                              {editingPaymentsAuthDeviceId === d.id ? (
                                <div className="space-y-2">
                                  <div className="grid gap-2 sm:grid-cols-2">
                                    ...
                                    <div className="flex flex-col gap-1">
                                      <span className="text-xs text-[#111]/55">Device ID</span>
                                      <Input
                                  ...
                                  <div className="space-y-1">
                                    <span className="text-xs text-[#111]/55">Device features</span>
                                    <p className="text-xs text-[#111]/55">
                                  ...
                                  <div className="flex-1 min-w-0 space-y-1">
                                    <div className="flex flex-wrap items-center gap-2">
                                      ...
                                      <Badge
                                        ...
                                        className={d.status === 'pending' ? 'border-[#111]/55 text-[#111]/55' : ''}
                                      >
                                        {d.status}
                                      </Badge>
                                      {d.isAdminDevice ? (
                                    ...
                                    {d.deviceId ? (
                                      <p className="text-xs text-[#111]/55 font-mono break-all">{d.deviceId}</p>
                                    ) : null}
                          ...
                          <div className="rounded-md border border-black/[0.08] bg-[#F5F5F3] p-3 space-y-2">
                            <p className="text-xs font-medium text-[#111]/55">Device features (optional)</p>
                            <AppFeaturesEditor
                              dense
