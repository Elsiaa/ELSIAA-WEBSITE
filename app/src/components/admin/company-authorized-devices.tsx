...
export default function CompanyAuthorizedDevices({ currentUser }: CompanyAuthorizedDevicesProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const projectsRes = await fetch(`/api/companies/${companyId}/projects`, { cache: 'no-store' });
  ...
  const countQuotaDevices = (devices: Device[]) =>
    devices.filter((d) => !d.isAdminDevice && (d.status === 'active' || d.status === 'paused')).length;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16 text-[#111]/55">
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <p className="text-sm text-[#111]/55">
            Manage authorized devices for your projects. Device quotas are set per project. Add devices or approve
      ...
      {projects.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-[#111]/55">
            ...
            <Card key={project.id}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex flex-col">
                    ...
                    {githubStatusByProject[project.id]?.hasGithubUrl && (
                      <div className="mt-2 flex flex-col gap-1">
                        ...
                        <span className="text-xs text-[#111]/55 whitespace-nowrap">Software Version:</span>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-medium text-[#111]/55">
                      Quota (active + paused):{' '}
                      <span className="font-semibold text-foreground">
                        {usedQuota}
                        {lim != null ? ` / ${lim}` : ''}
                      </span>
                      {lim == null && <span className="text-[#111]/55 font-normal"> (no limit)</span>}
                    </span>
                    ...
                    <div className="w-full bg-secondary/60 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full transition-all ${
                          atProjectLimit ? 'bg-destructive' : usedQuota / lim > 0.8 ? 'bg-slate' : 'bg-primary'
                        }`}
                ...
                <div className="mt-3 pt-3 border-t border-black/[0.08]/50">
                  <p className="text-xs font-medium text-[#111]/55 flex items-center gap-1.5 mb-1.5">
                    <Key className="w-3.5 h-3.5" />
                    Project API Key
                  </p>
                  <div className="flex items-center gap-2 flex-wrap">
                    {projectApiKeyReveal[project.id] === undefined ? (
                      <code className="text-xs bg-secondary/80 px-2 py-1 rounded font-mono text-[#111]/55">
                        ••••••••••••••••
                      </code>
                    ) : (
              ...
              <CardContent>
                {devices.length === 0 ? (
                  <p className="text-sm text-[#111]/55 mb-3">No devices registered yet.</p>
                ) : (
                    ...
                    {devices.map((d) => (
                      <div
                        ...
                        className={`p-3 border rounded-lg text-sm ${
                          d.status === 'pending'
                            ? 'border-black/[0.08] bg-black/[0.04]/50'
                            : d.status === 'paused'
                            ? 'border-muted bg-black/[0.04]/20'
                            : ''
                        }`}
                      >
                          ...
                          <div className="space-y-3">
                            <div className="grid gap-2 sm:grid-cols-2">
                              <div className="flex flex-col gap-1">
                                <label className="text-xs text-[#111]/55">Name</label>
                                <Input
                                  value={editAuthName}
                              ...
                              <div className="flex flex-col gap-1">
                                <label className="text-xs text-[#111]/55">Device ID</label>
                                <Input
                            ...
                            <p className="text-xs text-[#111]/55">
                              Clearing device ID and saving assigns a new random ID (required by the database).
                            </p>
                            <div className="flex flex-wrap gap-2">
                              ...
                              <Button size="sm" variant="outline" onClick={cancelEditDevice} type="button">
                          ...
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex-1 min-w-0 flex flex-wrap items-center gap-2">
                              ...
                              <Badge
                                ...
                                className={d.status === 'pending' ? 'border-muted-foreground text-[#111]/55' : ''}
                              >
                                {d.status}
                              </Badge>
                              {d.deviceId ? (
                                <span className="text-xs text-[#111]/55 font-mono break-all">{d.deviceId}</span>
                              ) : null}
                            </div>
                ...
                <div className="flex flex-wrap gap-2 items-end">
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-[#111]/55">Name (optional)</label>
                    <Input
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-[#111]/55">Device ID (optional)</label>
                    <Input
                  ...
                  <Button
                    ...
                    title={atProjectLimit ? 'Device limit reached for this project' : 'Add device (name and/or ID required)'}
                  >
                ...
                <p className="text-xs text-[#111]/55 mt-1.5">
                  Provide at least a name or device ID.
                </p>
                {atProjectLimit && (
                  <p className="text-xs text-red-500 mt-1">
                    Device limit reached for this project. Contact your administrator to increase the limit.
                  </p>
                )}
              </CardContent>
            </Card>
