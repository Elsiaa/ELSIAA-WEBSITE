...
export default function AdminCalendarClient({
  ...
  const [loading, setLoading] = useState(
    () =>
      initialMeetingRequests.length === 0 &&
  ...
  useEffect(() => {
    let cancelled = false;
    void import("@/lib/calendar-bootstrap.functions")
      .then(({ bootstrapAdminCalendar }) => bootstrapAdminCalendar())
      .then((data) => {
        if (cancelled) return;
  ...
  const getSlotStatus = (slot: string) => {
    ...
    const meeting = (meetings ?? []).find((m) => {
      ...
      const durationMin = typeof m.duration === "number" ? m.duration : 30;
      const meetingEnd = new Date(meetingStart.getTime() + durationMin * 60 * 1000);
  ...
  const getSlotColor = (status: string | { type: string; meeting: Meeting }) => {
    ...
    switch (statusStr) {
      ...
      case "past":
        return "bg-black/[0.04] border-transparent text-[#111]/40";
      case "available":
        return "bg-white border-black/15 text-[#111] hover:bg-black/[0.04]";
      default:
        return "bg-white border-black/15 text-[#111] hover:bg-black/[0.04]";
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-4xl font-bold mb-2 text-[#111]">Calendar Management</h1>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex flex-wrap gap-4 text-xs">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-white border border-black/15 rounded"></div>
              <span>Available</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-[#1e6b3c]/25 border border-[#1e6b3c] rounded"></div>
                  ...
                  <div
                    ...
                    className={`p-3 border rounded-lg cursor-pointer transition-all ${
                      selectedRequest?.id === request.id
                        ? "border-[#1e6b3c] bg-[#1e6b3c]/10"
                        : "border-black/10 hover:border-[#1e6b3c]/50"
                    }`}
                    onClick={() => setSelectedRequest(request)}
                  >
                    <div className="font-medium text-sm text-[#111]">{request.name}</div>
                    <div className="text-xs text-[#111]/55">{request.email}</div>
                    {request.company && (
                      <div className="text-xs text-[#111]/55">{request.company}</div>
                    )}
