...
export default function SupportAgentsManagement({ companies, onDataChange }: Props) {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="rounded-xl border border-border/70 bg-card/40 p-6 space-y-4">
        <h3 className="text-lg font-medium flex items-center gap-2">
          <UserPlus className="h-5 w-5 text-flame" />
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-flame px-4 py-2 text-sm font-medium text-white hover:bg-coral disabled:opacity-50"
        >
      ...
      <div className="space-y-6">
        ...
        {agents.length === 0 ? (
          <p className="text-sm text-muted-foreground">No support agents yet.</p>
        ) : (
          <div className="space-y-8">
            {agents.map((a) => (
              <div key={a.user.id} className="rounded-xl border border-border/70 p-5 space-y-3">
