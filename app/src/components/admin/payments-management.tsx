...
import { useState, useEffect, useMemo, type ClipboardEvent, type Dispatch, type SetStateAction } from 'react';
...
import { DollarSign, Check, CheckCircle, Copy, Trash2, Mail, CreditCard, FileText, Send, Pencil, Plus, Minus, Loader2 } from 'lucide-react';
...
export default function PaymentsManagementNew({ 
  ...
  const [editForm, setEditForm] = useState({ amount: '', nextBillingDate: '', recipientName: '', recipientEmail: '' });
  ...
  const loadProjectTransactions = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log('[PAYMENTS-MANAGEMENT] Transaction IDs:', transactions.map((tx: any) => ({
          id: tx.id,
          type: tx.type,
        ...
        console.log('[PAYMENTS-MANAGEMENT] Set projectTransactions state with:', transactions.length, 'transactions');
      }
    } catch (err) {
      ...
      if (res.ok) {
        ...
        if (sendRes.ok) {
          ...
          setPaymentRequests((prev) =>
            prev.map((r) => (r.id === request.id && r.status === 'pending' ? { ...r, status: 'invoiced' as const } : r))
          );
          toast.success(
            inv
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const body: Record<string, unknown> = {};
      if (editForm.recipientName !== editingRequest.recipient_name) body.recipientName = editForm.recipientName;
      if (editForm.recipientEmail !== editingRequest.recipient_email) body.recipientEmail = editForm.recipientEmail;
      ...
      const currentDue = editingRequest.next_billing_date ? editingRequest.next_billing_date.slice(0, 10) : '';
      if (currentDue !== editForm.nextBillingDate) body.nextBillingDate = editForm.nextBillingDate || null;
    ...
    <div className="space-y-6">
      ...
      {isSuperAdmin && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-[#111]/55 font-normal">
              Add line items with unit prices; we total them for Stripe and for the PDF when you email the invoice.
              Save a draft to keep working on line items (Edit) before sending. For recurring charges, use the Subscriptions tab.
            </p>
          </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2 rounded-md border border-dashed border-black/[0.08] bg-black/[0.04]/20 p-3">
              <Label htmlFor="create-line-paste" className="text-xs font-medium text-[#111]/55">
            ...
            <div className="border rounded-md divide-y">
              <div className="grid grid-cols-12 gap-2 px-3 py-2 text-xs font-medium text-[#111]/55 bg-black/[0.04]/40">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setInvoiceLines((lines) =>
                          lines.map((l) => (l.id === row.id ? { ...l, description: e.target.value } : l))
                        )
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setInvoiceLines((lines) =>
                          lines.map((l) => (l.id === row.id ? { ...l, quantity: e.target.value } : l))
                        )
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setInvoiceLines((lines) =>
                          lines.map((l) => (l.id === row.id ? { ...l, unitPrice: e.target.value } : l))
                        )
                      }
                    />
            ...
            <div className="flex justify-end text-sm">
              <span className="text-[#111]/55 mr-2">Invoice total</span>
          ...
          <div className="space-y-1">
            ...
            />
            <p className="text-xs text-[#111]/55">Leave empty to make the charge due immediately.</p>
          </div>
        ...
        <CardContent>
          {loading ? (
            ...
            >
              ...
              <TabsContent value="pending" className="mt-4">
                {paymentRequests.filter(r => {
                  ...
                  <p className="text-[#111]/55">No pending payments.</p>
          ) : (
            <div className="space-y-3">
                ...
                return (
                  <Accordion key={req.id} type="single" collapsible className="border rounded-lg">
                          ...
                          <div className="flex-1 text-left">
                            ...
                            <div className="text-sm text-[#111]/55">
                              ...
                              {req.payment_type === 'one_time' && req.next_billing_date && (
                                <span className="ml-2 text-[#111]/55">Due: {new Date(req.next_billing_date).toLocaleDateString()}</span>
                              )}
                            </div>
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {req.invoice_number && (
                                <Badge variant="secondary">Invoice #{req.invoice_number}</Badge>
                              )}
                            </div>
                          </div>
                        </AccordionTrigger>
                      ...
                      <AccordionContent className="px-3 pb-3">
                        <div className="space-y-3 pt-2 border-t">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              <span className="text-[#111]/55">Recipient:</span>
                              <p className="font-medium">{displayName}</p>
                              <p className="text-[#111]/55">{displayEmail}</p>
                            </div>
                            <div>
                              <span className="text-[#111]/55">Amount:</span>
                              <p className="font-medium">{(req.payment_type === 'interval_billing' || req.payment_type === 'monthly') ? 'Amount set when billing' : `$${req.amount.toFixed(2)}`}</p>
                            </div>
                            {req.invoice_number && (
                              <div>
                                <span className="text-[#111]/55">Invoice #:</span>
                                <p className="font-medium">{req.invoice_number}</p>
                              </div>
                            )}
                            <div>
                              <span className="text-[#111]/55">Created:</span>
              ...
              <TabsContent value="intervals" className="mt-4">
                <p className="text-sm text-[#111]/55 mb-4">
                  New recurring or interval billing should be set up under Subscriptions. This list is for existing linked payment requests that already have a saved card or {PAYMENT_METHOD_PHRASE_ACH} method.
                </p>
                {paymentRequests.filter(r => (r.payment_type === 'interval_billing' || r.payment_type === 'monthly') && r.stripe_payment_method_id).length === 0 ? (
                  <p className="text-[#111]/55">No payments with saved payment methods available for billing.</p>
                ) : (
                  <div className="space-y-3">
                    {paymentRequests
              ...
              <TabsContent value="history" className="mt-4">
                {(() => {
                  ...
                  if (allHistoryEntries.length === 0) {
                    return <p className="text-[#111]/55">No completed payments yet.</p>;
                  }
                  ...
                  return (
                    <div className="space-y-3">
                      {allHistoryEntries.map((entry, idx) => {
                        ...
                        return (
                          <Accordion key={`${req.id}-${billing?.paymentIntentId || idx}`} type="single" collapsible className="border rounded-lg">
                            <AccordionItem value={`${req.id}-${idx}`} className="border-none">
                                ...
                                <AccordionTrigger className="flex-1 hover:no-underline py-0">
                                  <div className="flex-1 text-left">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                      ...
                                      {billing && (
                                        <Badge variant="secondary" className="text-xs">
                                          {billing.paymentMethod === 'ach' ? PAYMENT_METHOD_LABEL_ACH : 'Card'}
                                        </Badge>
                                      )}
                                    </div>
                                  </div>
                              ...
                              <AccordionContent className="px-3 pb-3">
                                {req.invoice_line_items && req.invoice_line_items.length > 0 ? (
                                  <InvoiceLineItemsTable items={req.invoice_line_items} className="mt-2" />
                                ) : (
                                  <p className="text-sm text-[#111]/55 pt-2 border-t">No line-item breakdown stored for this invoice.</p>
                                )}
                              </AccordionContent>
                            </AccordionItem>
                          </Accordion>
              ...
              <TabsContent value="project-transactions" className="mt-4">
                {loadingProjectTransactions ? (
                  <p className="text-[#111]/55">Loading project transactions...</p>
                ) : projectTransactions.length === 0 ? (
                  <div className="space-y-2">
                    <p className="text-[#111]/55">Project payment history is not loaded yet.</p>
                    <Button type="button" variant="secondary" size="sm" onClick={() => loadProjectTransactions()}>
                  ...
                  <div className="space-y-3">
                    {(() => {
                      console.log('[PAYMENTS-MANAGEMENT] Rendering transactions. Total in state:', projectTransactions.length);
                      console.log('[PAYMENTS-MANAGEMENT] Transactions to render:', projectTransactions.map((tx: any) => ({
                        id: tx.id,
                        type: tx.type,
                        name: tx.type === 'fee' ? tx.feeName : tx.subscriptionName,
                        amount: tx.amount,
                      ...
                      <div key={tx.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex-1">
                          <div className="font-medium">
                            {tx.type === 'fee' ? (tx.feeName || 'Unknown Fee') : (tx.subscriptionName || 'Unknown Subscription')}
                          </div>
                          <div className="text-sm text-[#111]/55">
                            {tx.companyName} - {tx.projectTitle} - ${tx.amount.toFixed(2)} - {new Date(tx.transactionDate).toLocaleDateString()}
                            {tx.invoiceNumber && (
                              <span className="ml-2 font-semibold text-foreground">Invoice #: {tx.invoiceNumber}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {tx.billingPeriodStart && tx.billingPeriodEnd && (
                              <Badge variant="outline" className="text-xs">
                                {new Date(tx.billingPeriodStart).toLocaleDateString()} - {new Date(tx.billingPeriodEnd).toLocaleDateString()}
                              </Badge>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {tx.paymentRequestId && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleViewReceipt(tx.paymentRequestId, tx.stripePaymentIntentId)}
                              title="View Receipt"
                            >
                              <FileText className="w-4 h-4" />
                            </Button>
      ...
      <Dialog open={showBillingDialog} onOpenChange={setShowBillingDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the amount to charge the saved payment method. This will automatically charge the customer’s card or {PAYMENT_METHOD_PHRASE_ACH} account.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showReceiptDialog} onOpenChange={setShowReceiptDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{receiptDialogMode === 'invoice' ? 'Invoice' : 'Payment Receipt'}</DialogTitle>
            <DialogDescription>
          ...
          {loadingReceipt ? (
            ...
            <div className="space-y-4 py-4">
              ...
              {receiptData.lineItems && receiptData.lineItems.length > 0 && (
                <div>
                  <Label className="text-[#111]/55 mb-2 block">Line Items</Label>
                  <InvoiceLineItemsTable items={receiptData.lineItems} />
                </div>
              )}

              <div className="border-t pt-4">
                <Label className="text-[#111]/55">
                  {receiptDialogMode === 'invoice' ? 'Invoice Total' : 'Payment Details'}
                </Label>
                <div className="mt-2 space-y-2">
                  <div className="flex justify-between">
                    <span>{receiptDialogMode === 'invoice' ? 'Amount due:' : 'Service Amount:'}</span>
              ...
              {receiptDialogMode === 'invoice' && receiptData.statusLabel && (
                <div>
                  <Label className="text-[#111]/55">Status</Label>
                  <p className="font-medium">{receiptData.statusLabel}</p>
                </div>
              )}
            </div>
          ) : (
            <div className="py-8 text-center text-[#111]/55">No data available</div>
          )}
          <DialogFooter>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Update recipient, due date, and for one-time requests the line items (total updates amount for Stripe).
            </DialogDescription>
          </DialogHeader>
          {editingRequest && (
            <div className="space-y-4 py-4">
              ...
              {editingRequest.payment_type === 'one_time' ? (
                <div className="space-y-2">
                  ...
                  <div className="space-y-2 rounded-md border border-dashed border-black/[0.08] bg-black/[0.04]/20 p-3">
                    <Label htmlFor="edit-line-paste" className="text-xs font-medium text-[#111]/55">
                      Paste from billing (description;; qty;; unit price;; …)
                    </Label>
                    <Textarea
                      id="edit-line-paste"
                      ...
                      className="min-h-[4.5rem] font-mono text-xs resize-y"
                    />
                    <Button type="button" variant="secondary" size="sm" onClick={applyEditLinePaste}>
                      Load lines from paste
                    </Button>
                  </div>
                  <div className="border rounded-md divide-y">
                  ...
                  <div className="flex justify-end text-sm">
                    <span className="text-[#111]/55 mr-2">Invoice total</span>
                    <span className="font-semibold tabular-nums">${editInvoiceTotal.toFixed(2)}</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowEditDialog(false); setEditingRequest(null); }}>
              Cancel
            </Button>
            <Button
