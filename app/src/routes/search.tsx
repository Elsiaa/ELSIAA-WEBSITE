...
function highlight(text: string, terms: string[]) {
  ...
  return parts.map((p, i) =>
    terms.some((t) => p.toLowerCase() === t.toLowerCase()) ? (
      <mark key={i} className="rounded bg-[#1e6b3c]/12 px-0.5 text-[#1e6b3c]">
        {p}
      </mark>
    ) : (
      <span key={i}>{p}</span>
...
function Chip({
  ...
  if (href)
    return (
      <a href={href} className={cls} style={{ fontFamily: SANS }}>
        {label}
      </a>
    );
  return (
    <button onClick={onClick} className={cls} style={{ fontFamily: SANS }}>
      {label}
    </button>
  );
}
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd
      className="inline-flex h-5 min-w-5 items-center justify-center rounded-[5px] border border-black/[0.1] bg-white px-1.5 text-[11px] font-medium text-[#111111]/55"
...
function SearchPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-[#111111]">
      ...
      >
        <div
          className="absolute left-1/2 top-[-30%] h-[420px] w-[820px] -translate-x-1/2 rounded-full blur-[130px]"
          style={{ background: "radial-gradient(circle, rgba(30,107,60,0.07), transparent 66%)" }}
        />
      </div>

      <section className="relative mx-auto max-w-3xl px-6 pt-36 pb-14 md:pt-44 md:pb-28">
        <p className="text-[13px] tracking-[0.06em] text-[#1e6b3c]" style={{ fontFamily: MONO }}>
          Search
        </p>
        <h1
          className="mt-3 text-3xl font-semibold tracking-[-0.04em] md:text-[3.25rem] md:leading-[1.02]"
        ...
        <p
          ...
          style={{ fontFamily: SANS }}
        >
          A deep search across every system, case study, process, service, and location — the same
          precision we bring to the software we build.
        </p>
        ...
        <div
          className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-[12.5px] text-[#111111]/45"
          style={{ fontFamily: MONO }}
        >
        ...
        {!hasQuery && (
          <div className="mt-9 space-y-10">
            <div>
              <p className="text-[13px] text-[#111111]/55" style={{ fontFamily: MONO }}>
                Suggested deep paths
              </p>
              ...
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {AREAS.map((a) => (
                  <a
                    key={a.label}
                    href={a.href}
                    className="group flex items-center justify-between rounded-2xl border border-black/[0.08] bg-white px-5 py-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#1e6b3c]/40 hover:shadow-[0_24px_60px_-48px_rgba(17,17,17,0.4)]"
                  >
                    <span>
                      <span
                        className="block text-[15px] font-semibold tracking-[-0.01em] text-[#111111]"
                        style={{ fontFamily: SANS }}
                      >
                        {a.label}
                      </span>
                      <span
                        className="mt-0.5 block text-[13px] text-[#111111]/50"
                        style={{ fontFamily: SANS }}
                      >
                        {a.note}
                      </span>
                    ...
                    <span
                      className="text-[#111111]/30 transition-all group-hover:translate-x-0.5 group-hover:text-[#1e6b3c]"
                      aria-hidden
                    >
                      →
                    </span>
        ...
        {hasQuery && (
          <div
            className="mt-8"
            style={{ opacity: typing ? 0.55 : 1, transition: "opacity 0.2s ease" }}
          >
            {results.length === 0 ? (
              <div className="rounded-2xl border border-black/[0.08] bg-white p-8 text-center">
                ...
                <p
                  className="mx-auto mt-2 max-w-md text-[14px] leading-relaxed text-[#111111]/55"
                  style={{ fontFamily: SANS }}
                >
                  Tell us what you're trying to build and we'll point you to the right system — or
                  scope it from scratch.
                </p>
                ...
                <a
                  href="/contact"
                  className="mt-6 inline-block rounded-full bg-[#1e6b3c] px-6 py-3 text-[13px] font-bold text-white transition-all hover:bg-[#111111]"
                  style={{ fontFamily: MONO }}
                >
                  ...
                  {results.map((item, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-4">
                        <span
                          className="inline-flex items-center rounded-full bg-[#1e6b3c]/[0.08] px-2.5 py-0.5 text-[11.5px] font-medium tracking-[0.02em] text-[#1e6b3c]"
                          style={{ fontFamily: MONO }}
                        >
                          {item.group}
                        </span>
                        <span
                          className="shrink-0 text-[12px] tracking-[0.1em] text-[#111111]/30"
                          style={{ fontFamily: MONO }}
                        >
                          {item.href}
                        </span>
                      ...
                      <div className="mt-2 flex items-center justify-between gap-4">
                        <span
                          className="text-[17px] font-semibold tracking-[-0.015em] text-[#111111]"
                          style={{ fontFamily: SANS }}
                        >
                          {highlight(item.label, terms)}
                        </span>
                        <span
                          className={`shrink-0 transition-all ${active === i ? "translate-x-0.5 text-[#1e6b3c]" : "text-[#111111]/30 group-hover:translate-x-0.5 group-hover:text-[#1e6b3c]"}`}
                          aria-hidden
                        >
                          →
                        </span>
