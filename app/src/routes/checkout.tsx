...
function CheckoutBody() {
  ...
  const valid =
    ...
    f.country.trim() &&
    f.postal.trim() &&
    f.card.replace(/\s/g, "").length >= 12 &&
    f.exp.trim() &&
    f.cvc.trim().length >= 3;

  const pay = () => {
    if (!valid || paying) {
      setErr("Fill in the required fields to place your order.");
      return;
    }
    setErr("");
    setPaying(true);
    const order = {
      ...
      lines: lines.map((l) => ({
        ...l,
        name: bySlug(l.slug)?.name ?? l.slug,
        price: bySlug(l.slug)?.price ?? 0,
      })),
      placed: new Date().toISOString(),
    };
    setTimeout(() => {
      try {
        sessionStorage.setItem("elsiaa-last-order", JSON.stringify(order));
      } catch {
        /* ignore */
      }
      clear();
  ...
  return (
    <section
      className="mx-auto grid max-w-6xl grid-cols-1 gap-10 px-6 py-8 md:grid-cols-[1fr_380px] md:py-16"
      style={F}
    >
      {/* form */}
      <div>
        ...
        <div className="mt-3 flex flex-col gap-2.5 sm:flex-row">
          {(
            [
              ["standard", "Standard — Free", "5–8 business days"],
              ["express", "Express — $25", "1–2 business days"],
            ] as const
          ).map(([k, label, note]) => (
            <button
              key={k}
        ...
        <p className="mt-2 text-[12.5px] text-[#111111]/40">
          Encrypted checkout · You'll receive an email confirmation.
        </p>

        <h2 className="mt-8 text-[15px] font-semibold text-[#111111]">
          Order notes <span className="font-normal text-[#111111]/40">(optional)</span>
        </h2>
        <textarea
          className={`${field} mt-3 min-h-[90px] resize-y`}
          placeholder="Anything we should know — gift note, delivery instructions…"
          value={f.notes}
          onChange={set("notes")}
        />
      ...
      <aside className="md:sticky md:top-24 md:h-fit">
        ...
        <a
          href="/cart"
          className="mt-3 block text-center text-[13px] text-[#111111]/50 hover:text-[#111111]"
        >
          ← Edit cart
        </a>
      </aside>
