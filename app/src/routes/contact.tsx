  ...
  head: () => ({
    meta: [
      { title: "Contact — ELSIAA · AI Done Better" },
      {
        name: "description",
        content:
          "Let's talk. The first 20 minutes are free. Book a call or send ELSIAA a message.",
      },
...
const STEPS: Array<[string, string, string]> = [
  ["01", "Free call", "20 minutes to understand the problem."],
    ...
    <main className="min-h-screen bg-white text-[#111111]">
      ...
      <section className="mx-auto max-w-5xl px-6 pt-32 pb-16 md:pt-40 md:pb-20">
        <Reveal>
          ...
          <p className="mt-5 max-w-xl text-lg text-[#111111]/55 md:text-xl" style={inter}>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-14">
            <Reveal className="lg:sticky lg:top-32 lg:self-start">
              ...
              <h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] md:text-4xl" style={inter}>Twenty minutes, no charge.</h2>
              <p className="mt-4 max-w-md text-[15px] leading-relaxed text-[#111111]/55" style={inter}>
                Pick a time that works. You talk through the problem, we map where AI actually pays off — a straight answer, no deck, no obligation. We confirm your slot by email within a business day.
              </p>
            </Reveal>
            <Reveal delay={0.08}>
              <BookingCalendar />
      ...
      {/* 3 · Message form */}
      <section id="message" className="scroll-mt-24 border-t border-black/[0.06] bg-white px-6 py-16 md:py-20">
        <div className="mx-auto max-w-5xl">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-14">
            <Reveal>
      ...
      {/* 4 · Direct contact */}
      <section className="border-t border-black/[0.06] bg-[#F5F5F3] px-6 py-12">
        <div className="mx-auto max-w-5xl">
...
/*  Message form — routes through the existing meeting pipeline.
    ...
    The endpoint requires a slot, so a written message is sent with a
    date-stamped, 00:00 sentinel slot and the intent captured in `topic`.
    On failure, a mailto fallback to info@elsiaa.com is offered.          */
/* ------------------------------------------------------------------ */
const NEXT_OPTIONS: Array<[string, string]> = [
  ["call", "Free 20-min call"],
  ["proposal", "Written proposal"],
  ["details", "Just sending details"],
];
  ...
  return (
    <form className="rounded-2xl border border-black/[0.07] bg-white p-6 shadow-[0_1px_30px_-12px_rgba(0,0,0,0.12)] md:p-8" onSubmit={(e) => { e.preventDefault(); submit(); }}>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} autoComplete="name" placeholder="Jane Doe"
            className="mt-2 min-h-[46px] w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/40" style={inter} />
        </label>
        <label className="block">
          <span className="text-[13px] font-medium text-[#111111]" style={inter}>Company</span>
          <input value={company} onChange={(e) => setCompany(e.target.value)} autoComplete="organization" placeholder="Company name"
            className="mt-2 min-h-[46px] w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/40" style={inter} />
        </label>
      </div>
      ...
      <label className="mt-4 block">
        ...
        <textarea value={looking} onChange={(e) => setLooking(e.target.value)} rows={4} placeholder="A few sentences on the problem you'd like to solve."
          className="mt-2 min-h-[96px] w-full resize-y rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/40" style={inter} />
      </label>
...
/* ------------------------------------------------------------------ */
/*  Booking calendar — the free 20-minute call. Preserves the backend:
    POST /api/meeting { name, email, company, topic, slotDate, slotTime }  */
/* ------------------------------------------------------------------ */
          ...
          <form className="mt-7 border-t border-black/[0.06] pt-6" onSubmit={(e) => { e.preventDefault(); submit(); }}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <label className="block">
                ...
                <input className="min-h-[46px] w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/50" style={inter} placeholder="Your name *" autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} />
              </label>
              <label className="block">
                <span className="sr-only">Email address</span>
                <input className="min-h-[46px] w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/50" style={inter} type="email" autoComplete="email" placeholder="Email *" value={email} onChange={(e) => setEmail(e.target.value)} />
              </label>
            </div>
            <label className="mt-3 block">
              <span className="sr-only">Company (optional)</span>
              <input className="min-h-[46px] w-full rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/50" style={inter} autoComplete="organization" placeholder="Company (optional)" value={company} onChange={(e) => setCompany(e.target.value)} />
            </label>
            <label className="mt-3 block">
              <span className="sr-only">What would you like to talk about?</span>
              <textarea className="min-h-[84px] w-full resize-y rounded-lg border border-black/10 bg-white px-4 py-3 text-[15px] outline-none focus:border-[#1e6b3c] focus-visible:ring-2 focus-visible:ring-[#1e6b3c]/25 placeholder:text-[#111111]/50" style={inter} placeholder="What would you like to talk about? (optional)" value={topic} onChange={(e) => setTopic(e.target.value)} />
            </label>
