...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../poel-api/companies/[id]/fees-and-subscriptions/route");
...
export const Route = createFileRoute("/api/companies/$id/fees-and-subscriptions")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
