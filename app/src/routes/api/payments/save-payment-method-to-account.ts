...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../poel-api/payments/save-payment-method-to-account/route");
...
export const Route = createFileRoute("/api/payments/save-payment-method-to-account")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
