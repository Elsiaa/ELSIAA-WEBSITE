...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../poel-api/payments/create-setup-intent-account/route");
...
export const Route = createFileRoute("/api/payments/create-setup-intent-account")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
