...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../poel-api/projects/[id]/program-log-ingest-url/route");
...
export const Route = createFileRoute("/api/projects/$id/program-log-ingest-url")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
