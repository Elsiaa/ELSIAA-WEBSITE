...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../../poel-api/ingest/program-logs/[projectId]/[token]/route");
...
export const Route = createFileRoute("/api/ingest/program-logs/$projectId/$token")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
