...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../poel-api/admin/payments/attach-default-to-company/route");
...
export const Route = createFileRoute("/api/admin/payments/attach-default-to-company")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
