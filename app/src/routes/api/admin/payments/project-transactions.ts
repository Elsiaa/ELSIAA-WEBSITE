...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../poel-api/admin/payments/project-transactions/route");
...
export const Route = createFileRoute("/api/admin/payments/project-transactions")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
