...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../../poel-api/admin/projects/[id]/extension-source/route");
...
export const Route = createFileRoute("/api/admin/projects/$id/extension-source")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
