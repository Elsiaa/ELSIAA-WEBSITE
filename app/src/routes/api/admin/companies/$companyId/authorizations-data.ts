...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../../poel-api/admin/companies/[companyId]/authorizations-data/route");
...
export const Route = createFileRoute("/api/admin/companies/$companyId/authorizations-data")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
