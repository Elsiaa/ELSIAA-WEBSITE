...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../../../poel-api/support/threads/[id]/messages/[messageId]/route");
...
export const Route = createFileRoute("/api/support/threads/$id/messages/$messageId")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
