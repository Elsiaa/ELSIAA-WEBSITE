...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../../poel-api/pdf-signatures/requests/[id]/signatures/route");
...
export const Route = createFileRoute("/api/pdf-signatures/requests/$id/signatures")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
