...
function method(name: "GET" | "POST" | "PUT" | "PATCH" | "DELETE") {
  return async ({ request, params }: Ctx) => {
    const handlers = await import("../../../../../poel-api/pdf-signatures/public/[token]/submit/route");
...
export const Route = createFileRoute("/api/pdf-signatures/public/$token/submit")({
  server: {
    handlers: {
      GET: method("GET"),
      POST: method("POST"),
