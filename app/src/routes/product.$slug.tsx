...
function ProductBody() {
  ...
  if (!p) {
    return (
      <section className="mx-auto max-w-6xl px-6 py-10 md:py-16 text-center" style={F}>
        ...
        <a
          href="/store"
          className="mt-4 inline-block text-[14px] font-medium text-[#1e6b3c] hover:underline"
        >
          ← Back to the store
        </a>
      </section>
  ...
  return (
    <>
      <section
        className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-6 pt-8 pb-10 md:pb-16 md:grid-cols-2 md:gap-14 md:pt-14"
        style={F}
      >
        {/* gallery */}
            ...
            {[0, 1, 2].map((i) => (
              <div key={i} className="overflow-hidden rounded-2xl bg-[#f6f6f4]">
                <img
                  ...
                  style={{
                    objectPosition: `${30 + i * 25}% ${25 + i * 25}%`,
                    transform: `scale(${1.6 + i * 0.3})`,
                  }}
                />
              </div>
            ))}
        ...
        <div className="md:pt-4">
          ...
          <h1 className="mt-1.5 text-[26px] font-semibold tracking-[-0.02em] text-[#111111] md:text-[32px]">
            {p.name}
          </h1>
          <div className="mt-2 flex items-baseline gap-3">
            <span className="text-[20px] font-semibold text-[#111111]">${p.price}</span>
            {p.limited && (
              <span className="rounded-full bg-[#1e6b3c]/[0.08] px-3 py-1 text-[12px] font-semibold text-[#1e6b3c]">
                Limited
              </span>
            )}
          </div>
          {p.limited && (
            <p className="mt-2 text-[13px] text-[#111111]/45">
              One run. When this drop sells through, it's never reprinted.
            </p>
          )}
          <p className="mt-5 max-w-md text-[15px] leading-relaxed text-[#111111]/70">
            {p.description}
          </p>
          ...
          {/* size */}
          <p
            className={`mt-7 text-[13.5px] font-medium transition-colors ${warn ? "text-red-600" : "text-[#111111]/70"}`}
          >
              ...
              <button
                ...
                className={`min-h-[44px] rounded-xl border px-5 text-[14px] font-medium transition-all ${
                  size === s
                    ...
                    : `${warn ? "border-red-300" : "border-black/[0.14]"} text-[#111111]/70 hover:border-black/40`
                }`}
              >
                {s}
              </button>
          ...
          <p className="mt-4 text-[13px] text-[#111111]/40">
            Free standard shipping · Express available at checkout
          </p>
        </div>
      </section>
      ...
      <div
        className="fixed inset-x-0 bottom-0 z-30 border-t border-black/[0.06] bg-white/95 px-4 pt-3 backdrop-blur-md md:hidden"
        style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))", ...F }}
      >
        <button
          onClick={() => {
            const s = requireSize();
