...
function meetingToRow(meeting: Partial<Meeting>): Partial<MeetingRow> {
  ...
  if (meeting.participantUserIds !== undefined)
    row.participant_user_ids = meeting.participantUserIds;
  if (meeting.participantCompanyIds !== undefined)
    row.participant_company_ids = meeting.participantCompanyIds;
  ...
  if (meeting.meetingRequestId !== undefined)
    row.meeting_request_id = meeting.meetingRequestId || null;
...
export async function createMeeting(
  meeting: Omit<Meeting, "id"> & { id?: string },
): Promise<Meeting> {
...
export async function getUserMeetings(userId: string): Promise<Meeting[]> {
  ...
  const participantMeetings = (allMeetingsForParticipant || []).filter((meeting: any) => {
    return (
      Array.isArray(meeting.participant_user_ids) && meeting.participant_user_ids.includes(userUuid)
    );
  });
  ...
  if (participantError) {
    console.error(
      "Error fetching meetings for participant filter (continuing with empty array):",
      participantError,
    );
  }
