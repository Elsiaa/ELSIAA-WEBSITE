...
export function totalFromLineItems(items: InvoiceLineItem[]): number {
  return (
    Math.round(items.reduce((sum, row) => sum + row.quantity * row.unit_amount, 0) * 100) / 100
  );
}

export function validateLineItemsForCreate(
  items: unknown,
): { ok: true; items: InvoiceLineItem[] } | { ok: false; error: string } {
  const normalized = normalizeInvoiceLineItems(items);
  if (!normalized || normalized.length === 0) {
    return {
      ok: false,
      error: "Add at least one line item with description, quantity, and unit price.",
    };
  }
