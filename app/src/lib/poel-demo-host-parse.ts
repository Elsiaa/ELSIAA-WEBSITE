...
export const POEL_CLOCK_HOST = (() => {
  const fromEnv =
    ...
    (typeof process.env.POEL_CLOCK_DEMO_HOST === 'string' && process.env.POEL_CLOCK_DEMO_HOST.trim()) ||
    '';
...
export function isPoelDemoDevLoopbackHost(hostHeader: string | null): boolean {
  ...
  if (process.env.NODE_ENV !== 'development' || !process.env.POEL_DEMO_DEV_SLUG?.trim()) return false;
