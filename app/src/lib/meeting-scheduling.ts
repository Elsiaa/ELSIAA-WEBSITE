...
function meetingRequestToRow(request: Partial<MeetingRequest>): Partial<MeetingRequestRow> {
  ...
  if (request.confirmedTimeSlot !== undefined)
    row.confirmed_time_slot = request.confirmedTimeSlot || null;
...
export function generateTimeSlots(
  ...
  const startYear = parseInt(
    startDate.toLocaleString("en-US", { timeZone: TIMEZONE, year: "numeric" }),
  );
  const startMonth =
    parseInt(startDate.toLocaleString("en-US", { timeZone: TIMEZONE, month: "numeric" })) - 1;
  const startDay = parseInt(
    startDate.toLocaleString("en-US", { timeZone: TIMEZONE, day: "numeric" }),
  );

  const endYear = parseInt(
    endDate.toLocaleString("en-US", { timeZone: TIMEZONE, year: "numeric" }),
  );
  const endMonth =
    parseInt(endDate.toLocaleString("en-US", { timeZone: TIMEZONE, month: "numeric" })) - 1;
...
export async function filterAvailableSlots(
  ...
  try {
    ...
    try {
      ...
      existingMeetings = relevantMeetings.map((m) => ({
...
export async function getMeetingRequest(id: string): Promise<MeetingRequest | null> {
  ...
  const { data, error } = await supabase.from("meeting_requests").select("*").eq("id", id).single();
...
export async function confirmMeetingRequest(
  ...
  if (request.status === "cancelled") {
    throw new Error(
      `Cannot confirm a cancelled meeting request. Current status: ${request.status}`,
    );
  }
  ...
  if (isReconfirming) {
    ...
    throw new Error(
      `Meeting request is not in a confirmable state. Current status: ${request.status}`,
    );
  }
  ...
  const meeting = await createMeeting({
    ...
    description:
      request.message ||
      `Scheduled consultation with ${request.name}${request.company ? ` from ${request.company}` : ""}`,
...
export async function unblockTimeSlot(blockId: string): Promise<boolean> {
  ...
  const { error } = await supabase.from("blocked_time_slots").delete().eq("id", blockId);
 ...
 */
export async function getAllBlockedTimeSlots(
  startDate?: Date,
  endDate?: Date,
): Promise<BlockedTimeSlot[]> {
  const supabase = getServerSupabaseClient();
