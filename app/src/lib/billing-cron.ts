...
import {
  attachCompanyDefaultToActiveAutoChargeBills,
  getCompanyIdsWithDueBills,
} from "@/lib/bills";
import {
...
import {
  ...
  readOperationalLogoBase64ForPdf,
...
export async function processAllDueBillings(
  asOfDate?: Date,
  options?: { debug?: boolean; companyId?: string },
): Promise<ProcessAllDueBillingsResult> {
  ...
  const subscriptionDebug: ProcessAllDueBillingsResult["subscriptionDebug"] = debug
    ? []
    : undefined;
  const paymentRequestDebug: ProcessAllDueBillingsResult["paymentRequestDebug"] = debug
    ? []
    : undefined;
  const dryRunDebug: ProcessAllDueBillingsResult["dryRunDebug"] = debug
    ? {
        companyIdsToAttach: [],
        attachResults: [],
        duePaymentRequestsCount: 0,
        duePaymentRequestsAfterScopeCount: 0,
      }
    : undefined;
  ...
  for (const sub of allSubscriptions) {
    if (debug) {
      if (sub.stripe_subscription_id) {
        subscriptionDebug!.push({
          ...
          reason: "skipped: has stripe_subscription_id (billed by Stripe)",
        });
        continue;
      }
      if (!sub.next_billing_date) {
        subscriptionDebug!.push({
          ...
          reason: "skipped: no next_billing_date",
        });
        continue;
      }
      ...
      if (nextBillingDate > now) {
        subscriptionDebug!.push({
          ...
          name: sub.name || "",
          next_billing_date: sub.next_billing_date,
          reason: `skipped: next_billing_date (${sub.next_billing_date}) is after asOf (${now.toISOString().slice(0, 10)})`,
        });
        continue;
      }
    } else {
    ...
    try {
      ...
      if (!customerId || !paymentMethodId) {
        const methodForSubscription = await getPaymentMethodForBilling(
          sub.company_id,
          "subscription",
        );
        ...
        if (!customerId || !paymentMethodId) {
          ...
          for (const u of companyUsers || []) {
            const userMethod =
              (await getDefaultPaymentMethod({ userId: u.id })) ||
      ...
      if (debug) {
        subscriptionDebug!.push({
          ...
          name: sub.name || "",
          next_billing_date: sub.next_billing_date,
          reason: "would_process (debug: no charge)",
        });
        continue;
      }
      ...
      if (paymentIntent.status === "succeeded" || paymentIntent.status === "processing") {
        if (sub.payment_request_id && customerId && paymentMethodId) {
          try {
            await updatePaymentRequestStripeInfo(
              sub.payment_request_id,
              customerId,
              paymentMethodId,
            );
          } catch (e) {
            console.error(
              "Failed to attach used payment method to subscription payment_request:",
              e,
            );
          }
        }
        ...
        if (sub.payment_request_id && invoiceNumber != null) {
          try {
            ...
            console.error(
              "Failed to update subscription payment intent metadata with invoice number:",
              e,
            );
          }
        }
  ...
  if (scopeCompanyId) {
    const userIds = [
      ...new Set(duePaymentRequests.map((pr) => pr.user_id).filter(Boolean)),
    ] as string[];
    ...
    if (userIds.length > 0) {
      const { data: users } = await supabase
        ...
        .in("id", userIds);
      userCompanyMap = new Map(
        (users || []).map((u: { id: string; company_id: string | null }) => [u.id, u.company_id]),
      );
    }
    const companyLinkedPrIds = new Set<string>();
    const { data: feePrIds } = await supabase
      .from("project_fees")
      .select("payment_request_id")
      .eq("company_id", scopeCompanyId)
      .not("payment_request_id", "is", null);
    (feePrIds || []).forEach((r: { payment_request_id: string }) =>
      companyLinkedPrIds.add(r.payment_request_id),
    );
    const { data: subPrIds } = await supabase
      .from("project_subscriptions")
      .select("payment_request_id")
      .eq("company_id", scopeCompanyId)
      .not("payment_request_id", "is", null);
    (subPrIds || []).forEach((r: { payment_request_id: string }) =>
      companyLinkedPrIds.add(r.payment_request_id),
    );
    duePaymentRequests = duePaymentRequests.filter(
      (pr) =>
        (pr.user_id != null && userCompanyMap.get(pr.user_id) === scopeCompanyId) ||
        companyLinkedPrIds.has(pr.id),
    );
  }
  ...
  if (dryRunDebug && companyIdsToAttach.length > 0) {
    ...
    (subLinks || []).forEach((r: { payment_request_id: string; next_billing_date: string }) =>
      subscriptionNextBillingByPrId.set(r.payment_request_id, r.next_billing_date),
    );
    ...
    const companyIdsWithFutureSubscription = new Set(
      (futureSubCompanies || []).map((r: { company_id: string }) => r.company_id),
    );
    const breakdown: NonNullable<
      ProcessAllDueBillingsResult["dryRunDebug"]
    >["allPaymentRequestsBreakdown"] = [];
    const skippedNoMethodList: Array<{
      id: string;
      payment_type: string;
      next_billing_date: string | null;
      user_id: string | null;
    ...
    const prRow = (
      ...
      const subscriptionNotDueYetLinked =
        subNextBilling && new Date(subNextBilling).getTime() > nowTime;
      const subscriptionNotDueYetCompany =
        p.payment_type === "monthly" && companyIdsWithFutureSubscription.has(companyId);
      ...
      let reason: string;
      if (subscriptionNotDueYet)
        reason = subscriptionNotDueYetLinked
        ...
        reason = `not due yet (next_billing_date ${p.next_billing_date || "null"} is after today)`;
    ...
    for (const companyId of companyIdsToAttach) {
      ...
      const { data: feePrIds } = await supabase
        ...
        .not("payment_request_id", "is", null);
      (feePrIds || []).forEach((r: { payment_request_id: string }) =>
        prIdsFromCompany.add(r.payment_request_id),
      );
      const { data: subPrIds } = await supabase
        .from("project_subscriptions")
        .select("payment_request_id")
      ...
      const { data: prs } = await supabase
        .from("payments_requests")
        .select(
          "id, payment_type, status, amount, next_billing_date, user_id, stripe_customer_id, stripe_payment_method_id",
        )
      ...
      for (const p of allPrs) {
        ...
        if (!hasMethod && isDueByDate)
          skippedNoMethodList.push({
            id: p.id,
            payment_type: p.payment_type,
            next_billing_date: p.next_billing_date,
            user_id: p.user_id,
            reason: "due but no payment method attached",
          });
        breakdown.push({
    ...
    try {
      ...
      if (paymentIntent.status === "succeeded" || paymentIntent.status === "processing") {
        ...
        try {
          ...
          if (pr.user_id) {
            const { data: prUser } = await supabase
              .from("users")
              .select("company_id")
              .eq("id", pr.user_id)
              .maybeSingle();
      ...
      if (isInvalidPaymentMethod && pr.stripe_payment_method_id) {
        try {
          ...
          if (removed > 0) {
            console.warn(
              `[billing] Cleared invalid payment method ${pr.stripe_payment_method_id} from payment request ${pr.id} and removed ${removed} saved method(s). Customer should re-add a payment method.`,
            );
          }
        } catch (cleanupErr) {
  ...
  let billsResult:
    Awaited<ReturnType<typeof import("@/lib/bill-billing-engine").processDueBills>> | undefined;
  try {
...
function generateReceiptPdfBuffer(p: BillingReceiptParams): Promise<Buffer> {
  ...
  const docDef: any = {
    content: [
      ...
      {
        text: [{ text: `${p.recipientName}\n`, bold: true }, { text: p.recipientEmail }],
        margin: [0, 0, 0, 10],
      },
        ...
        table: {
          ...
          body: [
            [
              ...
              { text: `$${p.amount.toFixed(2)}`, style: "tableCell", alignment: "right" },
            ],
            ...(p.fee > 0
              ? [
                  [
                    { text: "Processing Fee (3%)", style: "tableHeader" },
                    { text: `$${p.fee.toFixed(2)}`, style: "tableCell", alignment: "right" },
                  ],
                ]
              : []),
            [
              { text: "Total Paid", style: "tableTotal" },
              { text: `$${p.total.toFixed(2)}`, style: "tableTotal", alignment: "right" },
            ],
          ],
        },
        layout: {
...
<body>
  ...
  await sendTransactionalMail({
    ...
    attachments: [
      {
        filename: `Receipt_${p.invoiceNumber}.pdf`,
        content: pdfBuffer,
 ...
 */
export async function sendOverdueWarningEmails(_now?: Date): Promise<WarningEmailsResult> {
