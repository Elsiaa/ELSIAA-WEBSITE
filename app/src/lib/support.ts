...
export async function canAccessSupportThread(
  ...
  if (companyUserHasModule(opts.appUser, 'support') && opts.appUser.company_id === thread.company_id) {
    return true;
  }
  if (
...
export async function listSupportThreadsForParticipant(appUserId: string): Promise<SupportThreadRow[]> {
  const supabase = getServerSupabaseClient();
...
export async function notifySupportMessageRecipients(input: {
  ...
  function considerEmail(
    ...
    if (n === senderNorm) {
      log('SKIP', { reason: 'is_sender_same_normalized_email', ...meta, normalized: n, raw: trimmed });
      return;
    }
    if (byNorm.has(n)) {
      log('SKIP', {
