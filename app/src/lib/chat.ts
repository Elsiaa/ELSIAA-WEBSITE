...
/**
 * Upload a file for support desk attachments (R2 key under support-files/)
 */
export async function uploadSupportFile(
  ...
  const timestamp = Date.now();
  const sanitizedFilename = file.name.replace(/[^a-zA-Z0-9.-]/g, "_");
  const fileKey = `support-files/${threadId}/${messageId}/${timestamp}-${sanitizedFilename}`;
...
export async function deleteChatFile(fileUrl: string): Promise<void> {
  try {
    ...
    const keyIndex =
      urlParts.indexOf("chat-files") >= 0
        ? urlParts.indexOf("chat-files")
        : urlParts.indexOf("support-files");
