...
import { requireSuperAdmin, requireCompanyAccess, isSuperAdmin, requireCompanyAdmin } from '@/lib/permissions';
...
export async function PATCH(
  ...
  try {
    ...
    if (superAdmin) {
      ...
      if (hasFilesFlag) updates.support_agent_company_files_allowed = body.support_agent_company_files_allowed;
    ...
    if (hasName) {
      return NextResponse.json({ error: 'Only super admins can rename a company' }, { status: 403 });
    }
    if (!hasFilesFlag) {
      return NextResponse.json({ error: 'No valid fields to update' }, { status: 400 });
    }
