...
export async function PATCH(
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: 'Only super admins or authorized support agents can set access override' }, { status: 403 });
    }

    const body = await request.json();
    const accessOverride = body.accessOverride;
    if (accessOverride !== undefined && accessOverride !== null && accessOverride !== 'allowed' && accessOverride !== 'blocked') {
    ...
    const updated = await updateProject(projectId, {
      accessOverride: accessOverride === null || accessOverride === undefined ? null : accessOverride,
    });
    if (!updated) {
