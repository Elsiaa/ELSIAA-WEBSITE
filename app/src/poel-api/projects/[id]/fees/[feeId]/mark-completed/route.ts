...
import { getNextInvoiceNumber, getPaymentRequestById, updatePaymentRequestInvoiceAndStatus } from '@/lib/payments';
  ...
  try {
    ...
    if (fee.paymentRequestId) {
      ...
      if (pr && pr.status !== 'completed') {
        await updatePaymentRequestInvoiceAndStatus(fee.paymentRequestId, invoiceNumber, 'completed');
      }
    }
