...
export async function PATCH(
  ...
  try {
    ...
    if (body.device_limit === undefined) {
      return NextResponse.json({ error: 'device_limit is required (number or null)' }, { status: 400 });
    }
    ...
    if (body.device_limit === null || body.device_limit === '') {
      ...
      if (!Number.isFinite(n) || n < 0 || !Number.isInteger(n)) {
        return NextResponse.json({ error: 'device_limit must be a non-negative integer or null' }, { status: 400 });
      }
      deviceLimit = n;
    }
