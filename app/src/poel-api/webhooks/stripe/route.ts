...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'invoice.payment_succeeded') {
    ...
    try {
      ...
      if (!subscription) {
        console.log(`No project subscription found for Stripe subscription ${stripeSubscriptionId}`);
      ...
      try {
        ...
        if (subscription.paymentRequestId) {
          ...
          recipientName = admin ? `${admin.first_name || ''} ${admin.last_name || ''}`.trim() || admin.email || subscription.name || 'Subscription' : subscription.name || 'Subscription';
        }
        if (recipientEmail) {
      ...
      console.log(`Created transaction for subscription ${subscription.id} (Stripe invoice ${invoice.id})`);
