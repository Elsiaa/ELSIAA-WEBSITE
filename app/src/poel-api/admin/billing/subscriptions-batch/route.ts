...
import {
  fetchCompanyPaymentStatusFast,
  fetchFeesSubscriptionsByCompanies,
} from "@/lib/admin-db-rpc";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [feesSubs, statusRows] = await Promise.all([
      ...
      Promise.all(
        companyIds.map(async (id) => ({ id, status: await fetchCompanyPaymentStatusFast(id) })),
      ),
    ]);
