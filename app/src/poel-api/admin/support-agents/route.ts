...
import {
  getGrantsForUser,
  replaceGrantsForUser,
  type SupportAgentGrantInput,
  ...
  try {
    ...
    if (effective.length === 0) {
      return NextResponse.json(
        {
          error:
            "At least one company grant (support, authorizations, program logs, and/or files) is required",
        },
        { status: 400 },
      );
    }
    ...
    if (existing) {
      return NextResponse.json(
        {
          error:
            "A user with this email already exists. Remove or convert them before creating a support agent.",
        },
        { status: 409 },
      );
    }
    ...
    return NextResponse.json({ user: updated, grants: grantsOut, invitationSent }, { status: 201 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Failed";
    if (msg.includes("Forbidden")) {
      return NextResponse.json({ error: msg }, { status: 403 });
