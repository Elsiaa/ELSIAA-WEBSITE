...
import {
  createBill,
  listAdminBillsWithCharges,
  normalizeBillRecurrenceInterval,
} from "@/lib/bills";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const coll = (
      collectionMode === "auto_charge" ? "auto_charge" : "invoice_link"
    ) as BillCollectionMode;
    ...
    const bill = await createBill({
      ...
      recurrenceInterval:
        sched === "recurring"
          ? (normalizeBillRecurrenceInterval(
              "recurring",
              recurrenceInterval as BillRecurrenceInterval,
            ) ?? undefined)
          : undefined,
