  ...
  try {
    ...
    if (isSuperAdmin) {
      return NextResponse.json(
        { error: "Super admins cannot attach payment methods" },
        { status: 403 },
      );
    }
    ...
    if (!dbUser || dbUser.role !== "admin") {
      return NextResponse.json(
        { error: "Only company admins can attach payment methods" },
        { status: 403 },
      );
    }

    const body = await request.json();
    const { paymentRequestId, subscriptionId, feeId, stripeCustomerId, stripePaymentMethodId } =
      body;
    ...
    if (feeId) {
      ...
      return NextResponse.json(
        { success: true, paymentRequestId: paymentRequest.id },
        { status: 200 },
      );
    }
      ...
      if (isDue && subscription.status === "active") {
        try {
          ...
          if (prFresh?.status === "completed") {
            return NextResponse.json(
              { success: true, paymentRequestId: paymentRequest.id, paymentProcessed: true },
              { status: 200 },
            );
          }
          ...
          if (paymentIntent.status === "succeeded") {
            ...
            // Send receipt email (in background, don't wait)
            const { email: subRecipientEmail, name: subRecipientName } =
              getRequestDisplayInfo(paymentRequest);
    ...
    if (!paymentRequestId) {
      return NextResponse.json(
        { error: "Payment request ID or subscription ID required" },
        { status: 400 },
      );
    }
    ...
    // Update the payment request with the payment method
    await updatePaymentRequestStripeInfo(paymentRequestId, stripeCustomerId, stripePaymentMethodId);
