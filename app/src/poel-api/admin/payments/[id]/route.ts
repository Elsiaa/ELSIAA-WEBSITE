...
export async function GET(
  ...
  try {
    ...
    if (!isSuperAdmin) {
      return NextResponse.json({ error: 'Only super admins can view payment request details' }, { status: 403 });
    }
...
export async function PATCH(
  ...
  try {
    ...
    const isSuperAdmin = await checkSuperAdmin();
    if (!isSuperAdmin) {
      return NextResponse.json({ error: 'Only super admins can edit payment requests' }, { status: 403 });
    }
    ...
    if (body.invoiceLineItems !== undefined) {
      if (body.invoiceLineItems === null) {
        ...
        if (body.amount !== undefined) {
          ...
          return NextResponse.json({ error: 'Amount is required when removing line items' }, { status: 400 });
        }
      } else {
    ...
    if (body.nextBillingDate !== undefined) {
      updates.next_billing_date = body.nextBillingDate === '' || body.nextBillingDate == null ? null : body.nextBillingDate;
    }
