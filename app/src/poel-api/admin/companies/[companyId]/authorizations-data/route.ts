...
import {
  loadAuthorizationsBundleProjects,
  bundleExtensionToSource,
} from "@/lib/company-authorizations-bundle";
  ...
  try {
    ...
    const projectsPayload = rows.map((row, i) => {
      ...
      return {
        ...
        devices: superAdmin ? row.devices : row.devices.filter((d) => !d.isAdminDevice),
        extensionSource,
