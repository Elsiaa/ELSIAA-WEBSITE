...
} from "@/lib/user-project-permissions";
  ...
  try {
    ...
    if (!superuser && (!requestingUser || requestingUser.id !== id)) {
      if (
        ...
        requestingUser.company_id !== targetUser.company_id
      ) {
  ...
  try {
    ...
    if (targetUser.role === "admin") {
      ...
      return NextResponse.json(
        {
          error:
            "Forbidden - admins have access to all projects and cannot have specific project permissions set",
        },
        { status: 403 },
      );
    }
