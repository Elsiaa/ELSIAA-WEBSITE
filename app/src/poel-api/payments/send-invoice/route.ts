...
export async function POST(request: NextRequest) {
  try {
    const { name, email, originalAmount, fee, total, paymentIntentId, public_token } =
      await request.json();
    ...
    if (isDummy) {
      ...
      try {
        ...
        displayMethod = rail === "ach" ? PAYMENT_METHOD_LABEL_ACH : PAYMENT_METHOD_LABEL_CARD;
      } catch (e) {
    ...
    const docDefinition: any = {
      content: [
        ...
        { text: "INVOICE", style: "title", alignment: "center", margin: [0, 20, 0, 20] },
        {
          columns: [
            {
              stack: [{ text: "Bill To:", bold: true }, { text: name }, { text: email }],
            },
            {
              stack: [
          ...
          table: {
            ...
            body: [
              ...
              [
                ...
                { text: `$${effectiveTotal.toFixed(2)}`, bold: true, alignment: "right" },
              ],
            ],
          },
