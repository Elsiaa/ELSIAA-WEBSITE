...
assert.equal(
  typeof workflowPlatform.createWorkflowPlatformAdapter,
  "function",
  "workflow-platform subpath broken",
);
assert.equal(
  typeof workflowPlatform.createFetchTransport,
  "function",
  "fetch transport missing from workflow-platform subpath",
);
assert.equal(
  root.createWorkflowPlatformAdapter,
  workflowPlatform.createWorkflowPlatformAdapter,
  "workflow-platform adapter duplicated across entrypoints",
);
...
console.log(
  "smoke ok: subpaths import, single error hierarchy, media codes registered, memory-backend round-trip works",
);
