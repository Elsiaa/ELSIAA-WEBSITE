...
function stubBackend(cost = 1): GenerationBackend {
  return {
    createJobs: async ({ jobSetType }) => [
      { id: "job-1", job_set_type: jobSetType, status: "completed", result_url: "memory://out" },
    ],
...
describe("fnf observability", () => {
  it("emits job lifecycle spans without changing client behavior", async () => {
    ...
    expect(
      rec.events.find((e) => e.name === "fnf.job.submit" && e.phase === "success")?.attributes,
    ).toMatchObject({ generation_count: 1 });
  ...
  it("emits typed error metadata for SDK failures", async () => {
    ...
    await expect(client.submit({ model: "missing_model" } as never)).rejects.toBeInstanceOf(
      ApiJobError,
    );
  ...
  it("observes transport requests with sanitized paths and safe status metadata", async () => {
    const rec = recorder();
    const transport = withObservedTransport(async () => ({ status: 201, body: { ok: true } }), {
      observer: rec.observer,
    });

    await transport({
      method: "POST",
      path: "/jobs/topaz-image?dry_run=true&token=secret",
      body: { prompt: "secret" },
    });

    const success = rec.events.find(
      (e) => e.name === "fnf.transport.request" && e.phase === "success",
    );
    expect(success?.attributes).toMatchObject({
      method: "POST",
      path: "/jobs/topaz-image?dry_run&token",
      status: 201,
    });
  ...
  it("media upload spans do not expose upload URLs, filenames, bytes, or result URLs", async () => {
    ...
    const mediaBackend: MediaBackend = {
      ...
      getUploadUrl: async () => ({
        id: "m1",
        url: "https://cdn/private.png",
        upload_url: "https://s3/private-put",
      }),
  ...
  it("profile spans expose only coarse metadata and ids", async () => {
    ...
    const snapshot = rec.events.find(
      (e) => e.name === "fnf.profile.get_snapshot" && e.phase === "success",
    );
  ...
  it("wrappers preserve behavior and observer failures do not affect operations", async () => {
    ...
    ).resolves.toHaveProperty("generations");
  ...
  it("withObservedTransport preserves error behavior", async () => {
    ...
    await expect(transport({ method: "GET", path: "/health" })).rejects.toMatchObject({
      code: "network",
    });
