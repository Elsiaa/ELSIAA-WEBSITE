...
export function composeObservers(
  ...observers: Array<FnfObserver | undefined | null | false>
): FnfObserver {
...
export function createConsoleObserver(
  consoleLike: Pick<Console, "debug" | "error"> = console,
): FnfObserver {
  return (event) => {
...
export function createObservabilityContext(
  options?: FnfObservabilityOptions,
  ...
  try {
    ...
    emit(
      ...
      makeEvent(obs, {
        ...
        attributes: {
          ...base,
          ...safeReadAttributes(() => options.successAttributes?.(result) ?? {}),
        },
        ...(options.parentId ? { parentId: options.parentId } : {}),
      }),
    );
    ...
    emit(
      ...
      makeEvent(obs, {
        ...
        attributes: {
          ...base,
          ...safeReadAttributes(() => options.errorAttributes?.(error) ?? {}),
        },
...
export function withObservedTransport(
  transport: Transport,
  observability?: FnfObservabilityOptions | FnfObservabilityContext,
): Transport {
...
export function withObservedGenerationBackend(
  ...
  return {
    createJobs: (req) =>
      observeAsync(
        ctx,
        "fnf.backend.generation.create_jobs",
        { job_set_type: req.jobSetType },
        () => backend.createJobs(req),
      ),
    getJob: (id) =>
      observeAsync(ctx, "fnf.backend.generation.get_job", { generation_id: id }, () =>
        backend.getJob(id),
      ),
    ...(backend.getJobSet
      ? {
          getJobSet: (id) =>
            observeAsync(ctx, "fnf.backend.generation.get_job_set", { job_set_id: id }, () =>
              backend.getJobSet!(id),
            ),
        }
      : {}),
    listJobs: (query) =>
      observeAsync(ctx, "fnf.backend.generation.list_jobs", listQueryAttributes(query), () =>
        backend.listJobs(query),
      ),
    estimateCost: (req) =>
      observeAsync(
        ctx,
        "fnf.backend.generation.estimate_cost",
        { job_set_type: req.jobSetType },
        () => backend.estimateCost(req),
      ),
    ...(backend.cancelJob
      ? {
          cancelJob: (id) =>
            observeAsync(ctx, "fnf.backend.generation.cancel_job", { generation_id: id }, () =>
    ...
    ...(backend.confirm
      ? {
          confirm: (req) =>
            observeAsync(
              ctx,
              "fnf.backend.generation.confirm",
              { job_set_type: req.jobSetType },
              () => backend.confirm!(req),
            ),
        }
...
export function withObservedMediaBackend(
  ...
  return {
    getMedia: (query) =>
      observeAsync(
        ctx,
        "fnf.backend.media.get_media",
        { media_id: query.id, media_type: query.type },
        () => backend.getMedia(query),
      ),
    listMedia: (query) =>
      observeAsync(
        ctx,
        "fnf.backend.media.list_media",
        { media_type: query.type, ...(query.size !== undefined ? { size: query.size } : {}) },
        () => backend.listMedia(query),
      ),
    ...(backend.getUploadUrl
      ? {
          getUploadUrl: (req) =>
            observeAsync(
              ctx,
              "fnf.backend.media.get_upload_url",
              {
                media_type: req.type,
    ...
    ...(backend.confirmMedia
      ? {
          confirmMedia: (req) =>
            observeAsync(
              ctx,
              "fnf.backend.media.confirm_media",
              {
...
export function withObservedProfileBackend(
  ...
  return {
    ...
    listWorkspaces: () =>
      observeAsync(ctx, "fnf.backend.profile.list_workspaces", {}, () => backend.listWorkspaces()),
    getCurrentWorkspace: () =>
      observeAsync(ctx, "fnf.backend.profile.get_current_workspace", {}, () =>
        backend.getCurrentWorkspace(),
      ),
    getWorkspaceWallet: () =>
      observeAsync(ctx, "fnf.backend.profile.get_workspace_wallet", {}, () =>
        backend.getWorkspaceWallet(),
      ),
    switchWorkspace: (req) =>
      observeAsync(
        ctx,
        "fnf.backend.profile.switch_workspace",
        { workspace_id: req.workspaceId },
        () => backend.switchWorkspace(req),
      ),
  };
}

export function withObservedUploader(
...
function asContext(
  ...
  if (isObservabilityContext(ctx)) return ctx;
...
function errorInfo(error: unknown): FnfObservationError {
  ...
  if (error instanceof Error) return { code: "unexpected", name: error.name };
...
function listQueryAttributes(query: {
  ...
  return {
    ...
    ...(query.status !== undefined
      ? { status_count: Array.isArray(query.status) ? query.status.length : 1 }
      : {}),
    ...(query.model !== undefined
      ? { model_count: Array.isArray(query.model) ? query.model.length : 1 }
      : {}),
  };
}
