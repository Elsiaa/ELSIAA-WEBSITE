...
export function closestAspectRatio(value: string, options: readonly string[]): string {
  ...
  if (target == null) return options[0] ?? value;
...
export function clampDuration(value: number, n: Extract<Normalize, { kind: "duration" }>): number {
  ...
  let out = value;
  if (n.min !== undefined) out = Math.max(n.min, out);
  if (n.max !== undefined) out = Math.min(n.max, out);
  return out;
}
...
export function normalizeSettings(
  ...
  for (const [key, n] of Object.entries(normalizers)) {
    ...
    if (!enabled.has(ADJUST_BY_KIND[n.kind])) continue;
    const value = out[key];
    ...
    if (next !== value) adjustments.push({ field: key, from: value, to: next });
    out[key] = next;
  }
