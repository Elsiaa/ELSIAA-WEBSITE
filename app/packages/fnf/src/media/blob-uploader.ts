...
function toBodyInit(bytes: MediaBytes): Blob | Uint8Array<ArrayBuffer> {
  if (bytes instanceof ArrayBuffer) return new Uint8Array(bytes); // a bare ArrayBuffer isn't a valid fetch body in every runtime; wrap it
...
export function createFetchUploader(
  opts: { fetch?: typeof globalThis.fetch; retries?: number } = {},
): BinaryUploader {
  ...
  const doFetch =
    opts.fetch ?? ((...args: Parameters<typeof globalThis.fetch>) => globalThis.fetch(...args));
  ...
  return {
    async transfer({ uploadUrl, bytes, contentType, signal }) {
      ...
      throw new UploadTransferError(
        `Upload failed after ${retries + 1} attempts${cause}`,
        lastStatus,
      );
    },
    ...
    async fetchBytes(url, fetchOpts) {
      ...
      if (!res.ok) throw new UrlIngestError(`Failed to fetch ${url} (${res.status})`);
      ...
      const buf = await res.arrayBuffer().catch((err) => {
        throwIfAborted(fetchOpts?.signal);
        throw new UrlIngestError(
          `Failed to read ${url}: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
      if (fetchOpts?.maxBytes != null && buf.byteLength > fetchOpts.maxBytes)
        throw new UrlIngestError(
          `Media exceeds maxBytes (${buf.byteLength} > ${fetchOpts.maxBytes})`,
        );
      return {
