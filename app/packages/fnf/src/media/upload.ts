  ...
  return observeAsync(
    ...
    async () => {
      ...
      return {
        ...
        ref: result.ref.url
          ? result.ref
          : { ...result.ref, ...(slot.url ? { url: slot.url } : {}) },
      };
    },
    {
...
export async function safeUploadMedia(
  ctx: MediaContext,
  input: UploadInput,
  ...
  try {
    ...
    if (err instanceof ApiJobError) return { ok: false, error: err.toJSON() };
    return {
      ok: false,
...
export async function uploadMediaFromUrl(
  ...
  return observeAsync(
    ctx.observability,
    "fnf.media.upload_from_url",
    {
      ...(input.maxBytes !== undefined ? { max_bytes: input.maxBytes } : {}),
      media_type: input.type ?? "auto",
    },
    async () => {
      if (!ctx.blobUploader.fetchBytes) throw new UploadNotSupportedError();
      throwIfAborted(input.signal);
      const { url, maxBytes, ...rest } = input;
      const { bytes, contentType } = await ctx.blobUploader.fetchBytes(url, {
        maxBytes,
        signal: input.signal,
      });
...
export async function getUploadUrl(
  ...
  return observeAsync(
    ...
    async () => {
      if (!ctx.mediaAdapter.getUploadUrl) throw new UploadNotSupportedError();
      ...
      try {
        ...
        if (err instanceof ApiJobError) throw err;
      ...
      if (!mediaId || !uploadUrl) throw new PresignError();
      return {
        mediaId,
        uploadUrl,
        url: str(raw.url),
...
export async function transferBytes(
  ...
  await uploader.transfer({
    uploadUrl: slot.uploadUrl,
    bytes,
    contentType: contentType ?? slot.contentType,
    signal,
  });
}
...
export async function confirmMedia(
  ...
  return observeAsync(
    ...
    async () => {
      if (!ctx.mediaAdapter.confirmMedia) throw new UploadNotSupportedError();
      let raw: Record<string, unknown> = {};
      ...
      try {
        ...
        if (err instanceof ApiJobError) {
          if (
            req.throwOnModeration === false &&
            (err.code === "ip_detected" || err.code === "ip_check_rate_limit_reached")
          )
      ...
      const ref: MediaRef = {
        id,
        type: REF_TYPE_BY_UPLOAD[req.type],
        ...(url ? { url } : {}),
      ...
      return {
        ...
        filename:
          req.filename ??
          defaultFilenameForContentType(inferContentType(req.filename, undefined), req.type),
        ...(url ? { url } : {}),
...
function moderationOf(raw: Record<string, unknown>): UploadModeration | undefined {
  ...
  if (
    (status == null || !MODERATION_STATUSES.has(status)) &&
    ipCheckFinished === undefined &&
    ipDetected === undefined
  )
    return undefined;
  return {
    status: status ?? "not_ready",
    ...(ipCheckFinished !== undefined ? { ipCheckFinished } : {}),
    ...(ipDetected !== undefined ? { ipDetected } : {}),
  };
}
...
async function readSource(source: MediaSource): Promise<MediaBytes> {
  const bytes =
    typeof source === "object" && source !== null && "read" in source
      ? await source.read()
      : source;
  if (!isMediaBytes(bytes)) throw new InvalidMediaSourceError();
  return bytes;
}

function contentTypeOf(input: UploadInput, bytes?: MediaBytes): string {
  const fromBlob =
    typeof Blob !== "undefined" && bytes instanceof Blob && bytes.type ? bytes.type : undefined;
