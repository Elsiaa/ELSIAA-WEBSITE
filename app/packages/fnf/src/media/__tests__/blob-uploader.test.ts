...
describe("createFetchUploader.transfer", () => {
  ...
  it("an abort mid-PUT throws JobAbortedError without burning the retry budget", async () => {
    ...
    await expect(up.transfer({ ...PUT, signal: controller.signal })).rejects.toBeInstanceOf(
      JobAbortedError,
    );
...
describe("createFetchUploader.fetchBytes", () => {
  ...
  it("rejects early via content-length and late via byteLength when over maxBytes", async () => {
    const declared = uploader(
      async () => new Response("", { status: 200, headers: { "content-length": "10" } }),
    );
    await expect(declared.fetchBytes!("https://x/a", { maxBytes: 5 })).rejects.toBeInstanceOf(
      UrlIngestError,
    );

    const undeclared = uploader(async () => new Response(new Uint8Array(10), { status: 200 }));
    await expect(undeclared.fetchBytes!("https://x/a", { maxBytes: 5 })).rejects.toBeInstanceOf(
      UrlIngestError,
    );
  });
  ...
  it("an abort during the download throws JobAbortedError, not UrlIngestError", async () => {
    ...
    await expect(
      up.fetchBytes!("https://x/a", { signal: controller.signal }),
