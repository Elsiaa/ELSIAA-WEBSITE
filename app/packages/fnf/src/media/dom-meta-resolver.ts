...
export function createDomMediaMetaResolver(): MediaMetaResolver {
  ...
  return async (ref: MediaRef) => {
    if (!dom() || !ref.url) return undefined;
    const url = ref.url;
    let pending = cache.get(url);
    if (!pending) {
      const kind =
        ref.type === "image" || ref.type === "video" || ref.type === "audio" ? ref.type : undefined;
...
async function measure(url: string, kind?: string): Promise<MediaMeta | undefined> {
  ...
  if (kind === "audio" || AUDIO_URL.test(url)) return measureElement("audio", url);
  if (kind === "video") return measureElement("video", url);
  if (kind === "image") return measureImage(url);
