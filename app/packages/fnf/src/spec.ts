...
export function parseSettings(input: GenerationInput, entry: JobEntry): Record<string, unknown> {
  try {
    ...
    throw new ValidationError(
      "Invalid settings",
      (err as { issues?: unknown }).issues ?? String(err),
    );
  }
}
...
export function buildWireParams(input: GenerationInput, entry: JobEntry): Record<string, unknown> {
  ...
  if (issues.length > 0) throw new ValidationError(issues.map((i) => i.msg).join("; "), issues);

  let wire: Record<string, unknown> = {};
  if (entry.prompt) Object.assign(wire, promptCodec.serialize(input.prompt ?? {}));
  if (entry.media) Object.assign(wire, mediaCodec(entry.media).serialize(input.media ?? {}));
  ...
  if (input.folderId) wire.folder_id = input.folderId;
  if (input.parentId) wire.parent_id = input.parentId;
  ...
  if (input.extra) Object.assign(wire, input.extra);
  if (entry.finalize) wire = entry.finalize(wire, input);
  return wire;
}
...
export function parseGeneration(job: JobResponse, entry: JobEntry): Generation {
  ...
  for (const [local, wireKey] of Object.entries(entry.wireNames)) wireToLocal[wireKey] = local;
  const settings: Record<string, unknown> = {};
  const extra: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(wire)) {
    if (claimed.has(key)) continue;
    const local = wireToLocal[key] ?? key;
    if (local in entry.settingsMap) settings[local] = value;
...
function buildResults(entry: JobEntry, job: JobResponse): GenerationResults | undefined {
  if (job.status !== "completed" || !job.result_url) return undefined;
  const results: GenerationResults = { rawUrl: job.result_url };
  if (entry.outputType === "image" && job.min_result_url) results.minUrl = job.min_result_url;
