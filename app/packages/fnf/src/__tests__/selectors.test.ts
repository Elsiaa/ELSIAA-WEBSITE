...
describe("url selectors", () => {
  it("getRawUrl returns the full-quality url, getPreviewUrl prefers min → thumbnail → raw", () => {
    ...
    const video = gen({
      type: "video",
      results: { rawUrl: "https://x/o.mp4", thumbnailUrl: "https://x/t.webp" },
    });
    expect(getPreviewUrl(video)).toBe("https://x/t.webp");

    expect(getPreviewUrl(gen({ results: { rawUrl: "https://x/only.png" } }))).toBe(
      "https://x/only.png",
    );
...
describe("predicates", () => {
  ...
  it("hasResult narrows results to non-optional", () => {
    ...
    if (hasResult(done)) expect(done.results.rawUrl).toBe("https://x/o.png"); // no ?. needed — narrowed
...
describe("getMediaType", () => {
  it("answers from a Generation, a MediaRef url, or a bare url; undefined otherwise", () => {
    ...
    expect(getMediaType({ id: "m1", type: "media_input", url: "https://cdn/a.heic" })).toBe(
      "image",
    );
