...
describe("errorFromResponse", () => {
  ...
  it("maps not_enough_credits to OutOfCreditsError carrying the real HTTP status", () => {
    ...
    const err = errorFromResponse(403, {
      detail: {
        ...
        workspace: { type: "shared", role: "owner" },
      },
    });
    expect(err).toBeInstanceOf(OutOfCreditsError);
    expect(err?.code).toBe("out_of_credits");
    expect(err?.status).toBe(403);
    expect((err as OutOfCreditsError).data).toEqual({
      plan: "pro",
      billingPeriod: "monthly",
      team: { workspaceType: "Shared", userRole: "Owner" },
    });
  ...
  it('lowercases plan_type before casting (shared workspaces send "Team")', () => {
    ...
    const err = errorFromResponse(403, {
      detail: {
        ...
        plan_type: "Team",
        billing_period: "monthly",
        workspace: { type: "shared", role: "member" },
      },
    });
  ...
  it("maps both nsfw (real wire type) and prompt_nsfw (alias) to PromptNsfwError", () => {
    expect(errorFromResponse(400, { detail: { error_type: "nsfw" } })).toBeInstanceOf(
      PromptNsfwError,
    );
    expect(errorFromResponse(400, { detail: { error_type: "prompt_nsfw" } })).toBeInstanceOf(
      PromptNsfwError,
    );
  });
  ...
  it("maps the 422 ip_detected error-body shape to IpDetectedError (not the ValidationError fallback)", () => {
    ...
    expect(
      errorFromResponse(422, {
        detail: { error_type: "ip_detected", message: "IP found in media" },
      })?.message,
    ).toBe("IP found in media");
  });
  ...
  it("collapses the minimum-plan error_types into MinimumPlanError with plan/feature in data", () => {
    ...
    expect(
      (
        errorFromResponse(402, {
          detail: { error_type: "ultimate_plan_required" },
        }) as MinimumPlanError
      ).data,
    ).toEqual({ plan: "ultimate" });
    expect(
      (
        errorFromResponse(402, {
          detail: { error_type: "job_minimum_pro_plan_required" },
        }) as MinimumPlanError
      ).data,
    ).toEqual({ plan: "pro" });
    expect(
      (
        errorFromResponse(402, {
          detail: { error_type: "avatar_minimum_pro_plan_required" },
        }) as MinimumPlanError
      ).data,
    ).toEqual({ plan: "pro", feature: "avatar" });
    expect(
      (
        errorFromResponse(402, {
          detail: { error_type: "job_minimum_creator_plan_required" },
        }) as MinimumPlanError
      ).data,
    ).toEqual({ plan: "creator" });
    expect(
      (
        errorFromResponse(402, {
          detail: { error_type: "train_minimum_basic_plan_required" },
        }) as MinimumPlanError
      ).data,
    ).toEqual({ plan: "basic", feature: "train" });
  });

  it("maps the remaining typed billing/plan error_types", () => {
    expect(
      errorFromResponse(402, { detail: { error_type: "non_eligible_vpn_detected" } }),
    ).toBeInstanceOf(VpnDetectedError);
    expect(
      errorFromResponse(402, { detail: { error_type: "not_enough_boost_credits" } }),
    ).toBeInstanceOf(NotEnoughBoostCreditsError);
    expect(
      errorFromResponse(402, {
        detail: { error_type: "batch_rate_limit_reached", concurrent_batches_limit: 2 },
      }),
    ).toBeInstanceOf(BatchRateLimitError);
    expect(
      errorFromResponse(402, { detail: { error_type: "auto_top_up_suspended" } }),
    ).toBeInstanceOf(AutoTopUpSuspendedError);
    expect(
      errorFromResponse(402, {
        detail: { error_type: "grace_daily_limit_reached", data: { type: "pay_less_notice" } },
      }),
    ).toBeInstanceOf(GraceDailyLimitError);
    expect(
      errorFromResponse(402, {
        detail: { error_type: "workspace_member_spend_paused", workspace_id: "w1", user_id: "u1" },
      }),
    ).toBeInstanceOf(WorkspaceMemberSpendPausedError);
    expect(errorFromResponse(402, { detail: { error_type: "account_blocked" } })).toBeInstanceOf(
      AccountSuspendedError,
    );
  });
  ...
  it("maps media_too_large to a base ApiJobError keeping status 413 and maxSizeMb", () => {
    ...
    const err = errorFromResponse(413, {
      detail: { error_type: "media_too_large", max_size_mb: 50 },
    });
  ...
  it("maps unlim_battery_rate_limit_reached with billing context + concurrentBatchesLimit", () => {
    ...
    const err = errorFromResponse(429, {
      detail: {
        error_type: "unlim_battery_rate_limit_reached",
        plan_type: "Team",
        billing_period: "monthly",
        workspace: { type: "shared", role: "member" },
        concurrent_batches_limit: 4,
      },
    });
    expect(err).toBeInstanceOf(ApiJobError);
    ...
    expect(err?.data).toEqual({
      plan: "team",
      billingPeriod: "monthly",
      team: { workspaceType: "Shared", userRole: "Member" },
      concurrentBatchesLimit: 4,
    });
  ...
  it("maps enterprise_group_credit_limit_reached with text + camelCased limit fields", () => {
    ...
    const text =
      "You've reached your weekly workspace group credit limit. Ask your workspace admin to increase the group limit or wait until it resets.";
    const err = errorFromResponse(429, {
      detail: {
        error_type: "enterprise_group_credit_limit_reached",
        text,
        limit_amount: 1000,
        period: "every_week",
        spent_in_period: 990,
  ...
  it("matches KYB by detail.message", () => {
    expect(
      errorFromResponse(403, { detail: { message: "KYB verification required" } }),
    ).toBeInstanceOf(KybVerificationRequiredError);
  });

  it("maps 409 workspace_selection_required, carrying workspaces in data", () => {
    const err = errorFromResponse(409, {
      detail: {
        error_type: "workspace_selection_required",
        workspaces: [{ id: "w1", name: null, type: "private" }],
      },
    });
    expect(err).toBeInstanceOf(WorkspaceSelectionRequiredError);
    expect((err as WorkspaceSelectionRequiredError).data?.workspaces).toEqual([
      { id: "w1", name: null, type: "private" },
    ]);
  });
...
describe("toJSON + errorFromJSON round-trip", () => {
  it("emits a uniform { code, message, status?, data? } shape", () => {
    ...
    expect(err.toJSON()).toEqual({
      ...
      data: { plan: "pro", billingPeriod: "monthly" },
    });
  });
  ...
  it("rehydrates ip_detected and ip_check_rate_limit_reached, preserving the message", () => {
    const ip = errorFromJSON(
      JSON.parse(JSON.stringify(new IpDetectedError("IP found in media").toJSON())),
    );
  ...
  it("rehydrates media-pipeline errors to their classes (registered from the media half)", () => {
    const moderation = errorFromJSON(
      JSON.parse(JSON.stringify(new MediaModerationError("nsfw").toJSON())),
    );
  ...
  it("does not crash on a data-less payload for any registered code", () => {
    ...
    for (const code of [
      ...
      "out_of_credits",
      "rate_limit",
      "workspace_subscription_inactive",
    ]) {
