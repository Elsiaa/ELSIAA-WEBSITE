...
export type AutoTopUpErrorType =
  "auto_top_up_suspended" | "auto_top_up_charge_failed" | "auto_top_up_in_progress";
export type MinimumPlanFeature = "avatar" | "train";
export type GraceLimitReachedType =
  "downgrade_notice" | "pay_less_notice" | "unlock_full_access_notice";
...
export class OutOfCreditsError extends BillingError {
  ...
  constructor(data: BillingContext) {
    super("out_of_credits", "Not enough credits", { data });
  }
}

export class RateLimitError extends BillingError<
  BillingContext & { maxCount?: number; isUnlimited?: boolean }
> {
...
export class SharedTeamSubscriptionInactiveError extends BillingError {
  constructor(data: BillingContext) {
    super("workspace_subscription_inactive", "Team subscription is inactive", { data });
  }
}

export class MinimumPlanError extends BillingError<
  BillingContext & { feature?: MinimumPlanFeature }
> {
  constructor(data: BillingContext & { feature?: MinimumPlanFeature }) {
    ...
    super(
      ...
      `Requires ${plan}${data.feature ? ` for ${data.feature}` : ""}`,
      { data },
    );
  }
}
...
export class PromptNsfwError extends ApiJobError {
  constructor() {
    super("prompt_nsfw", "Prompt flagged as unsafe");
  }
}
...
export class IpDetectedError extends ApiJobError {
  constructor(message = "Protected content is not allowed") {
    super("ip_detected", message);
  }
}
...
export class VpnDetectedError extends ApiJobError {
  constructor() {
    super("vpn_detected", "VPN detected — this model is not available over a VPN");
  }
}

export class KybVerificationRequiredError extends ApiJobError {
  constructor() {
    super("kyb_verification_required", "KYB verification required");
  }
}

export class NotEnoughBoostCreditsError extends ApiJobError {
  constructor() {
    super("not_enough_boost_credits", "Not enough boost credits");
  }
}

export class SubscriptionRenewalFailedError extends ApiJobError {
  constructor() {
    super("subscription_renewal_failed", "Subscription renewal failed");
  }
}

export class JobInProgressError extends ApiJobError {
  constructor() {
    super("job_in_progress", "Another job is already in progress", { status: 409 });
  }
}
...
export class UnlimitedGenerationNotAllowedError extends ApiJobError {
  constructor() {
    super(
      "unlimited_generation_not_allowed",
      "Unlimited generation is not allowed for this request",
    );
  }
}
...
export class GraceDailyLimitError extends ApiJobError<{
  graceType: GraceLimitReachedType;
  graceData?: unknown;
}> {
  constructor(graceType: GraceLimitReachedType, graceData?: unknown) {
    super("grace_daily_limit", "Daily generation limit reached during grace period", {
      data: { graceType, graceData },
    });
  }
}
...
export class WorkspaceMemberSpendPausedError extends ApiJobError<{
  ...
  constructor(data: {
    ...
    super("workspace_member_spend_paused", "Spending is paused for this workspace member", {
      data,
    });
  }
}

export class AccountSuspendedError extends ApiJobError<{
  suspendedCode: "account_suspended" | "account_blocked";
}> {
  constructor(suspendedCode: "account_suspended" | "account_blocked") {
    super(
      "account_suspended",
      suspendedCode === "account_blocked" ? "Account is blocked" : "Account is suspended",
      { data: { suspendedCode } },
    );
  }
}

export class WorkspaceSelectionRequiredError extends ApiJobError<{
  workspaces: Array<{ id: string; name: string | null; type: "private" | "shared" }>;
}> {
  constructor(
    workspaces: Array<{ id: string; name: string | null; type: "private" | "shared" }> = [],
  ) {
    super("workspace_selection_required", "Workspace selection required", {
      status: 409,
      data: { workspaces },
    });
  }
...
export class ApiMessageError extends ApiJobError {
  constructor(message: string) {
    super("api_message", message);
  }
}

export class ValidationError extends ApiJobError<{ issues?: unknown }> {
  constructor(message: string, issues?: unknown) {
    super("validation", message, { status: 422, data: { issues } });
  }
}

export class UnknownSubmitResponseError extends ApiJobError<{
  responseUrl?: string;
  responseBody?: unknown;
}> {
  constructor(status?: number, responseBody?: unknown, responseUrl?: string) {
    super("unknown", `Request failed${status != null ? ` with status ${status}` : ""}`, {
      status,
      data: { responseUrl, responseBody },
    });
  }
}
...
export class ConfirmationRejectedError extends ApiJobError {
  constructor(message = "Submission was not confirmed") {
    super("confirmation_rejected", message);
  }
}
...
export class JobTimeoutError extends ApiJobError<{ generation?: Generation }> {
  ...
  constructor(id: string, timeoutMs: number, generation?: Generation) {
    super("timeout", `Polling job ${id} did not reach a terminal status within ${timeoutMs}ms`, {
      data: { generation },
    });
...
const ERROR_TYPE_MAP: Record<string, (d: Record<string, unknown>) => ApiJobError> = {
  ...
  rate_limit_reached: (d) =>
    new RateLimitError({
      ...billingFrom(d),
      maxCount: num(d.concurrent_jobs_limit),
      isUnlimited: d.use_unlim === true,
    }),
  ...
  unlim_battery_rate_limit_reached: (d) =>
    new ApiJobError("unlim_battery_rate_limit_reached", "Unlimited battery rate limit reached", {
      data: { ...billingFrom(d), concurrentBatchesLimit: num(d.concurrent_batches_limit) },
    }),
  ...
  enterprise_group_credit_limit_reached: (d) =>
    new ApiJobError(
      "enterprise_group_credit_limit_reached",
      str(d.text) ?? "Workspace group credit limit reached",
      {
        data: {
          limitAmount: num(d.limit_amount),
          period: str(d.period),
          spentInPeriod: num(d.spent_in_period),
          cost: num(d.cost),
        },
      },
    ),
  workspace_subscription_inactive: (d) => new SharedTeamSubscriptionInactiveError(billingFrom(d)),
  workspace_member_spend_paused: (d) =>
    new WorkspaceMemberSpendPausedError({
  ...
  free_generation_not_allowed: () =>
    new ApiJobError(
      "free_generation_not_allowed",
      "Free generation is not allowed for this request",
    ),
  // HTTP 403, bare { error_type } (fnf-api src/exceptions/job.py:86-91).
  generation_not_available: () =>
    new ApiJobError("generation_not_available", "Generation is not available"),
  grace_daily_limit_reached: (d) =>
    new GraceDailyLimitError(
      (str(record(d.data)?.type) as GraceLimitReachedType) ?? "unlock_full_access_notice",
      d.data,
    ),
  ...
  media_too_large: (d) =>
    new ApiJobError("media_too_large", "Media is too large", {
      data: { maxSizeMb: num(d.max_size_mb) },
    }),
  // HTTP 408 — content (IP) check timed out, retryable; detail { error_type, text }
  // (fnf-api src/handlers/media.py:282-290).
  request_timeout: (d) =>
    new ApiJobError("request_timeout", str(d.text) ?? "Request timed out — please try again"),
  non_eligible_vpn_detected: () => new VpnDetectedError(),
  not_enough_boost_credits: () => new NotEnoughBoostCreditsError(),
  auto_top_up_suspended: (d) =>
    new AutoTopUpSuspendedError(
      (str(d.error_type) as AutoTopUpErrorType) ?? "auto_top_up_suspended",
    ),
  ...
  workspace_selection_required: (d) =>
    new WorkspaceSelectionRequiredError(
      Array.isArray(d.workspaces)
        ? (d.workspaces as Array<{ id: string; name: string | null; type: "private" | "shared" }>)
        : [],
    ),
...
export function errorFromResponse(status: number, body: unknown): ApiJobError | null {
  ...
  if (error && error.status == null) (error as { status?: number }).status = status;
  return error;
}
...
function mapResponse(status: number, body: unknown): ApiJobError | null {
  ...
  if (typeof safe.detail === "string") return new ApiMessageError(safe.detail);

  const detail = detailObject(safe);
  if (detail?.message === "KYB verification required") return new KybVerificationRequiredError();

  // Minimum-plan: many error_types collapse to one class (plan/feature in data).
  const type = errorType(safe);
  if (type && PLAN_TIERS[type]) return new MinimumPlanError(PLAN_TIERS[type]);
  if (type && ERROR_TYPE_MAP[type]) return ERROR_TYPE_MAP[type](detail ?? {});

  if (status === 422) return new ValidationError(validationMessage(safe), safe.detail);
...
const CODE_TO_CLASS: Record<string, (j: ApiJobErrorJSON<any>) => ApiJobError> = {
  ...
  auto_top_up_suspended: (j) =>
    new AutoTopUpSuspendedError(j.data?.autoTopUpType ?? "auto_top_up_suspended"),
  ...
  workspace_selection_required: (j) =>
    new WorkspaceSelectionRequiredError(j.data?.workspaces ?? []),
  ...
  unknown: (j) =>
    new UnknownSubmitResponseError(j.status, j.data?.responseBody, j.data?.responseUrl),
  ...
  timeout: (j) => {
    ...
    if (j.message) e.message = j.message;
    return e;
  },
};
...
export function registerErrorCode(
  code: string,
  make: (j: ApiJobErrorJSON<any>) => ApiJobError,
): void {
...
export function errorFromJSON(json: ApiJobErrorJSON<any>): ApiJobError {
  ...
  const error = make
    ? make(json)
    : new ApiJobError(json.code, json.message, { status: json.status, data: json.data });
