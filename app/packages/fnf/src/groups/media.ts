...
export function requiresOneOf<R extends string>(role: R, anyOf: readonly R[]): MediaRule<R> {
  return (counts) => {
    if ((counts[role] ?? 0) === 0) return null;
...
export function atLeastOneOf<R extends string>(roles: readonly R[]): MediaRule<R> {
  return (counts) =>
    roles.some((role) => (counts[role] ?? 0) > 0)
      ? null
...
export function maxTotal<R extends string>(roles: readonly R[], max: number): MediaRule<R> {
  return (counts) => {
    ...
    return total <= max
      ? null
      : `media roles ${roles.join("+")} take at most ${max} ref(s) in total, got ${total}`;
  };
}
...
export function dimensionsWithin<R extends string>(
  roles: readonly R[],
  ...
  return (_counts, refs) => {
    ...
    for (const { label, ref } of labeled(roles, refs)) {
      ...
      if (width == null || height == null) continue; // unknown size — nothing to judge locally
      ...
      if (limits.minSide != null && (width < limits.minSide || height < limits.minSide))
        ...
        problems.push(
          `${label} resolution is too low — at least ${limits.minPixels} pixels are required`,
        );
      else if (
        limits.ratio &&
        (width / height < limits.ratio[0] || width / height > limits.ratio[1])
      )
        problems.push(
          `${label} aspect ratio must be between ${limits.ratio[0]} and ${limits.ratio[1]}`,
        );
    }
...
export function durationsWithin<R extends string>(
  ...
  return (_counts, refs) => {
    ...
    for (const { label, ref } of labeled(roles, refs)) {
      ...
      if (seconds == null) continue; // unknown duration — nothing to judge locally
      combined += seconds;
    ...
    if (bounds.total != null && combined > bounds.total)
      problems.push(
        `combined ${roles.join(" + ")} duration must be at most ${bounds.total}s, got ${round1(combined)}s`,
      );
    return problems.length > 0 ? problems : null;
  };
}
...
function* labeled<R extends string>(
  roles: readonly R[],
  refs: Partial<Record<R, MediaRef[]>>,
): Generator<{ label: string; ref: MediaRef }> {
  for (const role of roles) {
    const bucket = refs[role] ?? [];
    for (let i = 0; i < bucket.length; i++) {
      const name = role.replace(/_/g, " ");
      yield {
        label: `${name.charAt(0).toUpperCase()}${name.slice(1)}${bucket.length > 1 ? ` ${i + 1}` : ""}`,
        ref: bucket[i],
      };
    }
  }
...
export function checkMedia(cfg: MediaConfig, media: MediaInput | undefined): MediaIssue[] {
  ...
  for (const [role, limit] of Object.entries(cfg.counts ?? {})) {
    ...
    if (limit?.min != null && count < limit.min)
      issues.push({
        loc: ["media", role],
        msg: `media role '${role}' requires at least ${limit.min} ref(s), got ${count}`,
      });
    if (limit?.max != null && count > limit.max)
      issues.push({
        loc: ["media", role],
        msg: `media role '${role}' takes at most ${limit.max} ref(s), got ${count}`,
      });
  }
...
function toData(ref: MediaRef): WireData {
  ...
  if (ref.url !== undefined) data.url = ref.url;
  return data;
}
...
function pushRole(media: MediaInput, role: string, ref: MediaRef): void {
  ...
  if (Array.isArray(existing)) existing.push(ref);
  else media[role] = [ref];
}
...
export function mediaCodec(cfg: MediaConfig): Codec<MediaInput> {
  return {
    ...
    serialize(media) {
      ...
      const undeclared = Object.keys(media ?? {}).filter(
        (role) => media[role] !== undefined && !cfg.roles.includes(role),
      );
      if (undeclared.length > 0)
        throw new ValidationError(
          `Media role(s) not declared by this job: ${undeclared.join(", ")} (allowed: ${cfg.roles.join(", ")})`,
        );
      ...
      if (cfg.format === "single") {
        if (items.length > 1)
          throw new ValidationError(
            `Media format 'single' takes exactly one ref, got ${items.length}`,
          );
