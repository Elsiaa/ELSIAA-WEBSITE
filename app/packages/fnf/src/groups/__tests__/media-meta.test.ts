...
describe("durationsWithin", () => {
  it("flags per-ref bounds and the combined budget", () => {
    const issues = checkMedia(CFG, {
      ...
      video: [
        ref("v1", { width: 1024, height: 1024, durationSec: 1 }),
        ref("v2", { width: 1024, height: 1024, durationSec: 9 }),
      ],
...
describe("resolveMediaMeta", () => {
  const resolver = async (r: MediaRef) =>
    r.id === "fails" ? Promise.reject(new Error("boom")) : { width: 640, height: 640 };
