...
describe("mediaCodec", () => {
  it("unwrapped: emits bare data array under the field", () => {
    ...
    expect(wire).toEqual({
      input_images: [
        { id: "a", type: "media_input", url: "https://x/a" },
        { id: "b", type: "media_input", url: "https://x/b" },
      ],
    });
  });
  ...
  it("wrapped: emits {role,data} entries under the field", () => {
    ...
    const wire = codec.serialize({
      start_image: ref("a", "start_image"),
      end_image: ref("b", "end_image"),
    });
    expect(wire).toEqual({
      medias: [
  ...
  it("wrapped round-trips role and id", () => {
    ...
    expect(parsed.start_image).toEqual([
      { id: "a", type: "media_input", url: "https://x/a", role: "start_image" },
    ]);
    expect(parsed.end_image).toEqual([
      { id: "b", type: "media_input", url: "https://x/b", role: "end_image" },
    ]);
  });

  it("single: rejects multiple refs instead of silently dropping all but the first", () => {
    const codec = mediaCodec({ field: "input_image", format: "single", roles: ["image"] });
    expect(() => codec.serialize({ image: [ref("a", "image"), ref("b", "image")] })).toThrow(
      /exactly one ref, got 2/,
    );
  });
  ...
  it("rejects an undeclared role instead of silently dropping it (typos, get-then-resubmit)", () => {
    ...
    expect(() => codec.serialize({ style_ref: ref("a", "style_ref") })).toThrow(
      /not declared by this job: style_ref/,
    );
  });
});
...
describe("checkMedia (cardinality + cross-role rules)", () => {
  ...
  it("reports min/max violations with pydantic-shaped issues", () => {
    const issues = checkMedia(cfg, {
      image: [ref("a", "image"), ref("b", "image"), ref("c", "image")],
    });
