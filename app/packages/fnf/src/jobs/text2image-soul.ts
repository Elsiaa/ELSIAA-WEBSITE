...
export const textToImageSoul = defineJob({
  ...
  validate: ({ settings }) => {
    ...
    if (settings.styleId == null)
      issues.push({
        loc: ["settings", "styleId"],
        msg: "styleId is required (or send fashion_factory_id via extra)",
      });
    return issues;
  },
  finalize: (wire) => {
    ...
    if (!row[ratio]) {
      ...
      if (parsed)
        ratio = closestRatioBySize(Object.keys(row), {
    ...
    const enhance =
      wire.image_reference != null
        ? false
        : styleId !== DEFAULT_SOUL_STYLE_ID
          ? true
