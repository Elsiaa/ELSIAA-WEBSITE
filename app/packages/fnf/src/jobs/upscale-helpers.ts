...
export function imageQuality(size: Size): UpscaleQuality {
  ...
  if (longest >= 9000) return "largest";
...
export function firstMediaSizeOrSettings(
...
export function imageFactorIssues(
  ...
  return available.includes(factor as UpscaleImageFactorValue)
    ...
    : [
        {
          loc: ["settings", "factor"],
          msg: `factor must be one of: ${available.join(", ") || "none"} for this image size`,
        },
      ];
}
...
export function scaleVideoSize(factor: VideoScaleFactorValue, inputSize: Size): Size {
  if (factor === "Original" || inputSize.width <= 0 || inputSize.height <= 0) return inputSize;
...
export function localBytedanceVideoCredits(
