...
const multiPromptSchema = z.array(
  z.object({
    prompt: z.string(),
    duration: z.number(),
  }),
...
export const kling3_0 = defineJob({
  ...
  credits: ({ settings }) => {
    if (settings.mode !== "4k") return null;
    const duration =
      settings.multiShots && settings.multiShotMode === "custom" && settings.multiPrompt?.length
  ...
  validate: ({ prompt, media, settings }) => {
    ...
    if (/<<<[^>]+>>>/.test(text) && !hasStart)
      issues.push({
        loc: ["media", "start_image"],
        msg: "Start frame is required when prompt contains element references",
      });
    if (hasEnd && !hasStart)
      issues.push({
        loc: ["media", "start_image"],
        msg: "Start frame is required when end frame is provided",
      });
    if (hasStart && hasEnd && multiShots)
      issues.push({
        loc: ["media", "end_image"],
        msg: "End frame cannot be used with multi-shots",
      });
    ...
    if (multiShots && settings.multiShotMode === "custom") {
      if (multiPrompt.length === 0)
        issues.push({
          loc: ["settings", "multiPrompt"],
          msg: "Multi-shot prompt is required when multi-shot mode is custom",
        });
      if (multiPrompt.length > MAX_SHOTS)
        issues.push({
          loc: ["settings", "multiPrompt"],
          msg: `Multi-shot supports up to ${MAX_SHOTS} shots`,
        });
      ...
      for (const shot of multiPrompt) {
        ...
        if (shotPrompt.length > SHOT_PROMPT_MAX)
          issues.push({
            loc: ["settings", "multiPrompt"],
            msg: `Shot prompt is too long (max ${SHOT_PROMPT_MAX})`,
          });
        if (!shot.duration || shot.duration < MIN_SHOT_DURATION)
          issues.push({
            loc: ["settings", "multiPrompt"],
            msg: `Each shot must have at least ${MIN_SHOT_DURATION}s duration`,
          });
      }
      if (multiPrompt.length > 0 && totalDuration < MIN_TOTAL_DURATION)
        issues.push({
          loc: ["settings", "multiPrompt"],
          msg: `Total duration must be at least ${MIN_TOTAL_DURATION}s`,
        });
      if (totalDuration > 15)
        issues.push({
          loc: ["settings", "multiPrompt"],
          msg: "Total duration must be at most 15s",
        });
    ...
    if (new Set(refs).size > MAX_ELEMENTS || elementIds.length > MAX_ELEMENTS)
      issues.push({
        loc: ["settings", "klingElementIds"],
        msg: `Too many elements (max ${MAX_ELEMENTS}). Please remove some elements from your prompt.`,
      });
  ...
  finalize: (wire, input) => {
    ...
    const ratio = measured
      ? closestRatioBySize(ASPECT_RATIOS, measured)
      : (wire.aspect_ratio as Kling3Ratio);
    ...
    return {
      ...
      duration:
        wire.multi_shots &&
        wire.multi_shot_mode === "custom" &&
        Array.isArray(wire.multi_prompt) &&
        wire.multi_prompt.length > 0
          ? wire.multi_prompt.reduce(
              (sum, shot) => sum + ((shot as { duration?: number }).duration || 0),
              0,
            )
          : wire.duration,
    };
