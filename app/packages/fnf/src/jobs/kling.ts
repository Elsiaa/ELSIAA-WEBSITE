...
export const klingVideo = defineJob({
  ...
  validate: ({ prompt, media, settings }) => {
    ...
    const issues =
      raw.length >= MAX_PROMPT_CHARACTERS
    ...
    const textOnlyAllowed =
      settings.model === KlingModel.v2_5Turbo && settings.resolution === "1080p";
    if (countRefs(media, "input_image") === 0 && !textOnlyAllowed)
      issues.push({
        loc: ["media", "input_image"],
        msg: "input image is required for this model and mode",
      });
    return issues;
  },
  finalize: (wire, input) => {
    ...
    return {
      ...
      enhance_prompt:
        motionId !== KLING_DEFAULT_MOTION_ID
          ? true
          : wire.input_image_end
            ? false
            : (wire.enhance_prompt ?? true),
    };
  },
  restore: (wire) => {
