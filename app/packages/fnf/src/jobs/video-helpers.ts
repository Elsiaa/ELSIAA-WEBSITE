...
export function firstMetaDuration(
  media: MediaInput | undefined,
  roles: readonly string[],
): number | undefined {
  for (const role of roles) {
...
export function integerRange(
  ...
  return [
    { loc: ["settings", field], msg: `${field} must be an integer between ${min} and ${max}` },
  ];
}

export function requiredPromptOrRole(
  input: GenerationInput,
  role: string,
  message: string,
): MediaIssue[] {
  const prompt = (input.prompt?.instruction ?? "").trim();
  if (prompt.length > 0 || refsFor(input.media, role).length > 0) return [];
...
export function extractAngleRefIds(text: string): string[] {
  ...
  for (const match of text.matchAll(/<<<([^>]+)>>>/g)) {
    ...
    if (!value || /^(image|video|audio|cast)_/.test(value)) continue;
