...
describe("nanoBanana2 (image, unwrapped input_images)", () => {
  ...
  it("round-trips wire-named settings: parseGeneration maps aspect_ratio back to aspectRatio", () => {
    const gen = parseGeneration(
      {
        ...
        params: { prompt: "cat", aspect_ratio: "1:1", resolution: "2k" },
      },
      nanoBanana2,
    );
    ...
    expect(buildWireParams(gen.input, nanoBanana2)).toMatchObject({
      prompt: "cat",
      aspect_ratio: "1:1",
      resolution: "2k",
    });
...
describe("seedance2_0 (video, wrapped medias)", () => {
  it("passes settings through unchanged (no normalization) and wraps media with roles", () => {
    ...
    expect(wire.medias).toEqual([
      { role: "image", data: { id: "s1", type: "image_job", url: "https://x/s1" } },
    ]);
  });

  it("enforces cardinality: a second start_image is rejected before any I/O", () => {
    const two = [
      { id: "a", type: "media_input" },
      { id: "b", type: "media_input" },
    ];
    expect(() =>
      buildWireParams(
        {
          model: "seedance_2_0",
          media: { start_image: two },
  ...
  it("enforces the cross-role rule: audio alone (no visuals) is rejected", () => {
    expect(() =>
      buildWireParams(
        {
          model: "seedance_2_0",
          media: { audio: { id: "m", type: "media_input" } },
          settings: { duration: 8, aspectRatio: "auto" },
        },
        seedance2_0,
      ),
  ...
  it("validate hook: prompt is required when no media is attached (seedance + nano)", () => {
    ...
    expect(() =>
      buildWireParams({ model: "nano_banana_2", settings: { aspectRatio: "auto" } }, nanoBanana2),
    ).toThrow(/Prompt is required/);
  });
      ...
      buildWireParams(
        {
          model: "seedance_2_0",
          media: { start_image: { id: "s", type: "media_input" } },
          settings: { duration: 8, aspectRatio: "16:9", batchSize: 9 },
        },
        seedance2_0,
      );
