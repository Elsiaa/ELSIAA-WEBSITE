...
function resolveVideoOutput(wire: Record<string, unknown>, source: Size): Size {
  ...
  if (factor && factor !== VideoScaleFactor.original) return scaleVideoSize(factor, source);
  const resolution = wire.resolution as TopazResolution | undefined;
  if (resolution) return topazFixedVideoSize(resolution);
...
function topazVideoSettings() {
  return {
    ...
    enhancementModel: z._default(
      z.enum(TOPAZ_ENHANCEMENT_MODELS),
      TopazVideoEnhancementModel.starlightPrecise,
    ),
...
function manualParams(wire: Record<string, unknown>) {
  if (wire.parameters !== TopazVideoParameters.manual) return null;
  return {
...
export const topazVideoUpscale = defineJob({
    ...
    return [
      ...
      ...(settings.frameInterpolation === true
        ? [
            ...intRange(
              "frameInterpolationFps",
              settings.frameInterpolationFps as number | undefined,
              15,
              240,
            ),
            ...integerRange(
  ...
  finalize: (wire, input) => {
    ...
    return stripUndefined({
      ...
      enhancement:
        wire.enhancement === false
          ? null
          : {
              model: (wire.enhancementModel ??
                TopazVideoEnhancementModel.starlightPrecise) as EnhancementModel,
              focus_fix_level: wire.focus_fix_level ?? TopazVideoFocusFix.normal,
              params: manualParams(wire),
            },
      frame_interpolation:
        wire.frameInterpolation === true
...
function simpleVideoUpscale(jobSetType: "video_upscale" | "video_deflicker") {
  return defineJob({
    ...
    finalize: (wire, input) => {
      const source = videoSourceSize(input)!;
      const output =
        wire.output_width != null && wire.output_height != null
          ? { width: wire.output_width as number, height: wire.output_height as number }
          : scaleVideoSize(
              (wire.scaleFactor ?? VideoScaleFactor.original) as VideoScaleFactorValue,
              source,
            );
      const rest = { ...wire };
