...
type ConcreteRatio = Exclude<
  (typeof Veo31LiteAspectRatio)[keyof typeof Veo31LiteAspectRatio],
  "auto"
>;
...
export const veo3_1Lite = defineJob({
  ...
  validate: ({ prompt, media, settings }) => {
    ...
    if (hasFLF && settings.duration !== 8)
      issues.push({
        loc: ["settings", "duration"],
        msg: "Duration must be 8 seconds when both first and last frames are provided",
      });
    if (settings.resolution === "1080p" && settings.duration !== 8)
      issues.push({
        loc: ["settings", "duration"],
        msg: "Duration must be 8 seconds for 1080p resolution",
      });
    return issues;
  },
  finalize: (wire, input) => {
