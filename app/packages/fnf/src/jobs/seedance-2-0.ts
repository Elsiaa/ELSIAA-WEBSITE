...
export const seedance2_0 = defineJob({
  ...
  params: {
    ...
    media: {
      ...
      rules: [
        ...
        dimensionsWithin(["image", "start_image", "end_image"], {
          minSide: 300,
          maxSide: 6000,
          ratio: [0.4, 2.5],
        }),
        dimensionsWithin(["video"], {
          minSide: 300,
          maxSide: 6000,
          minPixels: 409_600,
          ratio: [0.4, 2.5],
  ...
  validate: ({ prompt, media, settings }) => {
    ...
    const visuals =
      countRefs(media, "image") +
      countRefs(media, "start_image") +
      countRefs(media, "end_image") +
      countRefs(media, "video");

    if (text.length > MAX_PROMPT_CHARACTERS)
      issues.push({
        loc: ["prompt"],
        msg: `Prompt is too long (max ${MAX_PROMPT_CHARACTERS} characters)`,
      });
    if (!text && visuals === 0)
    ...
    if (FRAME_ROLES.some((role) => countRefs(media, role) > 0) && settings.aspectRatio !== "auto")
      issues.push({
        loc: ["settings", "aspectRatio"],
        msg: "aspectRatio must be 'auto' when a start/end frame is attached",
      });
    ...
    if (settings.mode === "fast" && settings.resolution === "1080p")
      issues.push({
        loc: ["settings", "resolution"],
        msg: "resolution '1080p' is not available in fast mode",
      });
    return issues;
  },
  finalize: (wire, input) => {
