...
export function aspectRatioDimensions(
  ...
  if (w >= h) return { width: even(shortSide * (w / h)), height: shortSide, normalized };
...
export function lookupSize<Res extends string, Ratio extends string>(
  ...
  if (!row) {
    throw new ValidationError(
      `resolution '${resolution}' is not supported (allowed: ${Object.keys(map).join(", ")})`,
  ...
  if (!size) {
    throw new ValidationError(
      `aspect ratio '${ratio}' is not supported (allowed: ${Object.keys(row).join(", ")})`,
 ...
 */
export function closestRatioBySize<R extends string>(
  ratios: readonly R[],
