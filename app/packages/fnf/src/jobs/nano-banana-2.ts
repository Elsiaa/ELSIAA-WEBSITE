...
export const nanoBanana2 = defineJob({
  ...
  params: {
    ...
    settings: {
      aspectRatio: z.wire(
        "aspect_ratio",
        z._default(z.aspectRatio(NANO_BANANA_ASPECT_RATIO_VALUES), "3:4"),
      ),
  ...
  credits: ({ settings }) =>
    (settings.batchSize ?? 1) * CREDITS_PER_IMAGE[settings.resolution ?? "1k"],
  ...
  finalize: (wire, input) => {
    ...
    const { width, height } = getNanoBananaDimensions(
      (wire.resolution ?? "1k") as NanoBananaResolution,
      ratio,
    );
