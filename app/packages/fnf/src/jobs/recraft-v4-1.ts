...
export const recraftV41Image = defineJob({
  ...
  params: {
    ...
    settings: {
      ...
      aspectRatio: z.wire(
        "aspect_ratio",
        z._default(z.aspectRatio(Object.values(RecraftV41AspectRatio)), "1:1"),
      ),
  ...
  finalize: (wire) => {
    ...
    const { width, height } = dimensionsFromRatios(
      ASPECT_RATIOS,
      aspectRatio,
  ...
  restore: (wire) => {
    const modelType =
      typeof wire.model_type === "string" ? (wire.model_type as RecraftModelType) : undefined;
    return modelType && TYPE_TO_MODEL[modelType] ? { model: TYPE_TO_MODEL[modelType] } : {};
  },
});
