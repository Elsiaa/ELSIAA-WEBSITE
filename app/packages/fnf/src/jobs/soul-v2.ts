...
function soulV2BaseSettings(
  defaultStyleId: string,
  defaultAspectRatio: SoulV2AspectRatioValue,
  customReferenceStrength: number | null,
) {
  return {
    ...
    customReferenceStrength: z.wire(
      "custom_reference_strength",
      z._default(z.nullable(z.number()), customReferenceStrength),
    ),
    quality: z._default(z.enum(["720p", "1080p"]), "1080p"),
    aspectRatio: z.wire(
      "aspect_ratio",
      z._default(z.aspectRatio(Object.values(SoulV2AspectRatio)), defaultAspectRatio),
    ),
...
export const soulCinemaImage = defineJob({
  ...
  finalize: (wire) => {
    ...
    if (wire.custom_reference_strength == null) delete wire.custom_reference_strength;
    const { width, height } = getSoulV2Dimensions(quality, aspectRatio);
