...
const badObservabilityOptions: FnfObservabilityOptions = {
  attributes: { unsafe: { nested: true } },
};
...
void client.submit({
  model: "seedance_2_0",
  settings: { duration: 8, aspectRatio: Seedance2AspectRatio.r16x9 },
});

void imageClient.submit({
  model: "text2image_soul_v2",
  settings: { aspectRatio: "3:4", quality: "1080p" },
});
void imageClient.submit({
  model: "soul_cinematic",
  settings: { cinematicVariant: "sultan", batchSize: 2 },
});
void imageClient.submit({
  model: "gpt_image_2",
  prompt: { instruction: "x" },
  settings: { aspectRatio: "auto", quality: "high", resolution: "2k", subModel: "videotape-alpha" },
});
void imageClient.submit({
  model: "seedream_v4_5",
  prompt: { instruction: "x" },
  settings: { aspectRatio: "3:4", quality: "basic" },
});
void imageClient.submit({
  model: "nano_banana_flash",
  prompt: { instruction: "x" },
  media: { image: { id: "u", type: "media_input" } },
...
void imageClient.submit({
  model: "recraft_v4_1",
  prompt: { instruction: "x" },
  settings: { model: RecraftV41Model.vector, resolution: "2k", colors: ["#ffffff"] },
});
void imageClient.submit({
  model: "topaz_image",
  media: { image: { id: "u", type: "media_input" } },
  settings: { sourceWidth: 512, sourceHeight: 512, factor: "x2", model: "Standard V2" },
});
void imageClient.submit({
  model: "topaz_image_generative",
  prompt: { instruction: "restore" },
  media: { image: { id: "u", type: "media_input" } },
  settings: { sourceWidth: 512, sourceHeight: 512, creativity: 2, texture: 2, detail: 1 },
});

void videoClient.submit({
  model: "seedance_2_0",
  prompt: { instruction: "x" },
  settings: { duration: 5, aspectRatio: "16:9", mode: "fast", bitrateMode: "high" },
});
void videoClient.submit({
  model: "kling3_0",
  prompt: { instruction: "x", enhance: true },
  media: { start_image: { id: "s", type: "media_input" } },
  settings: { duration: 5, aspectRatio: "16:9", mode: "std", sound: "off", multiShotMode: "auto" },
});
void videoClient.submit({
  model: "kling3_0_motion_control",
  media: { image: { id: "i", type: "media_input" }, video: { id: "v", type: "video_input" } },
  settings: {
    mode: "pro",
...
void videoClient.submit({
  ...
  prompt: { instruction: "x" },
  settings: { resolution: "1080p", aspectRatio: "3:4", duration: 5, batchSize: 2 },
});
void videoClient.submit({
  model: "grok_video",
  prompt: { instruction: "x" },
  settings: { resolution: "720p", aspectRatio: "auto", duration: 6 },
});
void videoClient.submit({
  model: "grok_video_v15",
  prompt: { instruction: "x" },
  media: { start_image: { id: "s", type: "media_input" } },
  settings: { resolution: "480p", duration: 2 },
});
void videoClient.submit({
  model: "veo3_1_lite",
  prompt: { instruction: "x" },
  media: {
    start_image: { id: "s", type: "media_input" },
    end_image: { id: "e", type: "media_input" },
  },
  settings: { resolution: "720p", aspectRatio: "auto", duration: 8, generateAudio: true },
});
void videoClient.submit({
  model: "wan2_7",
  prompt: { instruction: "x", negative: "blur" },
  settings: { quality: "1080p", aspectRatio: "4:3", duration: 5 },
});
void videoClient.submit({
  model: "topaz_video",
  media: { video: { id: "v", type: "video_input" } },
  settings: {
    sourceWidth: 1280,
    sourceHeight: 720,
    model: "slp-2.5",
...
void videoClient.submit({
  model: "video_upscale",
  media: { video: { id: "v", type: "video_input" } },
  settings: { sourceWidth: 1280, sourceHeight: 720, scaleFactor: "4k" },
});
void videoClient.submit({
  model: "video_deflicker",
  media: { video: { id: "v", type: "video_input" } },
  settings: { sourceWidth: 1280, sourceHeight: 720 },
});
void videoClient.submit({
  model: "bytedance_video_upscale",
  media: { video: { id: "v", type: "video_input" } },
  settings: { sourceWidth: 1280, sourceHeight: 720, resolution: "2k", preset: "aigc", fps: 60 },
});
...
void imageClient.submit({
  model: "nano_banana_flash",
  prompt: { instruction: "x" },
  media: { bogus: { id: "u", type: "media_input" } },
  settings: {},
});
...
void videoClient.submit({
  model: "kling3_0",
  prompt: { instruction: "x" },
  settings: { duration: 5, aspectRatio: "16:9", mode: "turbo" },
});
...
void videoClient.submit({
  model: "topaz_video",
  media: { video: { id: "v", type: "video_input" } },
  settings: { enhancementModel: "unknown" },
});
...
void videoClient.submit({
  model: "video_upscale",
  media: { image: { id: "i", type: "media_input" } },
  settings: {},
});
...
void client.submit({
  ...
  media: { start_image: { id: "u", type: "media_input" } },
});
