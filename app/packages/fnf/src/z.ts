...
export function duration<const V extends readonly number[] = never>(opts: {
  values?: V;
  min?: number;
  max?: number;
}) {
...
): base.ZodMiniDefault<T> &
  (T extends NormalizeType<infer U> ? NormalizeType<U | undefined> : object) {
  return base._default(innerType, defaultValue as never) as base.ZodMiniDefault<T> &
    (T extends NormalizeType<infer U> ? NormalizeType<U | undefined> : object);
}
