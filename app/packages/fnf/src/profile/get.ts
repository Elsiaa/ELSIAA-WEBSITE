...
export async function getProfileUser(ctx: ProfileContext): Promise<ProfileUser | null> {
  return observeAsync(
    ctx.observability,
    "fnf.profile.get_user",
    {},
    async () => mapProfileUser(await ctx.profileAdapter.getUser()),
    {
      successAttributes: (user) => ({
        has_user: user !== null,
...
export async function listProfileWorkspaces(ctx: ProfileContext): Promise<ProfileWorkspace[]> {
  return observeAsync(
    ctx.observability,
    "fnf.profile.list_workspaces",
    {},
    async () => mapProfileWorkspaces(await ctx.profileAdapter.listWorkspaces()),
    {
...
export async function getCurrentProfileWorkspace(
  ctx: ProfileContext,
): Promise<ProfileWorkspace | null> {
  return observeAsync(
    ctx.observability,
    "fnf.profile.get_current_workspace",
    {},
    async () => mapProfileWorkspace(await ctx.profileAdapter.getCurrentWorkspace()),
    {
      successAttributes: (workspace) => ({
...
export async function getProfileWallet(
  ctx: ProfileContext,
): Promise<ProfileWorkspaceWallet | null> {
  return observeAsync(
    ctx.observability,
    "fnf.profile.get_wallet",
    {},
    async () => mapProfileWallet(await ctx.profileAdapter.getWorkspaceWallet()),
    {
...
export async function getProfileCredits(
  ...
  return observeAsync(
    ctx.observability,
    "fnf.profile.get_credits",
    { include_on_demand: options?.includeOnDemand !== false },
    async () => {
