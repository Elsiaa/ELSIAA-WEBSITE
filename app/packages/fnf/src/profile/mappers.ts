...
export function mapProfileUser(raw: unknown): ProfileUser | null {
  ...
  return {
    ...
    workspaceMembershipExists: bool(
      raw.workspace_membership_exists ?? raw.workspaceMembershipExists,
      false,
    ),
    ...
    nextCreditAllocationAt: nullableStr(
      raw.next_credit_allocation_at ?? raw.nextCreditAllocationAt,
    ),
    ...
    qwenCameraControlCredits: nullableNum(
      raw.qwen_camera_control_credits ?? raw.qwenCameraControlCredits,
    ),
    ...
    veo3FastGenerationsCount: num(
      raw.veo3_fast_generations_count ?? raw.veo3FastGenerationsCount,
      0,
    ),
    isProPlanVeo3Available: nullableBool(
      raw.is_pro_plan_veo3_available ?? raw.isProPlanVeo3Available,
    ),
    hideReducedNanoBanana2Concurrent: bool(
      raw.hide_reduced_nano_banana_2_concurrent ?? raw.hideReducedNanoBanana2Concurrent,
      false,
    ),
    ...
    subscriptionDowngradesAt: nullableStr(
      raw.subscription_downgrades_at ?? raw.subscriptionDowngradesAt,
    ),
    lastDailyCreditsAwardedAt: nullableStr(
      raw.last_daily_credits_awarded_at ?? raw.lastDailyCreditsAwardedAt,
    ),
    ...
    showGenerationActivityNotice: nullableBool(
      raw.show_generation_activity_notice ?? raw.showGenerationActivityNotice,
    ),
...
export function mapProfileWorkspace(raw: unknown): ProfileWorkspace | null {
  ...
  return {
    ...
    isEnterpriseSubWorkspace: bool(
      raw.is_enterprise_sub_workspace ?? raw.isEnterpriseSubWorkspace,
      false,
    ),
