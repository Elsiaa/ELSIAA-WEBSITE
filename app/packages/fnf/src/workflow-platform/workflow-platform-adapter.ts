 ...
 */
export function createWorkflowPlatformAdapter(
  options: WorkflowPlatformAdapterOptions = {},
): FnfAdapter {
  ...
  const transport = options.observability
    ? withObservedTransport(baseTransport, options.observability)
    : baseTransport;
  ...
  return {
    ...
    createJobs: ({ jobSetType, params, confirmationToken }) =>
      send("POST", "/jobs/submit", {
    ...
    getJob: async (id) =>
      normalizeJobReadBody(await send("GET", `/jobs/${encodeURIComponent(id)}`)),

    getJobSet: async (id) =>
      normalizeJobSetBody(await send("GET", `/jobs/sets/${encodeURIComponent(id)}`)),

    async listJobs(query) {
      if (query.parentId !== undefined)
        throw new ApiJobError(
          "not_supported",
          "GET /jobs has no parent filter - the Workflow Platform feed cannot list a job set's children (parentId)",
        );
      return normalizeJobListBody(await send("GET", jobsFeedPath(query)));
    },

    estimateCost: ({ jobSetType, params }) =>
      send("POST", "/jobs/cost", {
        job_set_type: jobSetType,
        params,
      }),
    ...
    confirmMedia: (req) =>
      send("POST", `/jobs/media/${encodeURIComponent(req.mediaId)}/confirm`, mediaConfirmBody(req)),
    ...
    switchWorkspace: (req: SwitchWorkspaceRequest) =>
      send("POST", "/workspaces/switch", {
...
async function authorizationHeader(
  getToken: WorkflowPlatformAdapterOptions["getToken"],
): Promise<Record<string, string | undefined>> {
  const token = await getToken?.();
  return token ? { Authorization: `Bearer ${token}` } : {};
}

async function optionalNamedHeader(
  name: string,
  source: OptionalHeaderSource | undefined,
): Promise<Record<string, string | undefined>> {
...
function normalizeJobReadBody(body: unknown): unknown {
  if (isRecord(body) && isRecord(body.job)) return normalizeJobLike(body.job, body.job_set);
  if (isRecord(body)) return normalizeJobLike(body);
  return body;
}
...
function pathWithQuery(path: string, query: Record<string, string | number | undefined>): string {
  const search = new URLSearchParams();
  for (const [key, value] of Object.entries(query)) append(search, key, value);
  const qs = search.toString();
