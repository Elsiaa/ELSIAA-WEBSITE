...
export function normalizeProductJob(
  ...
  const status = normalizeStatus(
    stringValue(job.status) ?? "pending",
    job.ip_check_finished === false,
  );
  ...
  return cleanUndefined({
    ...
    job_set_type:
      stringValue(job.job_set_type) ?? stringValue(set?.type) ?? stringValue(set?.job_set_type),
    job_set_id: stringValue(job.job_set_id) ?? stringValue(set?.id),
    job_set_parent_id:
      nullableStringValue(job.job_set_parent_id) ?? nullableStringValue(set?.parent_id),
    status,
...
export function normalizeJobSetBody(body: unknown): unknown {
  if (Array.isArray(body)) return body.map((item) => normalizeJobLike(item));

  const record = recordValue(body);
  if (!record) return body;

  if (Array.isArray(record.jobs)) return record.jobs.map((item) => normalizeJobLike(item, record));
  if (Array.isArray(record.items))
