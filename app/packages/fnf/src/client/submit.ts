...
export async function submit(
  ...
  return observeAsync(
    ...
    async () => {
      ...
      const generations = settled.flatMap((r) =>
        r.status === "fulfilled" ? generationsFromBody(r.value, entry, input) : [],
      );
...
async function confirmSubmission(
  ...
  if (!ctx.adapter.confirm) return undefined;
  ...
  try {
    ...
    throw err instanceof ApiJobError
      ? err
      : new ConfirmationRejectedError(err instanceof Error ? err.message : undefined);
  }
 ...
 */
export function generationsFromBody(
  body: unknown,
  entry: JobEntry,
  input: GenerationInput,
): Generation[] {
  ...
  if (body && typeof body === "object") {
    ...
    if (typeof obj.id === "string") return [parseGeneration(body as JobResponse, entry)];
  }
  return [];
}
...
function asError(err: unknown): ApiJobError {
  return err instanceof ApiJobError
    ? err
