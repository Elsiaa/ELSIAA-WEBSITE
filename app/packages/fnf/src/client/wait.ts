 ...
 */
export async function waitGenerations(
  ...
  return observeAsync(
    ...
    async () => {
      ...
      const checkFail = (done: Generation): Generation => {
        ...
        if (
          opts?.throwOnFail &&
          (done.status === "failed" || done.status === "nsfw" || done.status === "ip_detected")
        )
          throw new ApiJobError("job_failed", done.failReason ?? `Job ${done.id} ${done.status}`, {
        ...
        await Promise.all([
          ...[...setGroups.entries()].map(async ([jobSetId, members]) => {
            const done = await pollJobSetGroup(ctx, jobSetId, members, {
              ...
              onProgress: (g) => {
                onProgress(g);
                if (isTerminal(g.status)) checkFail(g);
              },
            });
    ...
    {
      successAttributes: (done) => ({
        terminal_count: done.filter((g) => isTerminal(g.status)).length,
      }),
    },
  );
}
