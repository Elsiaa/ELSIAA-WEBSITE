...
export async function getGeneration(
  ...
  return observeAsync(
    ...
    async () => {
      ...
      const entry =
        (body.job_set_type ? ctx.registry.get(body.job_set_type) : undefined) ?? fallbackEntry;
      if (!entry) throw new ApiJobError("unknown_model", `Cannot resolve job type for job ${id}`);
