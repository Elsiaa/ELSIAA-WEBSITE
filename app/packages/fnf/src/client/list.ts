  ...
  return observeAsync(
    ...
    "fnf.job.list",
    {
      ...
      ...(opts.model !== undefined
        ? { model_count: Array.isArray(opts.model) ? opts.model.length : 1 }
        : {}),
      ...(opts.status !== undefined
        ? { status_count: Array.isArray(opts.status) ? opts.status.length : 1 }
        : {}),
    },
    async () => {
      ...
      const items = (body.items ?? body.jobs ?? []).map((item) =>
        parseListItem(ctx, item, opts.type),
      );
    ...
    },
    {
      successAttributes: (result) => ({
        ...
        has_cursor: result.cursor !== undefined,
      }),
    },
  );
}
...
function buildUnknownListResults(item: RawListItem, fallbackType?: OutputType): GenerationResults {
  ...
  if (item.min_result_url) results.minUrl = item.min_result_url;
  else if (item.thumbnail_url) results.thumbnailUrl = item.thumbnail_url;
  return results;
}
