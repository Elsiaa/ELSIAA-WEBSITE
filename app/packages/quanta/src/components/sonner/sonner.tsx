...
function emit(variant: SonnerVariant, title: ReactNode, options: ToastOptions = {}): string {
  ...
  return sonnerManager.add({
    ...
    actionProps: cfg ? { children: cfg.label, onClick: cfg.onClick } : undefined,
  });
}
...
export const toast = Object.assign(
  (title: ReactNode, options?: ToastOptions) => emit("default", title, options),
  {
    ...
    loading: (title: ReactNode, options?: ToastOptions) =>
      emit("loading", title, { duration: 0, ...options }),
...
function Glyph({ variant }: { variant: SonnerVariant }) {
  ...
  switch (variant) {
    ...
    case "loading":
      ...
      return (
        <svg viewBox="0 0 20 20" fill="none" aria-hidden className="size-5 q-sonner-spinner">
          <circle
            cx="10"
            cy="10"
            r="8"
...
function SonnerToast({
  ...
  toast: Primitive.Root.ToastObject<SonnerData>;
  ...
  return (
    <Primitive.Root
      ...
      className={cx("q-sonner", VARIANT_CLASS[variant])}
    >
      ...
      {actionNode != null ? (
        ...
        <Primitive.Action className="q-sonner-action" />
      ) : null}
      <Primitive.Close className="q-sonner-close" aria-label="Dismiss">
...
function ToastList({ swipe }: { swipe: SwipeDirection[] }) {
  ...
  return toasts.map((t) => (
    <SonnerToast key={t.id} toast={t as Primitive.Root.ToastObject<SonnerData>} swipe={swipe} />
  ));
}
...
export function Toaster({
  ...
  const viewportStyle =
    gap == null
      ...
      : ({
          ...style,
          ...({ "--q-sonner-gap": `${gap}px` } as Record<string, string>),
        } as CSSProperties);
  return (
