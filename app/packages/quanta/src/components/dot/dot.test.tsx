...
describe("<Dot>", () => {
  ...
  it("maps the Figma size ramp to fill box + outer stroke width", () => {
    ...
    expect(xs.container.querySelector("span")).toHaveClass(
      "size-q-100",
      "border-q-medium",
    ...
    expect(sm.container.querySelector("span")).toHaveClass(
      "size-q-150",
      "border-q-medium",
      "border-q-background-glass",
    );
    ...
    expect(md.container.querySelector("span")).toHaveClass(
      "size-q-200",
      "border-q-thick",
      "border-q-background-glass",
    );
  ...
  it("opts into pulse / glow and inherits the fill colour for the effect", () => {
    ...
    expect(pulse.container.querySelector("span")).toHaveClass(
      "q-dot-pulse",
      "text-q-palette-mint-bg",
    );
    pulse.unmount();

    const glow = render(<Dot color="red" animation="glow" />);
    expect(glow.container.querySelector("span")).toHaveClass(
      "q-dot-glow",
      "text-q-palette-pink-bg",
    );
  });
});
