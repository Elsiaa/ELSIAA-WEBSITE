...
describe("<NotFound>", () => {
  it("renders icon, title and subtitle slots through Typography (md composite + color)", () => {
    ...
    expect(screen.getByText("Refine your query")).toHaveClass(
      "text-q-caption-sm-regular",
  ...
  it("steps the title / subtitle composite by size (lg → body-sm)", () => {
    ...
    expect(screen.getByText("Big title")).toHaveClass(
      "text-q-body-sm-medium",
      "text-q-text-secondary",
    );
    expect(screen.getByText("Big subtitle")).toHaveClass(
      "text-q-body-sm-regular",
      "text-q-text-tertiary",
    );
  });
  ...
  it("swaps the host element via render, keeping the surface classes", () => {
    render(
      <NotFound
        render={<button type="button" data-testid="dz" />}
        variant="outline"
        title="Drop files"
      />,
    );
