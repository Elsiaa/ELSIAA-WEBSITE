...
function Row({
  ...
  indicator,
  ...props
}: { title: string; indicator?: "check" | "checkbox" | "switch" } & Parameters<
    ...
    <Dropdown.Item {...props}>
      ...
      {props.selectable ? (
        <Dropdown.ItemTrailing>
          <Dropdown.ItemIndicator indicator={indicator} />
        </Dropdown.ItemTrailing>
      ) : null}
    </Dropdown.Item>
  );
...
describe("Dropdown containers", () => {
  ...
  it("applies surface and shape presets to the popup", () => {
    render(
      <Dropdown.Root defaultOpen>
        <Dropdown.Trigger>Open</Dropdown.Trigger>
        <Dropdown.Content surface="solid" shape="panel">
          <Row title="X" />
        </Dropdown.Content>
      </Dropdown.Root>,
    );
    expect(screen.getByRole("menu")).toHaveClass(
      "q-dropdown-content-solid",
      "q-dropdown-content-panel",
    );
  });
});
      ...
      <Dropdown.Root defaultOpen>
        ...
        <Dropdown.Content>
          <Dropdown.Item value="model">
            ...
            <Dropdown.ItemContent>
              <Dropdown.ItemTitleRow>
                <Dropdown.ItemTitle>Model row</Dropdown.ItemTitle>
              </Dropdown.ItemTitleRow>
  ...
  it("fires onSelect for a non-selectable action item", async () => {
    ...
    render(
      <Dropdown.Root defaultOpen>
        <Dropdown.Trigger>Open</Dropdown.Trigger>
        <Dropdown.Content>
          <Row title="Action" onSelect={onSelect} />
        </Dropdown.Content>
      </Dropdown.Root>,
    );
    await user.click(screen.getByRole("menuitem", { name: "Action" }));
  ...
  it("uses the media tile slot for rich rows", () => {
    render(
      <Dropdown.Root defaultOpen>
        <Dropdown.Trigger>Open</Dropdown.Trigger>
        <Dropdown.Content>
          <Dropdown.Item media={<FolderIcon />} title="Model" />
        </Dropdown.Content>
      </Dropdown.Root>,
    );
    expect(
      screen.getByRole("menuitem", { name: /Model/ }).querySelector(".q-dropdown-item-media"),
    ).toBeInTheDocument();
  });
  ...
  it("auto-renders the indicator for a selectable slot row", () => {
    ...
    expect(
      screen
        ...
        .querySelector(".q-menu-item-trailing svg"),
    ).toBeInTheDocument();
    expect(
      screen
        .getByRole("menuitemcheckbox", { name: /Off/ })
        .querySelector(".q-menu-item-trailing svg"),
  ...
  it("toggles internally with NO value and NO handlers (keyed by title)", async () => {
    ...
    render(
      <Dropdown.Root defaultOpen>
        <Dropdown.Trigger>Open</Dropdown.Trigger>
        <Dropdown.Content>
          <Dropdown.Item title="Toggle me" selectable />
        </Dropdown.Content>
      </Dropdown.Root>,
    );
    ...
    expect(screen.getByRole("menuitemcheckbox", { name: /Toggle me/ })).toHaveAttribute(
      "aria-checked",
      "true",
    );
  });
  ...
  it("notifies via onCheckedChange even without a value", async () => {
    ...
    render(
      <Dropdown.Root defaultOpen>
        <Dropdown.Trigger>Open</Dropdown.Trigger>
        <Dropdown.Content>
          <Dropdown.Item title="Notify" selectable onCheckedChange={onCheckedChange} />
        </Dropdown.Content>
      </Dropdown.Root>,
    );
...
describe("Dropdown.Item selection", () => {
  it('selectable + ItemIndicator="checkbox" uses the real Checkbox and stays open on toggle', async () => {
    ...
    await user.click(item);
    expect(screen.getByRole("menuitemcheckbox", { name: /Toggle/ })).toHaveAttribute(
      "aria-checked",
      "true",
    );
  });
  ...
  it("default ItemIndicator (check) shows a trailing check only when checked", () => {
    ...
    expect(
      screen
        .getByRole("menuitemcheckbox", { name: /Sel/ })
        .querySelector(".q-menu-item-trailing svg"),
    ).toBeInTheDocument();
    expect(
      screen
        .getByRole("menuitemcheckbox", { name: /Unsel/ })
        .querySelector(".q-menu-item-trailing svg"),
    ).not.toBeInTheDocument();
  });
});
        ...
        <Dropdown.Content>
          <Dropdown.Sub>
            <Dropdown.SubTrigger ref={triggerRef}>
              <Dropdown.ItemContent>
                <Dropdown.ItemTitleRow>
                  <Dropdown.ItemTitle>More</Dropdown.ItemTitle>
                </Dropdown.ItemTitleRow>
              </Dropdown.ItemContent>
              <Dropdown.ItemTrailing>
                <Dropdown.ItemSubChevron />
              </Dropdown.ItemTrailing>
            </Dropdown.SubTrigger>
            <Dropdown.SubContent>
              <Row title="Nested" />
      ...
      <Dropdown.Root defaultOpen>
        ...
        <Dropdown.Content withSearch>
          <Dropdown.Item value="seedance">
            <Dropdown.ItemContent>
              <Dropdown.ItemTitleRow>
                <Dropdown.ItemTitle>Seedance 2.0</Dropdown.ItemTitle>
              </Dropdown.ItemTitleRow>
            </Dropdown.ItemContent>
          </Dropdown.Item>
          <Dropdown.Item value="kling">
            <Dropdown.ItemContent>
              <Dropdown.ItemTitleRow>
                <Dropdown.ItemTitle>Kling 3.0</Dropdown.ItemTitle>
              </Dropdown.ItemTitleRow>
            </Dropdown.ItemContent>
          </Dropdown.Item>
        </Dropdown.Content>
