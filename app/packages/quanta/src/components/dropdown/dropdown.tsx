...
function Trigger({ className, onPointerEnter, onPointerLeave, ...props }: TriggerProps) {
  const { open, openOnHover, scheduleHoverOpen, scheduleHoverClose, cancelHoverClose } =
    useContext(OpenContext);
  const handlePointerEnter = useCallback<NonNullable<TriggerProps["onPointerEnter"]>>(
    (event) => {
  ...
  const handlePointerLeave = useCallback<NonNullable<TriggerProps["onPointerLeave"]>>(
    (event) => {
      onPointerLeave?.(event);
      if (!event.defaultPrevented && openOnHover) scheduleHoverClose();
    },
...
function Root({
  ...
  const toggle = useCallback(
    (value: string, next: boolean) => {
      const compute = (prev: string[]): string[] => {
        if (selectionMode === "single") return next ? [value] : prev.filter((v) => v !== value);
  ...
  const cancelHoverOpen = useCallback(() => {
    if (hoverOpenTimeoutRef.current == null) return;
  ...
  const updateOpen = useCallback(
    (next: boolean, event?: RootOpenChangeEvent) => {
      cancelHoverOpen();
      cancelHoverClose();
  ...
  const scheduleHoverOpen = useCallback(() => {
    if (!openOnHover || currentOpen) return;
    cancelHoverClose();
    cancelHoverOpen();
  ...
  const handleOpenChange = useCallback<RootOpenChangeHandler>(
    (next, event) => {
...
function Content({
  ...
  const handlePointerEnter = useCallback<NonNullable<ContentProps["onPointerEnter"]>>(
    (event) => {
      onPointerEnter?.(event);
      if (!event.defaultPrevented && openOnHover) cancelHoverClose();
    },
    [cancelHoverClose, onPointerEnter, openOnHover],
  );
  const handlePointerLeave = useCallback<NonNullable<ContentProps["onPointerLeave"]>>(
    (event) => {
      onPointerLeave?.(event);
      if (!event.defaultPrevented && openOnHover) scheduleHoverClose();
    },
  ...
  return (
    <div ref={rootRef} className="q-dropdown-search-wrap">
      <div className="q-dropdown-search">
        <span className="q-dropdown-search-icon">
          <SearchIcon />
        </span>
        <input
          ref={inputRef}
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === "Escape") return; // let it bubble so the menu closes
            e.stopPropagation();
          }}
        />
      </div>
...
function SearchEmpty({ fallback }: { fallback?: ReactNode }) {
  ...
  if (query !== "" && matchCount != null && matchCount.n === 0)
    return (
      <>{fallback ?? <NotFound title="No results found" subtitle="Try a different search" />}</>
    );
  return null;
}
...
function Separator({ className, ...props }: SeparatorProps) {
  return (
    <Primitive.Separator render={<Divider />} className={cx("shrink-0", className)} {...props} />
  );
}
...
function Item(props: ItemProps) {
  ...
  if (query && !matchQuery(itemText(props), query)) return null;
  ...
  const body = hasSlots ? (
    <>
      ...
      {end != null || selectable ? (
        <ItemTrailing>
          {end}
          {selectable ? <ItemIndicator indicator={indicator} /> : null}
        </ItemTrailing>
      ) : null}
    </>
  ) : (
  ...
  if (!selectable) {
    return (
      <ItemContext.Provider
        value={{ checked: false, disabled: disabledState, selectable: false, value }}
      >
        <Primitive.Item
  ...
  const resolvedChecked = controlled
    ? checked
    : rootKeyed
      ? selection!.isSelected(resolvedValue)
      : selfChecked;
  const handleCheckedChange: NonNullable<ItemProps["onCheckedChange"]> = (next, event) => {
    if (rootKeyed) selection!.toggle(resolvedValue, next);
    else if (!controlled) setSelfChecked(next);
  ...
  return (
    <ItemContext.Provider
      value={{
        ...
        selectable: true,
        value: value ?? (resolvedValue || undefined),
      }}
    >
