...
describe("chip() class-builder", () => {
  ...
  it("merges extra classes and drops falsy values", () => {
    expect(chip({ color: "error" }, "is-custom", false)).toBe(
      "q-chip q-chip-error q-chip-sm is-custom",
    );
  });
});
...
describe("<Chip>", () => {
  ...
  it("renders selected state", () => {
    render(
      <Chip color="neutral" size="md" selected>
        Filter
      </Chip>,
    );
  ...
  it("forwards native props and caller classes", () => {
    render(
      <Chip className="is-custom" disabled type="submit">
        Filter
      </Chip>,
    );
  ...
  it("composes start / end slots around the label", () => {
    render(
      <Chip start={<CircleIcon data-testid="lead" />} end={<span data-testid="count">3</span>}>
        Tags
      </Chip>,
    );
