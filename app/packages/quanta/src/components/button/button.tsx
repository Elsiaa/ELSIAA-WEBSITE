...
export function button(options: ButtonOptions = {}, ...extra: ClassValue[]): string {
  ...
  const effectiveSize: ButtonSize =
    size === "xxs" && MARKETING_NO_XXS_VARIANTS.has(variant) ? "xs" : size;
  return cx(
    "q-button",
...
type ButtonProps<E extends ElementType> = ButtonOptions & {
  ...
  /** Trailing slot after the label — any node (a `<Badge>`, chevron, kbd…). */
    ...
    "as" | "asChild" | "className" | "children" | "start" | "end" | keyof ButtonOptions
  >;
...
export function Button<E extends ElementType = "button">(props: ButtonProps<E>) {
  const { as, asChild, variant, size, iconOnly, className, children, start, end, ref, ...rest } =
    props;
  ...
  const render = asChild ? (children as ReactElement) : as ? createElement(as) : undefined;
