...
import ChevronIcon from "@material-symbols/svg-400/outlined/keyboard_arrow_down.svg?react";
...
type RootProps<Value, Multiple extends boolean | undefined> = Primitive.Root.Props<
  Value,
  Multiple
> & {
  /**
...
function Content({
  ...
  const tree = (
    <Primitive.Positioner
      className={cx(
        "q-select-positioner",
        connected && "q-select-positioner-connected",
        positionerClassName,
      )}
      side={side}
    ...
    >
      <Primitive.Popup
        className={cx(
          ...
          CONTENT_SIZE_CLASS[size],
          connected && "q-select-content-connected",
          className,
        )}
        {...props}
      >
