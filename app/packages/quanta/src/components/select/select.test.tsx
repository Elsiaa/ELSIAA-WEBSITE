...
function Basic({
  defaultOpen = false,
  ...rootProps
}: Partial<Parameters<typeof Select.Root>[0]> & { defaultOpen?: boolean }) {
  return (
...
describe("Select popup", () => {
  it("renders the dropdown glass surface (defaultOpen)", () => {
    ...
    expect(screen.getByRole("listbox").closest(".q-select-content")).toHaveClass(
      "q-dropdown-content",
      "q-select-content",
    );
  });
  ...
  it("renders a group label and option rows", () => {
    ...
    expect(screen.getByRole("option", { name: "Soul 2.0" })).toHaveClass(
      "q-menu-item",
      "q-select-item",
    );
  });
...
describe("Select selection", () => {
  ...
  it("marks the selected option with data-selected", () => {
    ...
    expect(screen.getByRole("option", { name: "GPT Image 2" })).not.toHaveAttribute(
      "data-selected",
    );
  });
