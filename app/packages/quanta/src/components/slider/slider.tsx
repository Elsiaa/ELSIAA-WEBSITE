...
export function Slider(rawProps: SliderProps) {
  ...
  const initial = (() => {
    ...
    if (mode === "stepped") return 0;
    return min;
  })();

  const [internal, setInternal] = useState<number>(() =>
    sanitize(initial, mode, steps, min, max, snap),
  );
  ...
  const commitValue = useCallback(
    (next: number) => {
      ...
      if (clean === latestRef.current) return;
  ...
  const valueFromPointer = useCallback(
    (clientX: number): number => {
      ...
      if (rect.width <= 0) return latestRef.current;
  ...
  const fillPct = (() => {
    if (mode === "stepped") return (current / (steps - 1)) * 100;
    if (max === min) return 0;
  ...
  const contTickCount =
    ticksVisible && mode === "continuous" ? ((rawProps as ContinuousSliderProps).steps ?? 0) : 0;
    ...
    >
      ...
      {contTickCount > 0 && (
        <span
          ...
          style={{
            gridTemplateColumns: `repeat(${contTickCount}, minmax(0, 1fr))`,
...
function sanitize(
  ...
  if (mode === "stepped") return clamp(Math.round(value), 0, Math.max(0, steps - 1));
