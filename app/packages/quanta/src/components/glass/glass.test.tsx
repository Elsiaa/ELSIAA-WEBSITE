...
describe("Glass", () => {
  ...
  it("maps blur / elevation / rounded props to their utility classes", () => {
    render(
      <Glass data-testid="g" blur="lg" elevation="raised" rounded="300">
        x
      </Glass>,
    );
    const surface = screen.getByTestId("g");
  ...
  it("opts into the interactive treatment", () => {
    render(
      <Glass data-testid="g" interactive>
        x
      </Glass>,
    );
  ...
  it("applies a tint via the slot system (class + --q-tint var)", () => {
    render(
      <Glass data-testid="g" tint="brand">
        x
      </Glass>,
    );
    const surface = screen.getByTestId("g");
  ...
  it("keeps the caller className last and forwards arbitrary props", () => {
    render(
      <Glass data-testid="g" className="custom" aria-label="panel">
        x
      </Glass>,
    );
    const surface = screen.getByTestId("g");
  ...
  it("swaps the host element via render while keeping the surface", () => {
    render(
      <Glass render={<article />} data-testid="g">
        x
      </Glass>,
    );
    const surface = screen.getByTestId("g");
  ...
  it("forwards a click on an interactive render host", async () => {
    ...
    const onClick = vi.fn();
    render(
      <Glass render={<button type="button" />} interactive onClick={onClick}>
        Press
      </Glass>,
    );
...
describe("glass() recipe", () => {
  ...
  it("reflects options and appends extra classes", () => {
    expect(
      glass({ blur: "sm", elevation: "raised", rounded: "full", interactive: true }, "extra"),
    ).toBe("q-glass q-glass-blur-sm q-glass-raised q-glass-rounded-full q-glass-interactive extra");
  });
});
