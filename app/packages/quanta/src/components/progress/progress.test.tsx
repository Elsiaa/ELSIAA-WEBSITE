...
describe("<Progress> line", () => {
  it("renders N segments and fills them sequentially", () => {
    const { container } = render(
      <Progress variant="line" steps={4} value={60} aria-label="Steps" />,
    );
...
describe("<Progress> dots", () => {
  it("renders a row of N dots (no connectors) with the first round(p·n) filled", () => {
    const { container } = render(
      <Progress variant="dots" steps={10} value={50} aria-label="Steps" />,
    );
  ...
  it("marks all dots complete at 100%", () => {
    const { container } = render(
      <Progress variant="dots" steps={3} value={100} aria-label="Done" />,
    );
    container
      .querySelectorAll(".q-progress-dot")
      .forEach((d) => expect(d).toHaveAttribute("data-state", "complete"));
  });
});
...
describe("<Progress> circular", () => {
  ...
  it("renders N arc segments, the first round(p·n) complete", () => {
    const { container } = render(
      <Progress shape="circular" variant="line" steps={4} value={50} aria-label="Segs" />,
    );
  ...
  it("renders N dots around the ring, the first round(p·n) complete", () => {
    const { container } = render(
      <Progress shape="circular" variant="dots" steps={8} value={50} aria-label="Dots" />,
    );
  ...
  it("renders a center label from children", () => {
    render(
      <Progress shape="circular" value={62} aria-label="x">
        62%
      </Progress>,
    );
