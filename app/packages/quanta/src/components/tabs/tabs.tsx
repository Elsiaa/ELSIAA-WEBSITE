    ...
    >
      {items != null
        ? items.map(({ label, ...tab }, index) => (
            <Tab key={tab.value != null ? String(tab.value) : index} {...tab}>
              {label}
            </Tab>
          ))
        : children}
      {indicator ? (
        <Primitive.Indicator className="q-tabs-indicator" renderBeforeHydration />
      ) : null}
    </Primitive.List>
  );
}
...
function Tab({
  ...
  return (
    <Primitive.Tab
      className={cx("q-tabs-tab", iconOnly && "q-tabs-tab-icon-only", className)}
      {...props}
    >
      <span className={cx("q-tabs-tab-content", contentClassName)}>
        {useSlots ? (
          <>
            ...
            {children != null ? (
              <span className="q-tabs-tab-label">{lockTextWidth(children)}</span>
            ) : null}
...
function Panel({ className, ...props }: PanelProps) {
  return <Primitive.Panel className={cx("q-tabs-panel", className)} {...props} />;
}
