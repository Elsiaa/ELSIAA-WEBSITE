...
function Header({
  ...
  let lead: ReactNode;
  if (children != null) lead = children;
  else if (start != null || end != null)
    lead = (
      <div className="q-vault-header-lead">
        {start}
        {titleNode}
        {end}
      </div>
    );
  else lead = titleNode ?? <span />;
  return (
    ...
    <div className={cx("q-vault-footer", className)} {...props}>
      {children ?? (
        <>
          ...
          {actions != null ? (
            <div className={cx("q-vault-actions", full && "q-vault-actions-full")}>{actions}</div>
          ) : null}
        </>
      )}
    </div>
