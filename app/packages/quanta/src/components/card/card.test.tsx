...
describe("<Card>", () => {
  ...
  it("applies surface + elevation variants", () => {
    const { container } = render(
      <Card surface="solid" elevation="raised">
        x
      </Card>,
    );
  ...
  it("composes Header (title/description/actions), Body and Footer", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Done" }).closest(".q-card-actions"),
    ).toBeInTheDocument();
  ...
  it("forwards className + native div props", () => {
    const { container } = render(
      <Card className="extra" data-testid="c" aria-label="panel">
        x
      </Card>,
    );
    const root = container.firstElementChild!;
  ...
  it("`card()` builds the surface class string for any element", () => {
    ...
    expect(card({ surface: "solid", elevation: "raised" })).toBe(
      "q-card q-card-solid q-card-raised",
    );
