    ...
    <Command label="Test menu">
      ...
      <Command.List>
        ...
        <Command.Group heading="Files">
          ...
          <Command.Item onSelect={() => onSelect("open")}>
            <Command.ItemTitle>Open File</Command.ItemTitle>
          </Command.Item>
        </Command.Group>
...
describe("<Command>", () => {
    ...
    render(
      <Command label="Ref">
        ...
        <Command.List ref={ref}>
          <Command.Item>
            <Command.ItemTitle>One</Command.ItemTitle>
          </Command.Item>
        </Command.List>
      </Command>,
    );
    ...
    const Palette = ({ loading }: { loading: boolean }) => (
      <Command label="L" loading={loading} defaultValue="zzz">
        ...
        <Command.List>
          <Command.Empty>Nothing found.</Command.Empty>
          <Command.Item>
            <Command.ItemTitle>Alpha</Command.ItemTitle>
          </Command.Item>
        </Command.List>
      </Command>
    );
  ...
  it("uses a custom filter when provided", async () => {
    ...
    const filter: CommandFilter = (value, search) =>
    ...
    render(
      <Command label="F" filter={filter}>
        ...
        <Command.List>
          <Command.Item>
            <Command.ItemTitle>Apple</Command.ItemTitle>
          </Command.Item>
          <Command.Item>
            <Command.ItemTitle>Grape</Command.ItemTitle>
          </Command.Item>
        </Command.List>
      </Command>,
    );
    ...
    const Palette = ({ loop }: { loop?: boolean }) => (
      <Command label="Loop" loop={loop}>
        ...
        <Command.List>
          ...
          <Command.Item>
            <Command.ItemTitle>Two</Command.ItemTitle>
          </Command.Item>
        </Command.List>
      </Command>
    );
  ...
  it("composes items from icon / content / title / description / trailing parts", () => {
    ...
    expect(row.querySelector(".q-command-item-content")).toContainElement(
  ...
  it("detail pane reflects the active item and updates on navigation", async () => {
    ...
    render(
      <Command label="Detail">
        ...
        <Command.Body>
          <Command.List>
            <Command.Item detail={<p>Alpha details</p>}>
              <Command.ItemTitle>Alpha</Command.ItemTitle>
            </Command.Item>
            <Command.Item detail={<p>Beta details</p>}>
              <Command.ItemTitle>Beta</Command.ItemTitle>
            </Command.Item>
          </Command.List>
          <Command.Detail />
        </Command.Body>
  ...
  it("renders the two-pane layout: a Workspace list + the self-wrapping Detail pane", async () => {
    ...
    expect(screen.getByText("Alpha details").closest(".q-command-detail")).toHaveClass(
      "q-modal-workspace",
    );
    expect(await screen.findByText("Alpha details")).toBeInTheDocument();
  });
  ...
  it("hides the detail pane when the active item has no detail", async () => {
    ...
    const { container } = render(
      <Command label="Mixed">
        ...
        <Command.Body>
          <Command.List>
            <Command.Item detail={<p>Has detail</p>}>
              <Command.ItemTitle>WithDetail</Command.ItemTitle>
            </Command.Item>
            <Command.Item>
              <Command.ItemTitle>Plain</Command.ItemTitle>
            </Command.Item>
          </Command.List>
          <Command.Detail />
        </Command.Body>
    ...
    await waitFor(() =>
      expect(container.querySelector(".q-command-detail")).not.toBeInTheDocument(),
    );
  });
  ...
  it("footer action label tracks the active item", async () => {
    ...
    render(
      <Command label="Footer label">
        <Command.Input />
        <Command.List>
          <Command.Item action="Run one">
            <Command.ItemTitle>One</Command.ItemTitle>
          </Command.Item>
          <Command.Item action="Run two">
            <Command.ItemTitle>Two</Command.ItemTitle>
          </Command.Item>
        </Command.List>
        <Command.Footer>
          <Command.Action fallback="Pick">
            <span data-testid="kbd">↵</span>
          </Command.Action>
        </Command.Footer>
      </Command>,
    );
      ...
      <Command label="Footer">
        ...
        <Command.List>
          <Command.Item onSelect={onSelect}>
            <Command.ItemTitle>Run</Command.ItemTitle>
          </Command.Item>
        </Command.List>
        <Command.Footer>
          <Command.Action>Go</Command.Action>
        </Command.Footer>
      </Command>,
    );
  ...
  it("Command.Dialog composes the shared Modal shell and size presets", () => {
    render(
      <Command.Dialog defaultOpen size="lg" label="Command menu">
        ...
        <Command.List>
          <Command.Item>
            <Command.ItemTitle>Open dashboard</Command.ItemTitle>
          </Command.Item>
        </Command.List>
        <Command.Footer
          caption="Footer label"
          actions={
            <Command.Action fallback="Select">
              <span>↵</span>
            </Command.Action>
          }
        />
