...
const defaultFilter: CommandFilter = (value, search, keywords) =>
  score(search, `${value} ${keywords}`.trim());
  ...
  const visibleIds = useCallback(() => {
    ...
    return Array.from(
      root.querySelectorAll<HTMLElement>(
        '[data-command-item]:not([hidden]):not([aria-disabled="true"])',
      ),
    ).map((el) => el.id);
  }, []);
...
export function CommandEmpty({
  ...
  return (
    <div className={cx("q-command-empty", className)} role="presentation" {...props}>
      {children}
    </div>
  );
}
...
export function CommandLoading({
  ...
  return (
    <div className={cx("q-command-loading", className)} role="presentation" {...props}>
      {children}
    </div>
  );
}
  ...
  return (
    <GroupContext.Provider value={groupId}>
      <div className={cx("q-command-group", className)} role="group" hidden={!visible} {...props}>
        {heading != null ? (
          <div className="q-command-group-heading" aria-hidden>
            {heading}
          </div>
        ) : null}
        {children}
...
export function CommandItem({
  ...
  const { matched, activeId, setActiveId, register, select, setActiveDetail, setActiveAction } =
    useCommand();
  ...
  useLayoutEffect(() => {
    return register(
      { id, value: resolvedValue, keywords: resolvedKeywords, groupId, disabled },
      onSelect,
    );
  ...
  return (
    <div className={cx("q-command-footer", className)} {...props}>
      {children ?? (
        <>
          {caption != null ? (
            <span className="q-command-footer-caption">{caption}</span>
          ) : (
            <span aria-hidden />
          )}
    ...
    <CommandRoot label={label} className="q-command-dialog-context" {...commandProps}>
      <Modal.Root open={isOpen} onOpenChange={setOpen}>
        ...
        >
          ...
          <Modal.Body padded={false}>{body}</Modal.Body>
          {footer != null ? (
            <Modal.Footer full={footer.full}>
              {footer.children ?? (
                <>
                  ...
                  {footer.actions != null ? (
                    <Modal.FooterActions full={footer.full}>{footer.actions}</Modal.FooterActions>
                  ) : null}
                </>
              )}
            </Modal.Footer>
