...
describe("<Modal> composition", () => {
  it("renders the Base UI dialog with composed quanta parts", () => {
    ...
    expect(screen.getByText("Body content")).toHaveClass(
      "q-modal-workspace",
      "q-modal-workspace-padded",
    );
    expect(screen.getByText("Footer caption")).toHaveClass("q-modal-caption");
    expect(
      screen.getByRole("button", { name: "Confirm" }).closest(".q-modal-actions"),
    ).not.toBeNull();
  ...
  it("composes the back / search / tabs header layouts", () => {
    ...
    expect(
      search.container.ownerDocument.querySelector(".q-modal-search-icon .q-icon"),
    ).not.toBeNull();
    search.unmount();
  ...
  it("wraps plain body content in a single window, but keeps explicit Workspaces", () => {
    const single = render(
      <Modal.Root defaultOpen>
        <Modal.Content aria-label="Single">
          <Modal.Body>Plain</Modal.Body>
        </Modal.Content>
      </Modal.Root>,
    );
  ...
  it("stretches footer actions when full", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Done" }).closest(".q-modal-actions-full"),
    ).not.toBeNull();
  });
