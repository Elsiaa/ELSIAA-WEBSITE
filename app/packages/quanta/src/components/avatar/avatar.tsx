...
export type AvatarColor =
  "neutral" | "orange" | "mint" | "blue" | "pink" | "purple" | "brown" | "yellow";
...
export function Avatar({
  ...
  const avatarColor: AvatarColor = pending
    ? "neutral"
    : (color ?? (src ? "neutral" : autoColor(alt)));
    ...
    <Primitive.Root
      className={(state) =>
        cx(
          ...
          pending
            ? cx(
                "border-dashed border-q-transparent-light-30 text-q-text-primary",
                PENDING_BORDER[sizeKey],
              )
    ...
    >
      {src && !pending ? (
        <Primitive.Image
          ...
          decoding={decoding}
          className={cx(
            "pointer-events-none absolute inset-0 size-full rounded-q-full object-cover",
            imageClassName,
          )}
          {...restImageProps}
        />
...
function initials(name: string | undefined, size: AvatarSizeKey): string {
  if (!name) return "";
  if (size === "xs" || size === "xxs") return name.trim()[0]?.toUpperCase() ?? "";
  return name
    .trim()
    .split(/\s+/)
    .slice(0, 2)
