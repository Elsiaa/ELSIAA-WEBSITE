...
function Row({
  title,
  ...props
}: { title: string } & Parameters<typeof NavigationMenu.MenuItem>[0]) {
  return (
...
function MediaRow({
  ...
  media,
  ...props
}: { title: string; subtitle?: string; media?: ReactNode } & Parameters<
  typeof NavigationMenu.MenuItem
>[0]) {
  return (
    <NavigationMenu.MenuItem {...props}>
      ...
      <NavigationMenu.MenuItemContent>
        ...
        {subtitle ? (
          <NavigationMenu.MenuItemDescription>{subtitle}</NavigationMenu.MenuItemDescription>
        ) : null}
      </NavigationMenu.MenuItemContent>
    </NavigationMenu.MenuItem>
  );
...
describe("<NavigationMenu>", () => {
  ...
  it("renders Figma-sized column menus with large media rows", async () => {
    ...
    await waitFor(() =>
      expect(document.querySelector(".q-nav-menu-size-image")).toBeInTheDocument(),
    );
  ...
  it("renders custom menu content without the grid wrapper", async () => {
    ...
    expect(
      document.querySelector(".q-nav-menu-layout-custom .q-nav-menu-grid"),
  ...
  it("can render menu content statically outside a root", () => {
    render(
      <NavigationMenu.Menu standalone size="plugins" layout="columns">
        <NavigationMenu.Group>
          ...
          <MediaRow
            title="Premiere Pro"
            subtitle="Higgsfield inside Premiere"
            href="/premiere"
            interactive={false}
          />
        </NavigationMenu.Group>
    ...
    expect(screen.getByRole("link", { name: /premiere pro/i })).toHaveAttribute(
      "href",
      "/premiere",
    );
    expect(screen.getByTestId("media")).toHaveClass("q-nav-menu-media");
    ...
    const { container } = render(
      <NavigationMenu.Root>
        ...
        <NavigationMenu.Actions>
          <NavigationMenu.Action iconOnly aria-label="Search">
            <span />
    ...
    render(
      <NavigationMenu.Root>
        <NavigationMenu.List>
          ...
          <NavigationMenu.Item>
            <NavigationMenu.Link href="/docs" ref={linkRef}>
              Docs
            </NavigationMenu.Link>
          </NavigationMenu.Item>
        </NavigationMenu.List>
  ...
  it("marks the current section with active (aria-current=page) on a link and a trigger", () => {
    render(
      <NavigationMenu.Root>
        <NavigationMenu.List>
          <NavigationMenu.Item>
            <NavigationMenu.Link href="/image" active>
              Image
            </NavigationMenu.Link>
          </NavigationMenu.Item>
          <NavigationMenu.Item>
  ...
  it("Action swaps its element via render, keeping the pill styling", () => {
    render(
      <NavigationMenu.Root>
        <NavigationMenu.Actions>
          <NavigationMenu.Action render={<a href="/custom" data-testid="custom" />}>
            Custom
          </NavigationMenu.Action>
        </NavigationMenu.Actions>
      </NavigationMenu.Root>,
