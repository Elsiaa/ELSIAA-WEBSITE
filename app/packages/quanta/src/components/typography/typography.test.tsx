...
describe("<Typography>", () => {
  ...
  it("renders the element named by `as`", () => {
    render(
      <Typography as="h1" variant="display-lg-bold">
        Big
      </Typography>,
    );
  ...
  it("forwards native props to the rendered element", () => {
    render(
      <Typography as="span" title="tip" data-testid="t">
        x
      </Typography>,
    );
  ...
  describe("typography() recipe", () => {
    ...
    it("composes variant + color + truncate + extra in order", () => {
      expect(
        typography({ variant: "label-md-semi-bold", color: "brand", truncate: true }, "extra"),
      ).toBe("text-q-label-md-semi-bold text-q-text-brand truncate extra");
    });
  });
});
