...
function assertInjectable(name: string, tokens: ThemeTokens): void {
  ...
  for (const [key, val] of Object.entries(tokens)) {
    if (!NAME_OR_KEY.test(key))
      throw new Error(
        `defineTheme: invalid token key '${key}' in theme '${name}' — use letters, digits, '-', '_'`,
      );
    if (typeof val !== "string" || VALUE_FORBIDDEN.test(val))
      throw new Error(
        `defineTheme: invalid value for token '${key}' in theme '${name}' — a CSS value can't contain ';', '{', '}' or control characters`,
      );
  }
}
...
export function defineTheme(
  ...
  if (options.persist !== false) writePersistedTheme(storage, name, tokens);
}
...
function injectStyleTag(
  ...
  const styleNonce =
    options.styleNonce ?? (existing instanceof HTMLStyleElement ? existing.nonce : undefined);
...
function removePersistedTheme(storage: ThemeStorage, name: string): void {
  ...
  delete all[name];
  if (Object.keys(all).length === 0) storage.remove(THEMES_STORAGE_KEY);
  else storage.set(THEMES_STORAGE_KEY, JSON.stringify(all));
}
