...
describe("defineTheme — injection guard", () => {
  it("injects a :where block for valid tokens (the happy path still works)", () => {
    ...
    expect(tag?.textContent).toContain(
      "--hf-color-glow: linear-gradient(90deg, #000 0%, #fff 100%);",
    );
  });

  it.each([
    [
      "a `}` in a value escapes the block",
      { "surface-default": "#fff} body{background:url(//evil)" },
    ],
    [
      "a `;` in a value smuggles a sibling declaration",
      { "surface-default": "red; background-image: url(//evil)" },
    ],
  ...
  it("rejects a token key that is not slug-shaped", () => {
    expect(() => defineTheme("t", { "a:red;b": "#fff" }, { persist: false })).toThrowError(
      /invalid token key/,
    );
  });

  it("rejects a theme name that would escape the attribute selector", () => {
    expect(() => defineTheme('x"]{}', { a: "#fff" }, { persist: false })).toThrowError(
      /invalid theme name/,
    );
  });
  ...
  it("a bad redefinition does NOT destroy the valid theme it tried to replace", () => {
    ...
    expect(() => defineTheme("keeper", { a: "bad}" }, { persist: false })).toThrowError(
      /invalid value/,
    );
