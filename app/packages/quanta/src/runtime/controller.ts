...
import { DEFAULT_OVERRIDE_STORAGE_KEY, DEFAULT_STORAGE_KEY } from "./storage-keys.ts";
...
export class ThemeController {
  ...
  #resolveTheme(pref: ThemePref): string {
    if (pref !== "auto") return `${this.#brand}-${pref}`;
  ...
  setOverride(theme: string | null): void {
    ...
    if (this.#override === theme) return;
    this.#override = theme;
    if (this.#persistOverride) {
      if (theme === null) this.#overrideStorage.remove(this.#overrideStorageKey);
      else this.#overrideStorage.set(this.#overrideStorageKey, theme);
    }
    this.#apply();
  }
