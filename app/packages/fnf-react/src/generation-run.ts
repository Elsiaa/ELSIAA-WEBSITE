...
export class GenerationRun<Input> extends ExternalStore {
  ...
  constructor(
    private readonly client: GenerationRunClient<Input>,
  ...
  async start(input: Input): Promise<Generation[]> {
    ...
    try {
      ...
      if (run !== this.active) return done; // superseded while settling — the newer run owns the state
      ...
      if (run !== this.active) return []; // a newer start() owns the state now
