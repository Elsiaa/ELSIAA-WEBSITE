...
} from "@higgsfield/fnf/observability";
...
export class AttachmentsController extends ExternalStore {
  ...
  add(input: File | MediaRef | Array<File | MediaRef>, opts?: AttachmentUploadOptions): string[] {
    ...
    observeEvent(this.observability, "fnf.react.attachments.add", {
      ...
      ...((opts?.role ?? this.opts.upload?.role)
        ? { role: opts?.role ?? this.opts.upload?.role ?? null }
        : {}),
    });
      ...
      if (entry instanceof File) {
        ...
        this._items = [
          ...this._items,
          {
            key,
            file: entry,
            role: opts?.role ?? this.opts.upload?.role,
            previewUrl,
            status: "uploading",
          },
        ];
  ...
  clear(): void {
    observeEvent(this.observability, "fnf.react.attachments.clear", {
  ...
  private async upload(key: string, file: File, opts?: AttachmentUploadOptions): Promise<void> {
    ...
    const measured =
      this.opts.measure === false || !previewUrl
    ...
    try {
      ...
      if (ref.url) this.revokeLocalUrl(key);
      ...
      this.patch(key, {
        ref,
        status: "ready",
        previewUrl: ref.url ?? this.localUrls.get(key),
        moderation: result.moderation,
      });
    } catch (err) {
      const error =
        err instanceof ApiJobError
          ? err
          : new ApiJobError("unexpected", err instanceof Error ? err.message : String(err));
  ...
  private patch(key: string, changes: Partial<Attachment>): void {
    ...
    if (at < 0) return; // removed while uploading — drop the result
    this._items = [
      ...this._items.slice(0, at),
      { ...this._items[at], ...changes },
      ...this._items.slice(at + 1),
    ];
    this.commit();
  }
