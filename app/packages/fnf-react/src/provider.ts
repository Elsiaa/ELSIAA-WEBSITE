...
export function createFnfReactClients<const Jobs extends readonly JobEntry[]>(
  ...
  const observability = config.observability
    ? observabilityOptionsFromContext(createObservabilityContext(config.observability))
    : undefined;
  return {
    ...
    profileClient: createProfileClient({
      profileAdapter,
      ...(observability ? { observability } : {}),
    }),
    jobs: config.jobs,
...
export function useFnf<
  Jobs extends readonly JobEntry[] = readonly JobEntry[],
>(): FnfReactClients<Jobs> {
  const value = use(FnfContext);
  if (!value) throw new Error("useFnf must be used inside <FnfProvider>");
  return value as FnfReactClients<Jobs>;
}

export function useFnfJobClient<
  Jobs extends readonly JobEntry[] = readonly JobEntry[],
>(): JobClient<Jobs> {
