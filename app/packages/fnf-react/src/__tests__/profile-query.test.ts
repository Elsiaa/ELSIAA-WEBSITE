...
describe("profile query options", () => {
  it("fetches profile pieces through the SDK profile client into scoped keys", async () => {
    ...
    await expect(qc.fetchQuery(profileUserQueryOptions(client, scope))).resolves.toMatchObject({
      id: "u1",
      workspaceId: "w1",
    });
    await expect(qc.fetchQuery(profileWorkspacesQueryOptions(client, scope))).resolves.toHaveLength(
      1,
    );
    await expect(
      qc.fetchQuery(profileCurrentWorkspaceQueryOptions(client, scope)),
    ).resolves.toMatchObject({ id: "w1" });
    await expect(qc.fetchQuery(profileWalletQueryOptions(client, scope))).resolves.toMatchObject({
      subscriptionBalance: 500,
    });
    await expect(qc.fetchQuery(profileCreditsQueryOptions(client, scope))).resolves.toMatchObject({
      totalAvailableCredits: 7,
    });
    await expect(qc.fetchQuery(profileSnapshotQueryOptions(client, scope))).resolves.toMatchObject({
      user: { id: "u1" },
      wallet: { id: "w1" },
    });
    ...
    expect(fnfKeys.profileCredits({ ...scope, includeOnDemand: false })).toEqual([
      "fnf",
      "scope",
      "u1:w1",
      "profile",
    ...
    expect(fnfKeys.profileSnapshot({ ...scope, includeOnDemand: false })).toEqual([
      "fnf",
      "scope",
      "u1:w1",
      "profile",
  ...
  it("writes a composed snapshot into every profile cache leaf", () => {
    ...
    expect(qc.getQueryData(fnfKeys.profileWorkspaces({ scopeKey: "u1:w1" }))).toEqual([
      { id: "w1" },
    ]);
    expect(qc.getQueryData(fnfKeys.profileCurrentWorkspace({ scopeKey: "u1:w1" }))).toEqual({
      id: "w1",
    });
    expect(qc.getQueryData(fnfKeys.profileWallet({ scopeKey: "u1:w1" }))).toEqual({ id: "w1" });
    expect(qc.getQueryData(fnfKeys.profileCredits({ scopeKey: "u1:w1" }))).toEqual({
...
describe("workspace switch mutation options", () => {
  it("switches context, clears old scoped generation caches, writes next profile snapshot, and notifies the host", async () => {
    ...
    qc.setQueryData(fnfKeys.jobs({}, { scopeKey: "u:w1" }), {
      pages: [{ items: [gen("old", "queued")] }],
