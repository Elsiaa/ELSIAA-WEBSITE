...
describe("FnfProvider and client creation", () => {
  it("creates stable SDK clients from explicit adapters", async () => {
    ...
    await expect(clients.profileClient.getUser()).resolves.toMatchObject({
      id: "u1",
      workspaceId: "w1",
    });
  });
  ...
  it("passes shared observability into created clients", async () => {
    ...
    await clients.jobClient.cost({
      model: "demo",
      prompt: { instruction: "private" },
    ...
    expect(events.map((event) => event.name)).toEqual(
      expect.arrayContaining(["fnf.job.cost", "fnf.profile.get_user"]),
    );
...
describe("request helpers", () => {
  it("builds a local wire preview or returns a typed SDK error", () => {
    ...
    if (!bad.ok) expect(bad.error.code).toBe("unknown_model");
  });
  ...
  it("caches cost estimates under scoped request keys", async () => {
    ...
    await expect(
      qc.fetchQuery(costQueryOptions(client, input, { scopeKey: "u1:w1" })),
