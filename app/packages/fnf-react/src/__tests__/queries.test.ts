...
function evalWith<T>(fn: unknown, data: T): number | false | undefined {
  return (fn as (query: { state: { data?: T } }) => number | false | undefined)({
    state: { data },
  });
}
...
describe("generationQueryOptions", () => {
  it("keeps old unscoped key shapes and adds scoped variants additively", () => {
    ...
    expect(fnfKeys.jobSet("set-1", { scopeKey: "u:w" })).toEqual([
      "fnf",
      "scope",
      "u:w",
      "job-set",
    ...
    expect(fnfKeys.jobs({ type: "video" }, { scopeKey: "u:w" })).toEqual([
      "fnf",
      "scope",
      "u:w",
      "jobs",
  ...
  it("fetches through the client into the contract key", async () => {
    ...
    const data = await qc.fetchQuery(
      generationQueryOptions({ get: async (id) => gen(id, "completed") }, "g1"),
    );
...
describe("jobsFeedQueryOptions", () => {
  ...
  it("walks the cursor: the query is the identity, the cursor is the page param (verbatim to list)", async () => {
    ...
    const data = await qc.fetchInfiniteQuery({
      ...jobsFeedQueryOptions(client, { type: "video", size: 2 }),
      pages: 2,
    });
  ...
  it("keeps scoped feeds separate from unscoped feeds", async () => {
    ...
    const qc = new QueryClient();
    const data = await qc.fetchInfiniteQuery({
      ...jobsFeedQueryOptions(client, { type: "image" }, { scopeKey: "u:w" }),
      pages: 1,
    });
