...
function statuses<I>(run: GenerationRun<I>) {
  const seen: string[] = [];
  run.subscribe(() => {
    if (seen[seen.length - 1] !== run.status) seen.push(run.status);
  });
  return seen;
}
...
describe("GenerationRun", () => {
  ...
  it("starting again supersedes the previous run (late results are dropped)", async () => {
    ...
    const run = new GenerationRun({
      ...
      wait: async (gens) => {
        if (gens[0].id === "old") return new Promise((resolve) => (resolveFirst = resolve));
        return gens;
      },
    });
  ...
  it("emits safe observability events for lifecycle transitions", async () => {
    ...
    await run.start({
      model: "demo",
      prompt: { instruction: "private prompt" },
      settings: { aspectRatio: "1:1" },
    });
    run.reset();
