    ...
    async upload(input: UploadInput): Promise<UploadResult> {
      ...
      return {
        ref: {
          id: `m${uploads.length}`,
          type: "media_input",
          url: `https://cdn/m${uploads.length}.png`,
...
describe("AttachmentsController", () => {
  it("files preview immediately, upload in background, and become submit-ready refs", async () => {
    ...
    expect(uploads[0]).toMatchObject({
      ...
      role: "start_image",
      forceIpCheck: true,
      throwOnModeration: false,
    });
    expect(refs).toHaveLength(1);
    expect(attachments.items[0]).toMatchObject({
      status: "ready",
  ...
  it("a failed upload carries the typed error and is retryable", async () => {
    let attempts = 0;
    const { media, release } = fakeMedia(() =>
      attempts++ === 0 ? new Error("socket hang up") : {},
    );
    const attachments = new AttachmentsController(media, { measure: false });
    const [key] = attachments.add(file());
    release();
    ...
    expect(attachments.items[0]).toMatchObject({
      status: "failed",
      error: { code: "unexpected", message: "socket hang up" },
    });
