...
describe("applyGenerations — the single write door", () => {
  it("folds one snapshot into EVERY cache entry that holds it: job, job set, all feeds", () => {
    ...
    qc.setQueryData(fnfKeys.jobSet("set-1"), [
      gen("a", "queued", { jobSetId: "set-1" }),
      gen("b", "queued", { jobSetId: "set-1" }),
    ]);
    ...
    const fresh = gen("a", "completed", {
      jobSetId: "set-1",
      results: { rawUrl: "https://x/a.png" },
    });
    applyGenerations(qc, [fresh]);

    expect(qc.getQueryData<Generation>(fnfKeys.job("a"))?.status).toBe("completed");
    expect(qc.getQueryData<Generation[]>(fnfKeys.jobSet("set-1"))?.map((g) => g.status)).toEqual([
      "completed",
      "queued",
    ]);
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({ type: "image" }))?.pages[0].items[0]
        .status,
    ).toBe("completed");
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({}))?.pages[0].items[1].status,
    ).toBe("completed");
  });
  ...
  it("updates, never seeds: ids nothing holds are ignored, absent keys are not created", () => {
    ...
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({}))?.pages[0].items.map((g) => g.id),
    ).toEqual(["a"]);
  });
  ...
  it("the terminal anti-regress holds at the door: a stale tick cannot roll back a feed tile", () => {
    ...
    qc.setQueryData(
      fnfKeys.jobs({}),
      feedData([[gen("a", "completed", { results: { rawUrl: "https://x/a.png" } })]]),
    );

    applyGenerations(qc, [gen("a", "in_progress")]);

    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({}))?.pages[0].items[0].status,
    ).toBe("completed");
  });
  ...
  it("updates only the named scope when scoped", () => {
    ...
    applyGenerations(qc, [gen("a", "completed", { results: { rawUrl: "https://x/a.png" } })], {
      scopeKey: "u:w",
    });

    expect(qc.getQueryData<Generation>(fnfKeys.job("a"))?.status).toBe("queued");
    expect(qc.getQueryData<Generation>(fnfKeys.job("a", { scopeKey: "u:w" }))?.status).toBe(
      "completed",
    );
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({}, { scopeKey: "u:w" }))?.pages[0]
        .items[0].status,
    ).toBe("completed");
  });
  ...
  it("removes generation queries for one scope without touching profile or other scopes", () => {
    ...
    qc.setQueryData(fnfKeys.profileSnapshot({ scopeKey: "old" }), {
      user: null,
      workspaces: [],
...
describe("prependGenerations — explicit optimistic insert", () => {
  it("inserts fresh submits at the head of the named feed, deduplicated", () => {
    ...
    const items = qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({ type: "video" }))
      ?.pages[0].items;
  ...
  it("targets ONLY the named feed — fan-out across feeds is the app's explicit policy", () => {
    ...
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({ type: "video" }))?.pages[0].items,
    ).toHaveLength(1);
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(fnfKeys.jobs({}))?.pages[0].items,
    ).toHaveLength(0);
  });
  ...
  it("can optimistically insert into a scoped feed", () => {
    ...
    expect(
      qc.getQueryData<InfiniteData<ListResult>>(
        fnfKeys.jobs({ type: "video" }, { scopeKey: "u:w" }),
      )?.pages[0].items,
    ).toHaveLength(1);
    expect(qc.getQueryData(fnfKeys.jobs({ type: "video" }))).toBeUndefined();
  });
