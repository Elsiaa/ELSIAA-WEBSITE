...
export function createMemoryMediaAdapter(): MediaBackend {
  ...
  return {
    ...
    async getUploadUrl({ type }) {
      ...
      return {
        id,
        type: REF_TYPE_BY_UPLOAD[type],
        url: `memory://${type}/${id}`,
        upload_url: `memory://upload/${id}`,
      };
    },
...
export function createMemoryProfileAdapter(
  options: MemoryProfileAdapterOptions = {},
): ProfileBackend {
  const user = { ...DEFAULT_USER, ...options.user };
  const workspaces =
    options.workspaces?.map((item) => ({ ...item })) ??
    DEFAULT_WORKSPACES.map((item) => ({ ...item }));
