...
export const fnfKeys = {
  ...
  job: (id: string, opts?: FnfScopeOptions) =>
    opts?.scopeKey
      ? ([...scoped(opts.scopeKey, "job"), id] as const)
      : (["fnf", "job", id] as const),
  /** All members of one job set (`client.getSet`). */
  jobSet: (jobSetId: string, opts?: FnfScopeOptions) =>
    opts?.scopeKey
      ? ([...scoped(opts.scopeKey, "job-set"), jobSetId] as const)
      : (["fnf", "job-set", jobSetId] as const),
  ...
  jobs: (query: JobsQuery = {}, opts?: FnfScopeOptions) =>
    opts?.scopeKey
      ? ([...scoped(opts.scopeKey, "jobs"), query] as const)
      : (["fnf", "jobs", query] as const),
  cost: (input: unknown, opts?: FnfScopeOptions) =>
    opts?.scopeKey
      ? ([...scoped(opts.scopeKey, "cost"), input] as const)
      : (["fnf", "cost", input] as const),
  profileRoot: ["fnf", "profile"] as const,
  profile: (opts?: FnfScopeOptions) =>
    opts?.scopeKey ? scoped(opts.scopeKey, "profile") : (["fnf", "profile"] as const),
  profileSnapshot: (opts?: FnfCreditsKeyOptions) =>
    opts?.includeOnDemand === undefined
      ? ([...fnfKeys.profile(opts), "snapshot"] as const)
      : ([
          ...fnfKeys.profile(opts),
          "snapshot",
          { includeOnDemand: opts.includeOnDemand },
        ] as const),
  ...
  profileCurrentWorkspace: (opts?: FnfScopeOptions) =>
    [...fnfKeys.profile(opts), "current-workspace"] as const,
  profileWallet: (opts?: FnfScopeOptions) => [...fnfKeys.profile(opts), "wallet"] as const,
  profileCredits: (opts?: FnfCreditsKeyOptions) =>
    opts?.includeOnDemand === undefined
